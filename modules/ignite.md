...
| `probes/` | 6 probe scripts guarding this directory's jobs: `probe-detach-env.py`, `probe-jobcontain-degrade.py`, `probe-jobcontain-uncapped.py`, `probe-team-monitor-activity-fallback.py`, `probe-team-monitor-approval-title.py`, `probe-team-monitor-homings.py`. The 10 `probe-goal-watcher-*` / `probe-dead-room-sensor-session` / `probe-headless-retention-unknown` scripts were DELETED 2026-08-21 with the program they drove (`goal-watcher-job.py`). `probe-team-monitor-homings.py` landed with task 7.35 (the /tmp uid-quota canary and the ignorance escalation's bus half, re-homed into the surviving sensor); `probe-jobcontain-degrade.py` with 7.715 (`jobcontain.py`'s POSIX containment is real on Linux and degrades LOUD, never silent, where `fcntl`/`resource` are absent). |
...
- **`bindings`** (`ignite/capabilities/bindings/`) — the CASTING SHEET a workflow is run through
  ...
  (`.rbtv/config/modules/<module>/<component>/…` — module first, then the component that owns the value).
