...
**Install status: INSTALLED (as of 2026-06-10 — owner-approved at p6-checkpoint).** The studio module is in the install set. Four surfaces are live in `.claude/`: the `rbtv-designing` skill (Vivian, the Designer) and `rbtv-playwright-cli` skill (browser automation), and the `/rbtv-design-extractor` and `/rbtv-vision-to-json` commands. The Strategist persona, the studio loop beats, the artifact forks (`forks/site.md`, `forks/app.md`), and the critic remain loop-internal BY DESIGN — they are reached via `/rbtv-pitcher` → the studio loop, not as standalone commands.
...
- **Entry**: `/rbtv-pitcher` (office) — retargeted to this loop at P4; the Strategist runs beat 1, the Designer (`rbtv-designing` — now installed) runs beats 2–4.
