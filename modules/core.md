...
| [reasoning](../rules/reasoning.md) | Mandates a `<counter>` block before any agreement or endorsement; enforces position stability under pressure; prevents sycophancy. Includes an *Execution Discipline* section (simplicity, surgical changes, goal-driven, read-before-write, verify-absence, budgets, checkpoints, fail-loud) covering all artifact work, not only code |
