...
- **What**: Moves or renames a file or folder in one git-aware operation that also finds and fixes every reference to it — markdown links, wikilinks, frontmatter values, config paths, code imports (matched structurally with `ast-grep`, never by raw text), inline-code paths, and any literal occurrence of the exact old path (bare prose, command spans, config headers — always surfaced, never auto). Nested git repositories under the scope are skipped by default (pass `--include-nested-repos` to descend). Two stateless calls: `consult` (a dry run that finds, classifies, and risk-grades every reference and changes nothing) and `act` (performs the move and auto-applies the safe fixes, surfacing the risky ones for you). A per-reference hash handshake makes it drift-safe — editing a surfaced site between the two calls is safe; the tool refuses a changed site rather than clobbering it. The engine is a deterministic Python package (no LLM calls, no agent dispatch); the skill is a thin loader pointing to the usage guide and the package.
- **Also**: `--term "<term>"` adds a scan for references written as a registry/ontology record's human-readable TERM — relation edges (`- { verb: composed-of, to: goal contract }`) and prose — which carry no path or filename and are therefore invisible to every path matcher. The structured edge form can be `auto`; prose is always surfaced. With no `--term`, the moved record's own `term:` frontmatter value is used; `--new-term` supplies the rename target, and omitting it (a retirement) surfaces every hit with no rewrite.
- **When to use**: Any time you or the user need to relocate or rename a file or directory and keep its references intact — instead of moving by hand and grepping for callers. Folder moves are the highest-value case. Also when retiring or renaming a registry record whose references are written as its term.
- **How to invoke**: The skill fires when a move/rename with reference-fixing is needed; invocable directly as `rbtv-safe-move`. The tool runs in place from `{rbtv_path}/core/tools/safe-move` (`python -m safe_move consult|act|show …`) — it is NOT installed into `.claude/`; it is read from the RBTV source like a workflow.
- **What it produces**: From `consult`, a record per reference (id, file, line, match, proposed replacement, risk class — `auto`/`surface`/`protected` — and a hash) plus warnings and folder-cascade info — written in full to a report file under `{workspace_root}/.rbtv/runtime/safe-move/` (self-gitignored), with a compact stdout summary: counts, the surfaced decision set as ready-to-paste `id:hash` pairs, and the report path. From `act`, the performed move (staged, not committed), a compact action log (auto-fixed count; surfaced/drifted rows inline), and its own report file. `show <report>` slices full records back out by id/class/file-glob; `--json` restores the raw envelope on stdout. The caller commits.
