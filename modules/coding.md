...
Both rules load always-on once this module is installed (`rbtv-done-gate` thin trigger ≈404 words, `rbtv-non-technical-user` 719 — ≈1,123 always-on words). `rbtv-done-gate`'s heavy body lives in a protocol (`coding/skills/done-gate/done-gate-protocol.md`, ≈2,028 words) loaded **only when a coding task fires**, via the `rbtv-done-gate` skill loader — so non-coding sessions stop paying the body's ≈1,900 words while coding sessions keep full enforcement (the rule's Load Gate makes the load mandatory and the exempt decision stays in the always-on rule). The module is still a second scoping boundary: a workspace that does no coding **omits the `coding` module at install** and avoids even the ≈1,123-word always-on load. This is an install-time election the existing module-granular manifest already supports — not a runtime guess, so no protection is ever silent when a coding task fires. Workspaces that DO code keep both rules always-on.
