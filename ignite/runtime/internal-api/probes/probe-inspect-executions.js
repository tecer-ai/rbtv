...
const { TARGETS: CLI_TARGETS, HELP: CLI_HELP, run: cliInspect } = require('../../../ignite-cli/commands/inspect');
...
async function main() {
  ...
  // ── 7. TARGET-SET LOCKSTEP — the FOUR copies (the help was added by G-159). ─────────────────
  {
    ...
    const unexported = [
      gwT ? null : 'runtime/gateway/parse.js INSPECT_TARGETS',
      coreT ? null : 'dispatch.js INSPECT_TARGETS',
      cliT ? null : 'ignite-cli/commands/inspect.js TARGETS',
    ].filter(Boolean);
    ...
    const failures = unexported.length ? [] : checkClosedSetsLockstep({
      sets: [
        { name: 'runtime/gateway/parse.js INSPECT_TARGETS (front-door allowlist)', keys: [...gwT] },
        { name: 'dispatch.js INSPECT_TARGETS (server-core gate)', keys: [...coreT] },
        { name: 'ignite-cli/commands/inspect.js TARGETS (the CLI surface)', keys: [...cliT] },
        ...(helpUsable ? [{ name: 'ignite-cli/commands/inspect.js HELP (the prose an operator reads)', keys: [...helpT] }] : []),
      ],
      describe: {
    ...
    if (ddlStatuses) {
      ...
      const failures = (!gwS || !coreS) ? [] : checkClosedSetsLockstep({
        sets: [
          ...
          { name: 'runtime/gateway/parse.js EXEC_STATUSES (front-door allowlist)', keys: [...gwS] },
  ...
  // captures the payload it builds.
  {
    ...
    check("the CLI HELP documents the executions target (a surface nobody can discover is not shipped)",
      /inspect executions/.test(require('../../../ignite-cli/commands/inspect').HELP),
