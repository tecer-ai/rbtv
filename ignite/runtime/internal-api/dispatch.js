...
const { jobFireability } = require('../../state-store/heart/heart-store');
...
const { recordBusAnswer } = require('../../chat/bus-answer');
...
// and `supervisor/seeding.js#recordView` holds the seat's dependents while it is true. No stored
...
// exactly one writer, `coord.py`, and `chat/bus-answer.js` shells to it. This handler validates,
...
// and `state-store/heart/ask-record.js` performs the store write.
//
...
// (`runtime/gateway/parse.js#BUS_NAME_RE`) — DEC-3: gateway origin is not trust, and these two tokens
...
// (`runtime/gateway/parse.js#START_EXECUTION_COMMIT_RE`) — DEC-3, and [T5-R5]: a ref name would be a
...
const NOT_WIRE_REACHABLE = new Map([
  ...
  ['E_SPEC_HALVES_UNSUPPORTED', 'ticker-spawn-path only (the G-144 profile-shape guard, raised by spawn AND spawnSeat immediately after the profile lookup); spawn is never wire-triggered — spawn-via-named-profile is D70-dropped on entry, dispatch never calls spawnManager.spawn/spawnSeat, and the only route to spawnSeat is runtime/index.js spawnManagerWithPty.spawn, which is the ticker dispatch phase'],
  ...
  // called BOTH by resolveProfile and by `supervisor/spawn/spawn.js#composeArgv`, so the daemon raises
  ...
  ['E_NO_PORTABLE_HALF', 'raised only inside resolveProfile (supervisor/launch-profiles/profiles.js), which has NO daemon caller today — the spawn path resolves exec: directly (G-144). Re-rule at 7.43/7.54'],
  ...
  ['E_UNKNOWN_EFFORT', 'raised by resolveEffort (supervisor/launch-profiles/profiles.js) — the effort LADDER, shared by resolveProfile AND by the daemon spawn path since 2026-08-11 (d-0811lp-effort-lane-build-now). A request naming a rung outside the target profile\'s 1..N range, or a non-integer rung, reaches this code live; config-LOAD ladder validation still raises E_CONFIG_LOAD instead'],
  ['E_PINNED_FLAG_ABSENT', 'raised only inside preflightPinnedFlags (supervisor/launch-profiles/preflight.js), exported but deliberately NOT wired into the daemon spawn path (barrel comment, index.js:46). Re-rule when pre-flight is wired'],
  ['E_PREFLIGHT_UNAVAILABLE', 'raised only inside the pre-flight help reader (supervisor/launch-profiles/preflight.js), same deliberately-unwired module; a --help that cannot be read is a pre-flight outcome, not a request outcome'],
]);
...
function createInternalApi({ heartStore, spawnManager, secret, logger = null, authzPolicy = null, daemonStartTime = null, daemonLoadedCode = null, daemonConfig = null, readConfiguredTickIntervalMs = null, onEnqueue = null, liveSessions = null, workspaceRoot = null }) {
  ...
  // traversal and a directory read. `chat/bus-answer.js` checks them a THIRD time against the
  ...
  // between a traversal and a directory write. `state-store/heart/ask-record.js` checks them a THIRD
  ...
  // `state-store/heart/start-execution.js` checks the names a THIRD time against the module that owns
