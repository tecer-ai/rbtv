...
//
// ⚠ THE FACTS ARE HANDED IN BY THE PASS THAT ALREADY COMPUTES THEM. `supervisor/lane-watch.js` runs
...
// (`chat/system-digest.js` § Open conditions) from the process that does hold a token.
...
const { createOutbox, outboxStorePath } = require('../chat/outbox');
const { loadRecoveryConfig } = require('../supervisor/recovery-config');
const { restartSuppression } = require('../observation/restart-window');
