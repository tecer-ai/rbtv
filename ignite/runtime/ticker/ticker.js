...
const {
  ...
  generateSessionId,
  toolExecEnv,
} = require('../../supervisor/spawn/carrier');
const { exitFilePath, ensureExitFile, closeSeatSessionRow } = require('../../supervisor/spawn/spawn');
...
const { TERMINAL_TURN_STATUSES, sessionStatusForEndedTurn, seatKeyOf } = require('../../state-store/heart/heart-store');
...
const { expandArgv, checkFireToolWorkdir } = require('../../state-store/heart/argv-template');
...
const { resolveWorkspaceRoot } = require('../../supervisor/spawn/config');
...
// the liveness EVIDENCE in `runtime/lease/lease.js` (7.607 E1; the register read that used to sit in
...
function createTicker({ heartStore, spawnManager, config = {}, logger = null, feedPath = null, logPath = null }) {
  ...
  // (`supervisor/seeding.js#seatBootPrompt` → `coordinate boot-prompt --lane <marker>`), so the sitting
  ...
  function composeSeatBootPrompt(seatDir) {
    ...
    let lane = 'console';
    try { lane = require('../../supervisor/lane-watch').readLane(home.goalDir).lane; } catch { lane = 'console'; }
    if (lane !== 'daemon') return { prompt: null, reason: 'not-daemon-lane' };
    let boot;
    try {
      boot = require('../../supervisor/seeding').seatBootPrompt(home.goalDir, home.seat);
    } catch (err) {
  ...
  async function launchFireTool(queueRow, actions, tick, now) {
    ...
    // A FOLDER THAT REDIRECTS ITS CREDENTIALS — `ignite-cli/lib/config.js` resolveToken walks UP from cwd
  ...
  // DAEMON lane (`supervisor/lane-watch.js`, on the pass that first adopts a goal) passes `{ goal }`.
  ...
  async function enforce(now, tick, actions) {
    ...
    // probe, and the "boot rescan" comments elsewhere are stale (`runtime/index.js` boots retention
  ...
  // `supervisor/lane-watch.js` reaches it as `engine.ticker.ensureGoalChannel`, which is why the
  // engine already publishes `ticker` on its surface (`runtime/engine.js`). Exposing it beats the
  // alternative shapes: re-implementing the spawn in the engine would be a second performer of an
  // act that must stay singular, and threading a callback down from `runtime/index.js` would put
