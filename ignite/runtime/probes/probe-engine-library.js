...
const COMMITTED_CONFIG = path.join(IGNITE_SRC, 'envelope', 'spawn-profiles.yaml');
...
// reason the seat folders themselves once did (see C5's note below). `envelope/spawn-profiles.yaml`
...
const { parseSeatPath, materializeSeatFolder } = require(path.join(IGNITE_SRC, 'runtime', 'seat-identity', 'seat-folder.js'));
...
async function main() {
  ...
  const graph = JSON.parse(execFileSync(process.execPath, ['-e', `
    require(${JSON.stringify(path.join(IGNITE_SRC, 'runtime', 'engine'))});
  ...
  // — not `runtime/index.js`, which is an ENTRY POINT: requiring it calls main() and tries to boot
  ...
  const controlGraph = JSON.parse(execFileSync(process.execPath, ['-e', `
    require(${JSON.stringify(path.join(IGNITE_SRC, 'runtime', 'gateway', 'gateway.js'))});
    const loaded = Object.keys(require.cache).map((p) => p.replace(${JSON.stringify(IGNITE_SRC + path.sep)}, ''));
    const builtins = (process.moduleLoadList || []).filter((m) => m.startsWith('NativeModule'));
    console.log(JSON.stringify({ loaded, builtins }));
  `], { encoding: 'utf8' }));
  ...
  check('C1 the daemon consumes the engine library (asserted at its source)',
    /require\(['"]\.\.\/engine['"]\)/.test(fs.readFileSync(path.join(IGNITE_SRC, 'runtime', 'index.js'), 'utf8')),
    "server/index.js requires '../engine'");

  const { createEngine } = require(path.join(IGNITE_SRC, 'runtime', 'engine'));
  const { loadConfig } = require(path.join(IGNITE_SRC, 'supervisor', 'spawn', 'config'));
  ...
  const attached = require(path.join(IGNITE_SRC, 'operator', 'attached-execution.js'));
  ...
  const substrate = require(path.join(IGNITE_SRC, 'runtime', 'substrate.js'));
  ...
  const { openHeartStore } = require(path.join(IGNITE_SRC, 'state-store', 'heart', 'heart-store'));
  ...
  //
  // So the wave decision is now measured WHERE IT IS MADE: `launchOwed` returns the seats it
  ...
  // ⚠ THE FRONTIER IS COORD'S SINCE § D1 (`one-readiness-predicate.md`): `launchOwed` no
  ...
  const wave1 = attached.launchOwed(waveStore, waveRows, { profile: 'probe-seat', goalFolder: waveDir, ...frontier() });
  ...
  const wave1Again = attached.launchOwed(waveStore, waveRows, { profile: 'probe-seat', goalFolder: waveDir, ...frontier() });
  ...
  const wave2Blocked = attached.launchOwed(waveStore, waveRows, { profile: 'probe-seat', goalFolder: waveDir, ...frontier() });
  ...
  const heldWithoutCheckout = attached.launchOwed(waveStore, waveRows, { profile: 'probe-seat', goalFolder: waveDir, ...frontier() });
  ...
  const wave2 = attached.launchOwed(waveStore, waveRows, { profile: 'probe-seat', goalFolder: waveDir, ...frontier() });
  ...
  const dupes = attached.launchOwed(waveStore, waveRows, { profile: 'probe-seat', goalFolder: waveDir, ...frontier() });
  ...
  check('C5 the rbtv delegate is present AND executable (a permission bit is invisible to a content check)',
    (() => { try { fs.accessSync(path.join(IGNITE_SRC, 'operator', 'attached-execution', 'tool', 'rbtv-execution'), fs.constants.X_OK); return true; } catch { return false; } })(),
  ...
  for (let i = 0; i < 60 && !(firedBeforeKill > 0 && tickBeforeKill >= 1); i += 1) {
    ...
    if (fs.existsSync(killStore)) {
      const peek = spawnSync(process.execPath, ['-e', `
        const { openHeartStore } = require(${JSON.stringify(path.join(IGNITE_SRC, 'state-store', 'heart', 'heart-store'))});
