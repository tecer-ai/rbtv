...
// not in D1–D61): `runtime/internal-api/dispatch.js`'s handler always throws INTERNAL, never
...
// it to `supervisor/seeding.js#recordView` as the set whose dependents wait. This row is what reaps
...
// beat: `enqueue-job` carrying a `fire-tool` job needs a `envelope/spawn-profiles.yaml` entry plus a
...
// W4 closed it at seven; D2's routed types make it EIGHT. See heart-store.js MESSAGE_TYPES for
// the whole site list. `escalation` MUST be here: a leader's escalation crosses this door, and a
// door refusing it is the exact silent class W4 exists to close. `stuck` MUST be here for the
// same reason — `coord.py`'s `gateway_send_leg` rules it a CROSSING type (it is a health signal
// the daemon plane carries), so this door sees it on every emission.
...
// `chat/bus-answer.js`. What IS decidable from the payload alone is the SHAPE of the two names,
...
// (`chat/probes/probe-chat-boundary.js`), so it can neither write the table nor keep its
...
// (DEC-3) — see `state-store/heart/ask-record.js`. What IS decidable from the payload alone is the
...
// (`chat/probes/probe-chat-boundary.js`), so `approve` reached a `materialize` port
...
// `state-store/heart/start-execution.js`. What IS decidable from the payload alone is the SHAPE of the
