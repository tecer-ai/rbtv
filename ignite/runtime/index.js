...
const { closeHeartStore, isHeartStoreOpen } = require('../state-store/heart/heart-store');
const { WARNING_KINDS } = require('../state-store/heart/warnings');
...
const { runLaneWatch } = require('../supervisor/lane-watch');
...
const { validateCataloguePaths } = require('../state-store/heart/catalogue-paths');
...
// tree; RBTV_IGNITE_SRC names the live per-invocation tree (coord/, coord.py).
...
function resolveConfigPath(igniteSrc) {
  return process.env.RBTV_IGNITE_CONFIG_PATH || path.join(igniteSrc, 'envelope', 'spawn-profiles.yaml');
}
...
async function main() {
  ...
  if (mergedConfig.default_workdir_root === WORKDIR_ROOT_PLACEHOLDER) {
    throw new Error(
      ...
      `EnvironmentFile (never by editing envelope/spawn-profiles.yaml in the repo — that ` +
  ...
  // (`supervisor/execution-record.js`, owner ruling decisions.md#d-s23-single-execution-record-now).
  ...
  //   and `observation/daemon-watchdog/tool/watchdog-alarm.js` read: one channel, three processes.
