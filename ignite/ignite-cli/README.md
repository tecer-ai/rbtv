...
| `.rbtv/modules/ignite/server.json` | The workspace's committed endpoint record (D27 install model), **found by walking UP from the cwd to the nearest ancestor that holds it** — so the CLI works from any folder inside the workspace, including a daemon-spawned seat's own seat folder. It is a **machine-keyed map**: each machine's install lives under `machines[<hostname>]` (endpoint fields + that machine's `state_root`). Used when `IGNITE_GATEWAY_ADDR` is unset: this machine's own entry wins when it records a server, else the one entry that does; tailnet address preferred; the SSH-tunnel fallback is printed as a command to run yourself (this CLI never opens a tunnel). The legacy flat shape is still accepted. |
...
| `ignite inspect asks` | `inspect` (`target: asks`) | Read-only. Every OPEN, POSTED owner ask across EVERY goal, oldest first — the fleet set the 2-hourly system digest renders (`spec-owner-io` §5). A fixed view like `jobs`/`queue`: no id, no status, no paging, because the digest renders the whole waiting set or it is not that digest. Each row carries the ask id (the Slack thread [T5-R7]), its goal and seat, the one-liner read back from the ask's on-disk copy, and `opened_at` (when the ask reached the owner). The READ half of the `record-owner-ask` intent's write. |
...
## Router skills (`spec-component-map.md` §7.2 / §7.4)

Five FUNCTION-bundle router skills live beside this CLI, because the front door owns
routing. Each is a decision table ("need X → run Y") over tools that live in OTHER
components; none of them restates a tool's own `-h`, and none of them is a wrapper script.

| Bundle | File | Routes to |
|---|---|---|
| `daemon-ops` | `daemon-ops.md` | `rbtv ignite daemon`, `rbtv ignite ticker`, `probe-suite`, `link-tools` |
| `goal-ops` | `goal-ops.md` | `rbtv goal`, `rbtv-goal-request`, `materialize-seats`/`scaffold-seats`, `worktree-flow`, `rbtv run` |
| `observe` | `observe.md` | `owed-answers`, `tmux-overview`, `overview-compact`, `provider-usage`, the `rbtv ignite` job/queue/session verbs |
| `staffing` | `staffing.md` | `rbtv teambuild`, `rbtv-bindings`, `rbtv-master-profile`, `rbtv-seat-identity` |
| `coord-ops` | `coord-ops.md` | `coordinate`, `file-issue`, `floor-lint` |

Which roles load which bundles is `cli-role-bundles.md` (§7.4); who each underlying tool is
FOR is `cli-audience-map.md` (§7.1). The `SKILL.md` loader for each bundle is generated by
`install.py` from this component's `exposure.csv` `method=skill` rows — never hand-written.
