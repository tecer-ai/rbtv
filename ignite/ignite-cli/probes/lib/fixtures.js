...
// row) here via `state-store/heart/heart-store` is TEST-FIXTURE SETUP, not the
...
const SERVER_ENTRY = path.join(IGNITE_SRC, 'runtime', 'index.js');
const CLI_ENTRY = path.join(IGNITE_SRC, 'ignite-cli', 'ignite.js');

const { hashToken } = require(path.join(IGNITE_SRC, 'runtime', 'gateway', 'sender-auth'));
