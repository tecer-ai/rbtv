...
const body = [
  'probe-reconcile — runs engine/reconcile.selftest.js (derivation, the D33(a) word split, the D34 no-progress counter, the D44 stuck-brake, D35 mail, durability, pause gate, red arms)',
  `exit: ${r.status}`,
  r.stdout || '',
  r.stderr ? `stderr:\n${r.stderr}` : '',
].join('\n');
