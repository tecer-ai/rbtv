...
The rule does not yet reach a CAGED seat at run time. `materialize-seats.py` generates each seat's `CLAUDE.md`/`AGENTS.md` guidance pair, including an "Always-on rules — binding for this whole sitting" block, and that generated text is what a caged seat is guaranteed to read without an extra step. CORRECTED: every compiled cage read-only-mounts the whole vault (cage family 5, `vault-wide-read`) and the whole rbtv repo (cage family 6, `rbtv-repo`), ungated by seat declarations, so `meta/planning/references/headless-seat-cannot-wait.md` was always reachable from inside a cage — only credential-pattern paths (`**/*.env`, `**/*token*`, `**/*.key`, `**/credentials/`) are masked. The real gap was discoverability — nothing pointed a seat at the file — closed by commit `c48d2fa1` (seat `seat-guidance-wait-rule`). Until that commit, the rule reached produced seats only through the drafter that authors their bodies.
...
- CORRECTED: `meta/planning/references/headless-seat-cannot-wait.md` IS reachable from inside a cage — every compiled cage read-only-mounts the whole vault (cage family 5, `vault-wide-read`) and the whole rbtv repo (cage family 6, `rbtv-repo`), ungated by seat declarations; only credential-pattern paths are masked. The real gap was discoverability, not reachability, and it was closed by commit `c48d2fa1` (seat `seat-guidance-wait-rule`). A seat still only learns the rule if something points it there — do not assume a produced seat has read it on its own.
...
CORRECTED 2026-09-02 by owner grant (closeout of plan redesign-continue-1; see planning/20260831-i-wait-trap-rule-reached-no-alre.md and decisions.md#d-ask16-wait-rule-reaffirmed).
