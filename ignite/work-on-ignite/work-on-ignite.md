...
ignite/work-on-ignite/
  work-on-ignite.md          ← this file (the ONE reference)
  CLAUDE.md                  ← 3-line pointer
  AGENTS.md                  ← same 3 lines
  templates/issue.md
  templates/creation.md
  skill/                     ← the build-ignite skill (read before, file after)
  memory/
    <component>/
...
The filing command writes one title line `# <stem> — <title>`, then the body sections below (it copies `templates/issue.md` / `templates/creation.md`).
...
The `skill/` procedure is the when; this file is the what.
...
Who writes: the ignite-engine goal at build/closure, **and** any ignite-editing session (owner at a terminal, console agents) at its close. The command guarantees shape; `skill/` says when.
