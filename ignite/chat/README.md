...
the answer lands, and the ✅ ack it stamps when an owner reply releases an ask
(chat-bridge.js § pending marker / § landed-answer ack). Without it every reaction call fails,
...
`<channel>:<threadTs>` → `{ goalId, seat, askId, label, kind, paused, released }`, persisted
additively in the state file (`STATE_VERSION` unchanged). The ask's own **state** is `open_asks`,
daemon-side, and a second copy of it in this process would be a second source of one fact; what the
map carries are the bridge's facts about the **thread** — which dispatch a parsed token takes
(`kind`), whether an approval thread is reject-and-paused (`paused`), and whether this bridge has
already released this thread's ask (`released`).
...
| **Inbound** | `onChatMessage` looks the thread up in `askThreads` **before every other leg**, keyed off the raw event (`_channel`/`_threadTs`) because for goal traffic the routed conversation id IS the channel and cannot tell one ask thread from another. A hit is handled at the release door and **does not fall through** — a fall-through would mint a sitting on an unauthorized remark, answer an authorized one twice, and (on a thread already released) buy a goal-master sitting for the owner's re-send. |

The map entry is never dropped by an outcome. Wrong thread, unauthorized sender, unparsed token
and a mechanical verb all leave the ask `open` and the entry untouched — an ask still open whose
thread the bridge has forgotten is an ask that can never be answered.

**An actual release MARKS the entry `released: true`** (with `releasedAt` and the recorded
`outcome`) rather than deleting it [G-second-brain-43-0828-2119, owner-ordered 2026-08-30]. The
deletion it replaces is what made the owner's next message in an answered thread unrecognizable:
it fell through to the goal-channel forward path and summoned a goal-master sitting that read a
bare `a` as a check-in — three times on 2026-08-28, one of them joining a planning goal's roster
as a paneless owner door. A later authorized, parsed reply in a released thread is now refused
**in that thread** (`already answered`, naming the outcome recorded), with `reap: false` so nothing
is reaped twice, before the `kind` fork so no approval can dispatch D12 twice, and with no forward.

*Bound on the map's growth* — a count, never a timer: at most `RELEASED_KEEP` (200) released
entries are kept, and marking a new one evicts the oldest by `releasedAt`. There is no reaper and
no clock to be wrong about; eviction happens on the only event that can grow the set, and the whole
cost of an eviction is that a reply to a months-old answered thread falls through as it did before.
A state-file entry written before `released` existed carries none of these keys, loads as
`released: false`, and behaves exactly as it always did — **no migration is needed or performed**.

*The ✅ landed-answer ack* — a reply that actually releases an ask (`released === true`) gets
`white_check_mark` stamped on the owner's own message, best-effort on the same serialized reaction
chain as ⏳. It is deliberately a different marker: ⏳ means "accepted, an agent will answer you"
and is removed when that answer lands; ✅ means "your answer landed" and is never removed. Every
§2.4 refusal gets no ack — each already has its own visible outcome, or is silent by ruling.
...
letting it *acquire* an arm would be a behaviour bought with a defect. `supervisor/lane-watch.js` warns
...
again**. `supervisor/execution-record.js#blockAndQueueVerdict`, `#askParkedAtGate` and `#outcomeForSeat`
are **deleted**.
...
The engine's role is CONSUMPTION — `supervisor/seeding.js#recordView` folds the DERIVED wait (§2.1: an
...
`chat/probes/probe-owner-ask-hold.js` arms U1/U2 and recorded there as a FINDING. Whether the
...
Measured by `chat/probes/probe-owner-ask-hold.js` (arms H\*/P\*, plus the § F control/treatment
...
Producers today: `operator/master-profile` emits `[deliver: post]` on its settled-outcome row.
(the deleted `capabilities/goal-launch-delay` was the second producer until task 7.778 deleted that capability
...
| bound | why |
|---|---|
| the **bridge never spawns** | `chat-bridge-spec.md` lines 14/26 and `probe-chat-boundary`. The bridge asks; `chat/bus-answer.js` acts. Same split `live-sessions.js:10-15` describes for the warm-session manager |
...
⚑ **The manager is DAEMON-SIDE** (`supervisor/spawn/live-sessions.js`), and this subtree could not
...
| `to: owner`, **gates open** | **the sending agent's own thread in the goal channel**, its header marked with the sender's fallback arm when it declared one (§ *The seat's fallback arm*). (§ *Thread per agent*). A goal with **no channel** no longer falls back anywhere: a map miss re-asks Slack, and if the channel genuinely does not exist (or Slack simply did not answer) the row is **held and retried**, with one system-channel alarm naming the goal and the seat at the third failed pass — never the owner DM, which carries no goal traffic (`d-escalation-surface` parts 4, 6). An **escalation** blocked this way is retried **without a cap**, never abandoned. |
...
kill clock open forever. **Never the owner DM**, which carries no goal traffic at all
(`d-escalation-surface` part 4 — the goal channel and, for daemon-level faults, the system
channel are the surfaces now; the DM is only for conversations the owner starts): the
unreachable-channel alarm is deliberately not fired for this row either — a finished goal is not
a blocked escalation. No `#system-channel` post.
...
| `forward-path.js` | the D104/D105 forward contract (session-create / follow-up / reply type), plus the ONE `record-bus-answer` gateway call that puts the owner's reply back on the asking seat's bus (§ The answer goes back onto the bus). The ACT is daemon-side (`chat/bus-answer.js`) — this subtree may hold no spawn |
...
| `live-sessions.js` | the WARM leg's caller: decides whether a turn is a warm candidate, makes ONE `live-feed` gateway call, hands the reply back. The manager itself is daemon-side (`supervisor/spawn/live-sessions.js`) — see § The warm path |
...
Relocatable subtree (root CLAUDE.md § "ignite/ — Runnable Service Code"): the runtime source imports NO sibling
module (`runtime/`, `runtime/gateway/`, `ignite-cli/`) — it reaches the daemon only over the gateway
...
chat/probes` (or `--only probe-chat-<name>` for one). Never invoke a probe file
directly: that rewrites its capture with pure noise and the run is neither counted nor
graded for staleness (`ignite/deploy/component.md` § Probes). Evidence → `probe-chat-<name>.out`.
...
(`state-store/heart/start-execution.js`) runs the supervised Path-B birth
...
`runtime/internal-api/probes/probe-start-execution.js`.
