...
//
// The manager is daemon-side (`supervisor/spawn/live-sessions.js`); this probe lives here because the
...
const REAL_PROFILES = path.join(IGNITE, 'envelope', 'spawn-profiles.yaml');
const LIVE_MODULE = path.join(IGNITE, 'supervisor', 'spawn', 'live-sessions.js');
  ...
  try {
    ...
    // ── ARM 7 — MUTATION: cut the feed, arm 2 must go RED ──────────────────────────────────────
    {
      ...
      if (!src.includes(target)) {
        ...
        const beside = path.join(IGNITE, 'supervisor', 'spawn', `live-sessions.MUTANT-${process.pid}.js`);
