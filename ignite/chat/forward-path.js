...
function createForwardPath({ forwarder, threadMap, allowlist, config, logger = null, deliver = null, listAgentThreads = null, askStore = null }) {
  ...
  // `supervisor/seeding.js#recordView` treats that set as the seats whose dependents wait. This row is
  ...
  // a JavaScript writer. So the bridge asks and the DAEMON acts — `chat/bus-answer.js` shells to
  ...
  // at `spawn()`, and the WARM leg (`supervisor/spawn/live-sessions.js`) never went through `spawn()`.
