...
// "no queue handle, no spawn path"; enforced by `chat/probes/probe-chat-boundary.js`, which
...
// `chat/live-sessions.js:10-15` already describes: the warm-session manager hit this exact
// wall, was moved server-side, and the bridge kept only a caller. Routing the spawn through a
// helper in `chat/lib/` would pass the probe's regex while handing the bridge process a
...
// the address of the one writer — both exactly as `supervisor/seeding.js` names them, and for its
...
const { openOwnerAsks } = require('../supervisor/execution-record');
...
const { isSafeName } = require('./bus-ferry');
