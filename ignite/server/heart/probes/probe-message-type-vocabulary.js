...
const SITES = [
  ['server/heart/heart-store.js', /const MESSAGE_TYPES = new Set\(\[([^\]]*)\]\)/],
  ['server/internal-api/dispatch.js', /const MESSAGE_TYPES = new Set\(\[([^\]]*)\]\)/],
  ['gateway/parse.js', /const MESSAGE_TYPES = new Set\(\[([^\]]*)\]\)/],
  ['bridges/chat/forward-path.js', /const CMP8_TYPES = new Set\(\[([^\]]*)\]\)/],
  ['server/heart/schema.sql', /CHECK \(type IN \(([^)]*)\)\)/],
