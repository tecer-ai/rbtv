...
const checks = [];
let skipped = 0;   // no arm raises it since §6 retired; the counter stays because the capture prints it
...
async function main() {
  ...
  // --- 6. RETIRED. Task 7.736 DOOR 1 (`edge_fastpath` on a start-workflow dry-run) is DELETED
  // with the mechanism it reported on (`one-readiness-predicate.md` § Deletions): there is no arm
  // file, no edge-runner, and readiness is `coordinate ready-seats`. Its three arms are gone rather
  // than inverted — a probe asserting the ABSENCE of a key nobody writes measures nothing.
  //
