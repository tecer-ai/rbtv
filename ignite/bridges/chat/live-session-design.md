...
| Capability | Verdict | Evidence |
|---|---|---|
| (1) stream-json stdin, mid-turn queueing | **FAIL — no mechanism** | `where opencode` finds no binary on this box (`7641-opencode-absence.txt`) — no `--help` to check directly. Historical (dated probe excerpt): the then-opencode package manual (`orchestration/models/opencode/manual.md`, `7641-opencode-manual-excerpt.txt`) said "`opencode run` executes one headless turn (or resumes a session)... There is no separate print/quiet flag — `run` IS the headless mode" (line 153). Live source is `cast` / opencode's own `--help`. The only structured-JSON option documented then was `--format json` for OUTPUT events (line 179) — no `--input-format`/stream-json INPUT mechanism appeared in that excerpt. |
