...
`runtime/jobs/edge-runner-job.py#check_deriver_mirrors_the_composer` — and it was DELETED with its file.
...
the Python deriver.** So the arms read the LIVE template — `envelope/spawn-profiles.yaml`'s
...
# cagespec.py and the template it mirrors are both `ignite/envelope/` since the
# component-first move — this probe travelled with its product file.
ENVELOPE = HERE.parent
OUT = HERE / "probe-cagespec-mirror.out"
PROFILES = ENVELOPE / "spawn-profiles.yaml"

sys.path.insert(0, str(ENVELOPE))
...
def main():
    ...
    import subprocess
    r = subprocess.run([sys.executable, str(ENVELOPE / "cagespec.py")],
