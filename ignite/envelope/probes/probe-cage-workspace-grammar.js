...
const { surfaceCageRefusal } = require('../../supervisor/seeding');
const { loadConfig } = require('../../supervisor/spawn/config');
const { resolveRwPathGrants } = require('../../supervisor/spawn/seat-grants');
const { contains } = require('../../supervisor/spawn/cage');
...
function liveSeatBinds() {
  const config = loadConfig(path.join(HERE, '..', '..', 'envelope', 'spawn-profiles.yaml'));
