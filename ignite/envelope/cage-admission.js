...
const { composeSeatCage, specToBwrapFlags, contains, RW_VERBS } = require('../supervisor/spawn/cage');
...
} = require('../supervisor/spawn/seat-grants');
...
function admitDeclaredOutputs({ seatBinds, goalFolder, seat, successorReads = 'no', workspaceRoot = null }) {
  ...
  try {
    ...
    return `undecided: the live cage template does not compose for this seat (${err.message}). An underivable `
      + 'wall NEVER defaults to admissible — teach `supervisor/spawn/cage.js` the shape, never admit past it.';
  }
  ...
  return 'DECLARED-OUTPUT TOKEN NOT ADMISSIBLE FOR A CAGED LAUNCH: ' + bad.join('; ')
    ...
    + '`supervisor/spawn/cage.js#composeSeatCage` over the profile-resolved `cage.SeatBinds` the daemon itself launches '
  ...
  for (const { verb, arg } of entries) {
    ...
    if (produced === null) {
      try {
        ...
        return `undecided: the live cage template does not compose for this seat (${err.message}). An underivable `
          + 'wall NEVER defaults to reachable — teach `supervisor/spawn/cage.js` the shape, never admit past it.';
      }
    }
