...
const SERVER_ENTRY = path.join(IGNITE_ROOT, 'runtime', 'index.js');
const TICKER_TOOL = path.join(IGNITE_ROOT, 'operator', 'ticker-settings', 'tool', 'rbtv-ignite-ticker');
const DAEMON_TOOL = path.join(IGNITE_ROOT, 'operator', 'daemon-operator', 'tool', 'rbtv-ignite-daemon');
...
function main() {
  ...
  const port = freePort();
  const cfg = fs.readFileSync(path.join(IGNITE_ROOT, 'envelope', 'spawn-profiles.yaml'), 'utf8')
