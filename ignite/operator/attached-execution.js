...
const substrate = require('../runtime/substrate');
const { loadConfig } = require('../supervisor/spawn/config');
...
const {
  ...
  uncastSeats,
} = require('../supervisor/seeding');
const { createBoard, snapshotFrom } = require('../runtime/run-board');
...
const { openExecution, closeExecution, laneOf } = require('../supervisor/execution-record');
...
// The live board (`runtime/run-board.js`) is the human surface for this lane: three columns
...
//     from `chat/bus-ferry.js` — the SAME readers the status verb and the message gate use.
...
function appendForegroundSessionRow({ goalFolder, seat, sessionId, harness, model, workdir, logger = null }) {
  ...
  try {
    // The lazy require is the shape this file already uses for every `supervisor/spawn/` reach
    // (`composeArgv`, `generateSessionId` below): the daemon subtree is relocatable, so nothing
    // here holds a LOAD-time dependency on it.
    const { appendRowEnsuringHeader } = require('../supervisor/spawn/spawn');
...
function runForegroundSeat({
  ...
  const { launchSpecForSeat } = require('../supervisor/spawn/spawn');
  ...
  const { effortRungFor } = require('../supervisor/launch-profiles/catalog');
  const { seatDeclaresValue } = require('../supervisor/spawn/spawn');
  ...
  const { generateSessionId } = require('../supervisor/spawn/carrier');
  const { composeArgv } = require('../supervisor/spawn/spawn');
  ...
  const { harnessOf } = require('../supervisor/spawn/harness-config');
  const { bindingOf } = require('../supervisor/launch-profiles/catalog');
...
// daemon lane (`supervisor/reconcile.js`): a non-terminal ending with no later sitting is owed work.
//
...
function evaluateExit(heartStore, rows, view = null, ready = null, goalFolder = null) {
  ...
  // `supervisor/reconcile.js` on the daemon lane, with nothing to mint and nothing to lose.
...
// readers are the chat bridge's own (`chat/bus-ferry.js`) so the status surface and the
...
function statusAttached({ goalFolder: goalFolderInput, openStore = null }) {
  ...
  if (everRun) {
    const open = openStore || ((p) => require('../state-store/heart/heart-store').openHeartStore({ dbPath: p }));
