# attached-run — the `rbtv run` verb
...
|---|---|---|
| entry | `rbtv ignite daemon start` | `rbtv run <run-folder> --profile <name>` |
| store | `{state_root}/heart.db` | `<run-folder>/heart.db` |
...
`tool/rbtv-run` — reached as `rbtv run`. It owns argv, output and exit codes and NO advancement
...
The run folder's `taskforce.csv` is the workflow. Each row is a seat; the row's **`after` column is
...
State lives in `<run-folder>/heart.db`. Re-running the verb reopens it and continues: job
