...
4. Own ``write_if_changed`` — the idempotent, check-mode-aware text-write
   primitive every render leg (``library.py``, ``config_assets.py``) writes
   managed files through — and ``ALWAYS_EXCLUDED_PREFIXES``, the goal-folder
   protection ``uninstall`` consults. Both were absorbed from the retired
   ``mirror.py`` / ``guidance.py`` in the 2026-08-18 team-kit move.
...
#:
#: ``apply_deletions``'s caller (``driver.uninstall``) consults this set (row
#: 7.597): a workspace whose ``rbtv.json`` still records such a router from a
#: pre-retirement render must NOT have it deleted on teardown — the banner-guard
#: cannot spare it (the scaffold's own header carries the same DO-NOT-EDIT
#: sentinel), so uninstall drops it from the delete set and un-manages it
#: instead. Absorbed from the retired ``guidance.py`` (2026-08-18 team-kit move).
...
# ---------------------------------------------------------------------------
# Shared text-write primitive — absorbed from the retired mirror.py
# (2026-08-18 team-kit move). library.py and config_assets.py write every
# managed file through this for idempotent, check-mode-aware renders.
# ---------------------------------------------------------------------------
