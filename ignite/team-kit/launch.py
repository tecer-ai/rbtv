...
def unanswered_ask_block(pkg):
    ...
    A row counts as still open only when it is `posted` AND `state='open'` — the same two facts
    §2.1 derives a seat's wait from. An unposted row is an ask nobody has seen.
    ...
    """
    open_asks = []
    for row in ending_store.open_asks(pkg, seat="goal-master"):
