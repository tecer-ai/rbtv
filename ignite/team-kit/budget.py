...
#
# Owner ruling `r-watch-loop-30s` (2026-07-30) + `p-cadence-home-and-catalog-root` ASK 2: the watch
# loop's cadence is a POLICY NUMBER, so `r-bar-home-is-the-run-budget-json` binds it exactly as it
# binds a floor, and this is the same read-never-copied lane the floors proved.
#
# A THIRD KIND OF NUMBER, IN ITS OWN SECTION, DELIBERATELY. A cadence is not a cap (a count of
# panes) and not a floor (an MB threshold an act is refused or flagged at); it is a CEILING on an
# interval, in SECONDS. It gets its own field and its own reader rather than borrowing a floor's
# slot, because a number that borrows another kind's home inherits that kind's semantics.
CADENCE_FIELD = "watch_loop_max_seconds"


class CadenceUndeclared(Exception):
    """No cadence is declared for this package. The caller decides — and for a LOOPING caller the
    answer is 'refuse to start unless an explicit flag was passed', because a loop cannot choose
    its own interval without becoming that interval's second home."""


class CadenceUnreadable(Exception):
    """A budget.json IS declared here and could not be read, does not parse, or declares a cadence
    that is not a positive integer number of seconds. ALWAYS loud — the same distinction
    `FloorUnreadable` carries, and for the same measured reason: absence of a declaration and
    failure to read one look identical to a fail-soft caller."""


def read_cadence(run_root):
    """The run's declared watch cadence CEILING in seconds, from `{run_root}/budget.json`.

    ⚠ NO `default=` PARAMETER, for the reason `read_floor` states: a default is a literal, a
    literal is a home, and homes are what this lane exists to delete.
    """
    path = os.path.join(str(run_root), "budget.json")
    if not os.path.exists(path):
        raise CadenceUndeclared("no budget.json at %s — nothing declares a cadence for this package"
                                % path)
    b, err = _load(path, "budget.json")
    if err:
        raise CadenceUnreadable(err)
    cadence = b.get("cadence") or {}
    if CADENCE_FIELD not in cadence:
        raise CadenceUndeclared(
            "budget.json at %s declares no cadence.%s. The cadence's one home is this file "
            "(r-bar-home-is-the-run-budget-json, p-cadence-home-and-catalog-root ASK 2); a "
            "consumer must not invent one." % (path, CADENCE_FIELD))
    v = cadence[CADENCE_FIELD]
    if not isinstance(v, int) or isinstance(v, bool) or v <= 0:
        raise CadenceUnreadable(
            "budget.json at %s has cadence.%s = %r, which is not a positive integer number of "
            "seconds" % (path, CADENCE_FIELD, v))
    return v


def cadence_source(run_root, flag_value):
    """Resolve the cadence a loop will actually sleep, and the REASON, as (seconds, why).

    ⚠ THE `why` IS NOT DECORATION — it is what makes an override VISIBLE instead of environmental,
    the same acceptance `floor_source` carries. A consumer that resolves correctly and prints
    nothing fails it.

    Raises CadenceUnreadable always, and CadenceUndeclared only when there is no flag to fall back
    on: an unreadable or missing declaration is a hard start failure for a looping caller, and the
    ONLY thing that may proceed in its place is an EXPLICIT operator override, never a number this
    code chose.

    ⚠ AN OVERRIDE ABOVE THE DECLARED CEILING IS SAID OUT LOUD AS A BREACH, not merely reported as a
    disagreement. The cadence is a MAXIMUM (unlike a floor, where the flag simply wins), so a
    larger override is the one direction that violates the ruling rather than tightening it — and
    `r-watch-loop-30s`'s own interim overshoot is precisely that case, disclosed rather than
    normalized. It still proceeds: an operator override that could be silently clamped would be a
    second policy home wearing a clamp's clothing.
    """
    if flag_value is not None:
        try:
            declared = read_cadence(run_root)
        except CadenceUndeclared:
            return flag_value, ("--cadence-s %d (explicit override; no cadence.%s declared at %s)"
                                % (flag_value, CADENCE_FIELD, run_root))
        if flag_value > declared:
            return flag_value, (
                "--cadence-s %d (explicit override) — budget.json declares cadence.%s = %d and "
                "this OVERRIDE BREACHES THAT CEILING by %ds. The flag wins because an operator "
                "passed it. Remove the flag to follow the ruling."
                % (flag_value, CADENCE_FIELD, declared, flag_value - declared))
        if flag_value < declared:
            return flag_value, (
                "--cadence-s %d (explicit override, TIGHTER than the declared ceiling "
                "cadence.%s = %d)" % (flag_value, CADENCE_FIELD, declared))
        return flag_value, ("--cadence-s %d (explicit override; agrees with budget.json)"
                            % flag_value)
    v = read_cadence(run_root)
    return v, "budget.json cadence.%s = %d (the declared ceiling; no override passed)" % (
        CADENCE_FIELD, v)
