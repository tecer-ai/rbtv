...
| `screen-read-summary` | The audit record appended at the CLOSE of a continuous screen-read run, carrying the read count and the first/last read timestamps. Its partner is the fail-closed `screen-read` record written BEFORE the first screen is served — the two-record design keeps every read audited without one record per ~250 ms attach poll. Owner-approved 2026-07-20 (registry `system-definition/decisions.md#d-recon-batch08-run-close-rulings`). | Per-session audit file; `server/internal-api/keys-audit.js`, `server/internal-api/dispatch.js`; `probe-keys-coalesce.js` |
