...
// this set move in ONE change — this one, `internal-api/dispatch.js`, `runtime/gateway/parse.js`,
// `chat/forward-path.js`, `coord.py`'s `MESSAGE_TYPES` + `TYPE_COLOR`, and `schema.sql`'s
...
// (`supervisor/seeding.js` and `operator/attached-execution.js` path.join theirs), and resolving a relative path
...
function validateArgs(args, schemaJson, actionType, tools = null, workdirRoot = null) {
  ...
  if (actionType === 'fire-tool') {
    ...
    // (`envelope/spawn-profiles.yaml` rule 9 states it to operators: the daemon "refuses them at FIRE
    // as a shape error and records the execution `failed`"). MEASURED, not reasoned: without this
    // test, `runtime/ticker/probes/probe-argv-template.js` arm S2b could no longer build the fixture
...
class HeartStore {
  constructor(opts = {}) {
    ...
    try {
      ...
      this.config = {
        ...
        // nothing; it is here because `supervisor/seeding.js` needs each seat's cage template at
  ...
  // per seat, minted by `operator/goal-creation-request` and `supervisor/seeding.js`, whose
  ...
  // the `operator/attached-execution.js` launchOwed grammar (pending row OR prior execution), read
  ...
  enqueue(req) {
    ...
    // own cast is resolved at spawn (`supervisor/launch-profiles/catalog.js#specForSeatCast`), and an uncast
    ...
    if (sessionMode === 'headed') {
      ...
      // deletes. They are not dropped: `supervisor/spawn/spawn.js#spawn` now runs both against the
    ...
    // live: `chat/forward-path.js` forwardSessionCreate homes every goal-channel session
  ...
  listEnqueueUnfired(goal, cutoffIso) {
    // D52/D66 · a `braked` row was REFUSED — never inserted into `queue`, so it can never fire.
    // Without this exclusion every admission the brake stops would raise a false "enqueued, never
    // fired" stall alarm (brake-queue lane constraint 7 / risk: "the brake will trip the daemon's
    // own frozen-work alarm").
    return this._prepare(`
      SELECT * FROM enqueue_log e
       WHERE e.goal = ? AND e.at <= ? AND e.outcome != 'braked'
  ...
  // store 2026-08-12, every tick, to serve a caller (`supervisor/execution-record.js#publishToRecord`)
...
module.exports = {
  ...
  // (`runtime/ticker/warnings-check.js`'s unhomed fallback) reads rather than carrying its own
  // guess. Its former co-reader, `supervisor/lane-watch.js`'s `alarmOnStall`, is deleted [T4-R10].
