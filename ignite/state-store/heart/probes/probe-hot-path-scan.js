...
//   2. `supervisor/execution-record.js#publishToRecord` (every tick) called `listExecutionsByStatus` for
...
try {
  ...
  const dispatchSrc = fs.readFileSync(path.join(IGNITE, 'runtime', 'internal-api', 'dispatch.js'), 'utf8');
  const recordSrc = fs.readFileSync(path.join(IGNITE, 'supervisor', 'execution-record.js'), 'utf8');
  // ⚠ TWO FILES, ONE SCAN SET. `recordView`'s all-status walk moved to `supervisor/ending-reads.js`
  ...
  const seedingSrc = fs.readFileSync(path.join(IGNITE, 'supervisor', 'seeding.js'), 'utf8')
    + fs.readFileSync(path.join(IGNITE, 'supervisor', 'ending-reads.js'), 'utf8');
