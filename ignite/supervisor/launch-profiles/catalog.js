...
// abolished: `launch-specs:` in `envelope/spawn-profiles.yaml` is keyed BY THE PAIR ITSELF, and no
...
// `operator/bindings/tool/bindings.py`, because the two answer the same question from opposite
...
// The twin of `operator/bindings/tool/bindings.py#catalog`, which reads the same keys.
...
// as. The daemon lane passed one profile for every seat of a goal (`supervisor/seeding.js`), and the
// chat bridge passed a deployment-wide chat profile by surface (`chat/forward-path.js`) —
...
// 2026-08-11). It shipped inside `supervisor/spawn/spawn.js`, where `probe-caged-settings`'s standing
// "no per-spec special case anywhere in `supervisor/spawn/`" invariant reads spec knowledge as a
...
// `probe-caged-settings` holds `supervisor/spawn/` to "no per-spec special case anywhere" and a ladder
