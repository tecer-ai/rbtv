...
check('(6) on a cage-less host the SAME profile resolves the PORTABLE half', () => {
  const out = inCagelessChild(`
    const lp=require(${JSON.stringify(path.join(IGNITE_ROOT, 'supervisor', 'launch-profiles'))});
...
const body = [
  ...
  'command: node supervisor/launch-profiles/probes/probe-launch-profiles.js',
  ...lines,
