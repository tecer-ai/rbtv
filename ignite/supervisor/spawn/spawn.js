...
// through `supervisor/spawn/config.js`'s own upstream — this module already depends on that adapter,
...
function composeArgv(profile, mode, sessionId, workdir, prompt, dataRoot, resumeRef = null, effort = null, profileName = null) {
  ...
  // (probe-caged-settings, 'no per-profile special case anywhere in supervisor/spawn/'): which
...
// refusals and the reasoning all live in `supervisor/launch-profiles/catalog.js#specForSeatCast`; this side
...
// special case anywhere in `supervisor/spawn/`" and greps this tree for profile-name literals — the
...
function composeCageFor(resolvedSandbox, seatPath, resolvedWorkdir, gatewayAddr = null, log = () => {}, stamp = null) {
  ...
  // the `{runDir}` slot) in `envelope/spawn-profiles.yaml`, so the mountpoint has no consumer and
...
function createSpawnManager({ heartStore, configPath, logger = null, userManager = true, dataRoot: dataRootOverride = null }) {
  ...
  // `supervisor/launch-profiles/catalog.js#effortRungFor`, reading the profile's OWN ladder.
  //
  ...
  // `supervisor/spawn/` to "no per-profile special case anywhere". What a word MEANS is the config's
  ...
  async function spawn(execId, sessionMode = 'headless', prompt = null, workdir = null, enqueuedBy = 'unknown', resumeRef = null, effort = null) {
    ...
    // 2026-08-20 17:03/17:08, that is false — `runtime/index.js` routes to `spawnSeat` only when
    ...
    // existence. This is the ONE door it would pass. `runtime/index.js` routes session_mode
  ...
  async function spawnSeat(execId, { room, seatName, seatDir, dryRun = false, enqueuedBy = 'unknown', readLease = undefined, effort = null } = {}) {
    ...
    // (`runtime/lease/lease.js`) — same shape, same fail-closed posture, live evidence instead of a
  ...
  return {
    ...
    // (`runtime/engine.js`) can thread it to `supervisor/seeding.js`'s pre-enqueue admission gate. The
...
// were duplicated in `live-sessions.js` and `appendRowEnsuringHeader` in `operator/attached-execution.js`,
...
module.exports = {
  ...
  resolvePidStarttime,
  // Exported for `supervisor/launch-profiles/probes/probe-binding-catalog.js`: the seat-cast resolution is
  ...
  catalogRootForSeat,
  // Exported for `envelope/cage-admission.js` (§ D5): the pre-enqueue admission gate must reason
