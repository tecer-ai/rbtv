...
// runtime `.js` under `chat/` for `child_process|\bspawn\s*\(` and fails the suite on a
...
const { parseSeatPath, parseServiceSeatPath } = require('../../runtime/seat-identity/seat-folder');
const { appendRow } = require('../../runtime/seat-identity/csv');
...
function seatIsHumanInteractive(seatDir) {
  const { seatDirIsHumanInteractive } = require('../../chat/bus-ferry');
...
function createLiveSessions({
  ...
  function finish(s, reason) {
    ...
    // own log path is the honest pointer — passing nothing would leave the store's evidence
    // requirement satisfied by a bare session id that says nothing about the death.
    Promise.resolve(s.recorded).catch(() => {}).then(() => {
      try {
        closeSeatSessionRow({
          workdir: s.workdir, sessionId: s.sessionId, log,
          exitCode: `live-session-ended:${reason}`, logPath: s.logPath || null,
        });
      } catch {}
    });
  }
  ...
  async function feed({ conversationId, workdir, prompt, sessionRef = null, start = false }) {
    ...
    // is exactly when that is knowable. `operator/master-profile` restarts the chat bridge on
