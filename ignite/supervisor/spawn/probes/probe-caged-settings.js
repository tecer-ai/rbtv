...
// the REAL launch-spec table, and proves the wall it does NOT widen.
//
// Four checks, each with the control that makes it capable of failing:
//
//   1. ALL FOUR claude specs rewrite, loaded from the real `envelope/spawn-profiles.yaml`
...
const CONFIG = path.join(IGNITE, 'envelope', 'spawn-profiles.yaml');
...
for (const name of CLAUDE) {
  const profile = config.launchSpecs[name];
...
check('all four claude specs rewrite --settings into the launch dir (real catalogue)',
...
const { argv: haikuArgv, copies } = planCagedSettings(config.launchSpecs['claude/claude-haiku-4-5'].exec.argv, seatDir);
...
const mutated = CLAUDE.map((name) => {
  const argv = config.launchSpecs[name].exec.argv.map((a) => (a === '--settings' ? '--settings-MUTATED' : a));
...
// ── 3. no per-SPEC special case, with a fired positive control ───────────────────────────────
...
const inSpawn = grepFiles(NAMES, path.join(IGNITE, 'supervisor', 'spawn'));
const inConfig = grepCountFile(NAMES, CONFIG);
check('POSITIVE CONTROL: the model-literal pattern DOES match where the literals live',
  inConfig > 0, `${inConfig} line(s) in ${path.relative(IGNITE, CONFIG)}`);
check('no per-spec special case anywhere in supervisor/spawn/ (probes excluded, see grepFiles)',
  inSpawn.length === 0, inSpawn.map((f) => path.relative(IGNITE, f)).join(', ') || '0 files name a claude model');
...
const template = config.launchSpecs['claude/claude-haiku-4-5'].sandbox && config.launchSpecs['claude/claude-haiku-4-5'].sandbox.SeatBinds;
