...
const { splitRow } = require('../runtime/seat-identity/csv');
const { DOORS, refuseLaunch } = require('./doors');
...
const { deriveOwed, seatState: owedSeatState } = require('./owed');
const { admitLaunch, launchThroughDoor, storeDisagreeRefusal } = require('./launch-door');
...
// `envelope/spawn-profiles.yaml` and `workflow_launcher.py#launch_argv` states. The repo file is the
...
// `supervisor/launch-profiles/catalog.js#specForSeatCast`, keyed by the (harness, model) its bindings sheet
...
const GOAL_CLI_PY = path.join(__dirname, '..', 'operator', 'goals-tree', 'tool', 'goal_cli.py');
...
function seatCast(goalFolder, seat) {
  const { seatDeclaresValue } = require('./spawn/spawn');
...
// ledgers (`supervisor/reconcile.js`), never because a reader was told to look away.
...
module.exports = {
  ...
  recordView,
  launchOwed,
  // Exported for `envelope/probes/probe-cage-workspace-grammar.js`: the refusal->bus wire, driven
