...
$SUPERVISE launch --only <seat> --rerun <anchor>   # THE RE-RUN DOOR (D42) — `coordinate` no longer owns a `launch` verb (front door split, owner ruling 2026-08-25); `$COORD launch --only <seat> --rerun <anchor>` errors `invalid choice: 'launch'`. RE-RUN a seat whose last ENDED row reads `failed` off the ending store with reason class `crash` or `provider-error` (`RERUN_ADMITTED_REASON_CLASSES`, ignite/supervisor/launch.py) — the kit-written `exited` word is retired vocabulary and no longer the admitting state — as an ordinary working session citing the leader's investigation anchor; the admitted row stays and is superseded by the new one. LANE-AWARE (E22): on a goal whose `execution-lane` reads `daemon`, this ENQUEUES a caged headless sitting through the daemon (no tmux pane; --tmux-target refused there); on the `console` lane it composes an uncaged tmux pane instead (E22-3 control) — that variant applies only to console-lane goals, never the daemon production path.
