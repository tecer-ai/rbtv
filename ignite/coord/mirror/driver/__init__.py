...
worker packages (codex / opencode) consumes — the shared ``.agents/``
...
``render`` and the note in ``state.py`` at ``ALWAYS_EXCLUDED_PREFIXES``.  Only
the teardown side still knows the word (legacy records from a pre-retirement
render).

kimi-code-cli's config leg was retired 2026-08-18 (kimi models are reached via
opencode instead) — see ``config_assets.SUPPORTED_PACKAGES``.

This package composes the render legs:
  - ``library.py``       → ``.agents/behavior-rules/`` + ``.agents/skills/``
  - ``config_assets.py`` → ``.codex/`` config dir (a package with no assets
    seed, e.g. opencode, has ``config_dir=None`` and renders no config dir)
and ``state.py`` (the ``model_mirror`` block of ``rbtv.json`` + ref-counted
deletion + the shared ``write_if_changed`` primitive + ``ALWAYS_EXCLUDED_PREFIXES``,
the retired guidance leg's only surviving export).
...
from . import config_assets, library, state
...
class PackageFacts:
    ...
    config_dir: str | None  # config tree rendered into the target root; None = the package ships no config assets
...
PACKAGE_FACTS: dict[str, PackageFacts] = {
    ...
    "opencode": PackageFacts(
        config_dir=None,        # opencode ships no config assets — providers live in the machine-global opencode config
        guidance_owner="agents-md",
    ),
}
...
class UninstallResult:
    ...
    #: ``state.ALWAYS_EXCLUDED_PREFIXES`` — someone else's files that a
...
def _detect_leftover_worker_dirs(
    ...
    Scope:
      - a config dir (``.codex``) is in scope when its package
      ...
      - on a FULL teardown (no worker remains) EVERY known config dir plus the
        ...
        ``.codex/`` a tool created without rbtv ever electing it) is surfaced
...
def uninstall(
    ...
    always_excluded = list(state.ALWAYS_EXCLUDED_PREFIXES)
    protected = [
