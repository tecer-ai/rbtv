...
"Is this sitting alive?" is `liveness.py` (`supervisor/probe.js`) and NOTHING else
[spec-supervisor §6, T4-R8]. The three legacy predicates are retired as liveness and kept
for what they actually are: a tmux pane is a VIEWPORT (`live_panes` answers only whether a
wake can be delivered), the cgroup carrier is IDENTITY (`carrier_self_session`, and token
minting stays [T2-R8]), and tick silence was a statement about work product. The answer is
three-valued — `alive` / `dead` / `unsupervised` — and the third value is never collapsed
into either of the others: absence of a registry row has never been evidence of death.
The reap debt is derived from the same registry (`supervisor_door.awaiting_reap`); the
deleted `awaiting-close.json` and its pane-walking reaper have no successor here.

| File | What it is |
|------|-----------|
| `liveness.py` | The kit's ONE liveness surface — `sitting_alive` / `goal_liveness` / `occupied`, each a `node supervisor/probe.js` call against the supervisor registry (pid + `/proc` start-time). Every double-launch wall, the roster's status column and the reap debt ask here; no caller may read a pane, a carrier or a stored ledger status as a heartbeat [T4-R8]. |
| `coord.py` | The coordination CLI. Commands — everyday: `checkin` · `status` · `read` · `send` · `pending` · `checkout`; leader: `launch` · `close-seat` · `approve` · `panel` · `owner` · `add-to-group`; other: `workers` · `create-group` · `export-transcript` · `depart` · `selftest`. Identity is RESOLVED (calling pane → roster row, or `$COORD_AGENT`/`--as`) and verified, never typed — no command carries the caller's own name, and a claim contradicting the pane is refused. Messages are typed and threaded (`--re <ask#>`, required on answers); `read` is bounded (10 at a time, cursor advances only through what it SHOWED) with `--digest`/`--msg N`/`--after N`, and every filtered view is peek-only; `status` and `pending` answer "where am I" and "what is still open" in one shot; `--pretty` (or `COORD_PRETTY=1`) colours the view commands for a human reader, default output stays plain. Multi-harness (claude, codex, opencode — per-briefing `harness:`/`model:`/`effort:`/`ctx-refresh:`), per-seat launch profiles; `launch` pre-validates every seat's harness/model (alias/slug shapes, local knowledge only) and refuses BEFORE opening any pane (PROP-8). All state lives in the run package, resolved `--package DIR` > `--run TAG` (auto-registry) > `$COORD_PACKAGE` > cwd walk-up — a seat working in its own folder passes no flag at all. Session identity is TOLD to the seat: `checkin` stamps the durable trace `{package}/sessions.csv` — whose `checkin` column is the 15th, appended last, holding that session's LATEST check-in (a re-check-in moves the cell, it never adds a row), stamped on the seat's last open row and reaching live traces by header-widening, with an EMPTY cell meaning never-checked-in or a row predating the column and never read as `started` — and then prints the identity line `session: … · native: … · scratchpad: …`, every field on every check-in, each printing `UNRESOLVED` when that id could not be resolved. Every session opened through the kit creates the seat's per-session scratchpad `{seat folder}/sessions/{session-id}/` at session-open, and the boot prompt tells the seat to put every working file it produces there — the seat's `seat.md`/`agent.md` descriptor, its `memory.md` and any `conventions.md` STAY at the seat-folder root; the scratchpad is excluded from the boot-staleness walk alongside `transcripts/`, since neither is read at boot. `python3 coord.py selftest` verifies the mechanics; `coordinate -h` is the grouped command index, `coordinate <command> -h` the detail. It reaches PATH as `coordinate` through the deploy step below, never by hand. |
| `owed-answers.py` | The owner's OWED ANSWERS across every live package, in ONE command — `owed-answers` (on PATH through the deploy step below, like `coordinate`; `--workspace DIR` for another workspace, `--selfcheck` for its runnable check). It enumerates packages from `goals.csv` plus each goal's `runs.csv` OPEN rows — never a glob, which walks into seat scratch folders full of throwaway fixture packages — and asks each one for the UNION of the store's two owner-debt predicates: `coord.open_asks(..., to="owner")`, the SAME predicate `pending` and the check-out hold run, plus `coord.open_escalations` — an unanswered escalation is owner debt that HALTS the run, and is rendered with a `⛔ RUN HALTED · ` prefix and sorted above the asks so the cap can never truncate it away. The union lives in `owed-answers.py`, never in `open_asks` (ruling `p-owed-answers-locus`: widening that predicate would self-deadlock the escalating seat at its own check-out hold). It re-implements no pairing (PRIN-11): `coord.py` is imported as a library. Output is `seat.md` § 1's presentation exactly — the count, then the oldest-first list capped at 5, each with its thread pointer — and the empty state is EXPLICIT (`no owed answers`) so "nothing is owed" can never be confused with "this did not run". ⚠ WHEN THE CAP TRUNCATES, THE HEADER CARRIES A TOTAL FOR EVERY KIND — `17 owed answers · 10 ⛔ run-halted across 3 goals · 7 open asks — oldest 5 shown` — because a truncated list may never be the only place a count lives: on 2026-08-26 the header stated only the owed total and the channel master relayed the CAP as the halted count ("5 ⛔ RUN HALTED" against ten halts across three goals). READ THE HALTED COUNT OFF THE HEADER, NEVER BY COUNTING THE `⛔` ROWS SHOWN. `--all` lists every row (header without the cap clause); below the cap nothing is hidden, so the header stays the bare count and the empty state is byte-unchanged. ⚠ It exists because the master seats' prompts named the derivation without naming a COMMAND: every Slack DM re-derived the list by exploration at ~120 s of a ~131 s turn (measured 2026-08-12, ignite session log `6371e7ba`). It runs in ~0.15 s. |
...
| `roles.md` | The role catalogue (leader, deputy, scientist, judge, verifier, worker) + the codex/opencode harness note — read by a seat that HOLDS one of the special roles or runs a non-claude harness. Split out of `protocol.md` 2026-07-28. |
...
| `tmux-overview` | Owner utility: live view of one tmux session's windows/panes plus Claude plan usage (`tmux-overview <session>`; on PATH through the deploy step below). `--compact --package <run-package>` renders the ≤7-line control-panel dashboard via `overview-compact.py` — plan-usage bar charts left, windows with roster-resolved seat names right; the `panel` subcommand embeds exactly that in the leader window's 8-row strip. |
...
## Putting the kit on PATH — the deploy step

Four of these tools are NAMED as bare commands by callers that cannot see this repo's layout:
`module.md`'s "Reach it" column says `coordinate -h`, and the channel/console master prompts say
`owed-answers`. The names resolve through `~/.local/bin` symlinks, which are per-machine and never
synced by git — so a fresh clone, a redeploy or a second machine starts with none of them, the
caller hits its command-missing fallback, and `owed-answers` in particular reverts to the ~120 s
manual derivation it was built to delete.
...
Idempotent — a correct link is reported `ok` and left untouched; a link pointing elsewhere is
repaired; a REGULAR FILE with one of these names is refused, never clobbered. Adding a tool to the
kit that callers name as a bare command means adding its row to `TOOLS` in that file, in the same
change. Other repos' PATH names (`rbtv`, `teamview`, `acct`, `sb-task`, `sd-graph`) have the same
gap and their own owners — each module exposes its own.
...
3. Lifecycle: a seat ends with `checkout` — plain for the done disposition, `--renew --handoff
   ...
   agent in that path; protocol item 8 carries the evidence) — which exports its transcript
   first (ephemeral seats use
   `depart`, which exports, checks out and kills their own pane in one command). `close-seat
   <agent> [--renew]` is the FAILURE PATH, run directly by the daemon or leader (the `closer-*`
   seat class that used to spawn to do this was deleted [T2-R9], "only the daemon acts on other
   seats"): it kills — and with `--renew` freshly relaunches —
