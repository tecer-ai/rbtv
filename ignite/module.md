...
| `goal-launch-delay` | The channel master retiming its OWN queue delay — `show` what it is, `request` a change (the seat's verb: stage + trigger), `apply` it (the daemon's verb: edit `spawn-profiles.yaml` line-precisely, then restart). The knob has no settings file; it is the `--delay-seconds` operand of the `goal-creation-request` argv. | `capabilities/goal-launch-delay/tool/rbtv-goal-launch-delay <verb>` · `-h` |
| `master-profile` | The channel master choosing its OWN harness and model — the `master_profile` field of the chat-bridge config, validated against the LIVE `profiles:` set. Same three verbs; the apply restarts `rbtv-chat-bridge`, which ends the requesting sitting. No `--effort`: effort is measurably not on the master spawn wire. | `capabilities/master-profile/tool/rbtv-master-profile <verb>` · `-h` |
