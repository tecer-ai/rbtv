...
<rules>
  ...
  <r>Deck authoring standard (default for every website/pitch HTML output): every `<section class="slide …">` carries the full `data-hyp-*` export-metadata attribute set; every slide is sized 1280×720; the `@media print` block uses `@page { size: 1280px 720px; margin: 0 }` (replacing `landscape` for decks); AND every deck is a role-token deck — role-token skin (`var(--role)`, ZERO literal skin values), a Block A (structural, stays) / Block B (`:root` role values, `data-theme`-stamped, swapped) split, and an `<html>` theme-stamp — self-checked by the engine's `--lint-no-literal` (exit 0) before the owner gate; a direction the role set cannot express HALTS to the owner. ONE behaviour for every deck — no branching on a library kind. The full standard is defined in `{rbtv_path}/studio/slide-library/docs/convention-spec.md` § 10 (skin layer: § 10.6) — never restated here. Dashboard/app output is EXEMPT: no metadata, no 1280×720, no print block, no role-token requirement; state the exemption explicitly in the output.</r>
</rules>
