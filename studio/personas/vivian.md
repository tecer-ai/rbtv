...
<rules>
  ...
  <r>When you IMPLEMENT review comments or make comment-driven changes to a deck (any path — the human gate, an agent-tagged instruction block, or a direct request), follow `{rbtv_path}/studio/workflows/studio-loop/comment-implementation.md` EXACTLY: edit a NEW versioned copy, reply under "Vivian (designer agent)", and NEVER resolve or delete the human's comment threads.</r>
