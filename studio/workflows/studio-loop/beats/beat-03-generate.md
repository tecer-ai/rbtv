...
5. Record the winning trio as the deck's **visual contract** in design-state: write it to `{output_folder}/artifacts/art-direction/trio-winning.md`, set the `trio_contract` frontmatter path. Every subsequent slide conforms to it. Capture the contract AS ROLE TOKENS (§ 10.6): the trio's palette, type, depth, and geometry become Block B's `:root` role VALUES, and its component styling is expressed `var(--role)` (Block A) — so every slice is role-token from the first character, never tokenized after the fact.
...
6. **Output contract — deck authoring standard (default, website/pitch output):** a full-screen browser deck with print-to-PDF CSS. Every top-level slide is `<section class="slide …">` carrying the full `data-hyp-*` export-metadata attribute set defined in `{rbtv_path}/studio/slide-library/docs/convention-spec.md` § 10.1 (eight attributes: `data-hyp-slide-id`, `-section`, `-title`, `-audience`, `-lang`, `-summary`, `-kind`, `-provenance`). Every slide is sized **1280×720** (§ 10.2). The mandatory `@media print` block follows G-2 (`-webkit-print-color-adjust: exact`, `.slide { page-break-inside: avoid; }`) with `@page { size: 1280px 720px; margin: 0; }` replacing G-2's `@page { size: landscape }` for decks — one `@page` rule, not two (§ 10.3). Render-for-review via the local-server pattern; `file://` is blocked. NO PPTX.
   **Role-token skin (§ 10.6) — MANDATORY for every deck:** the deck is a role-token deck per `{rbtv_path}/studio/slide-library/docs/convention-spec.md` § 10.6 — Block A (the `.slide` base + every component rule, all skin properties `var(--role)`, ZERO literal skin values; stays on theme switch) + Block B (`<style data-theme="{theme}" data-theme-contract="2.0">` = a `:root` of the deck's role VALUES; swapped on switch) + an `<html>` `data-theme`/`data-theme-contract`/`data-theme-library` stamp. Role vocabulary = the engine's `ROLE_CONTRACT_V2` (`{rbtv_path}/studio/slide-library/engine/assemble.py`); only skin properties tokenize — layout/radius literals are copied verbatim. A direction that cannot be expressed in the role set HALTS to the owner (§ 10.6 escape) — never a literal skin value, never a distorted design. **Self-check (deterministic, BEFORE the owner gate):** `python {target-library}/assemble.py --lint-no-literal <deck> --json` → exit 0; any non-zero exit is a defect to patch surgically (3B step 4), never to surface.
...
- Any charting-library dependency; any PPTX affordance; a missing `@media print` block; a deck output missing the `data-hyp-*` export-metadata attribute set or 1280×720 sizing (§ 10.1–10.3); a deck carrying literal skin values or undefined-role `var()` tokens (fails `--lint-no-literal`), missing the Block A/B split or the `data-theme` stamp, or shipped to the owner without the `--lint-no-literal` self-check passing (§ 10.6); a literal skin value emitted to dodge a role the vocabulary lacks instead of HALTING to the owner; a dashboard output that incorrectly carries the deck standard without stating the escape.
