...
- **PHASE 1 — Strategist (message/narrative, surgical).** Revise the message working FROM the existing artifact and its comments. This is NOT blank-slate beat-01: the existing structure is the base, NOT content input to discard. Produce NO HTML. Execute Phase 1 in this fixed order:
  1. **Read cheaply.** Read the existing artifact and its comments. Comments live in the file's `#hyp-comments` island + agent-instruction block (`{rbtv_path}/studio/workflows/hypresent-comments/comment-implementation.md` § Locate the Comments First). For a large artifact, read the token-reduced lean view (`python {rbtv_path}/studio/hypresent/tools/hypresent.py dehydrate --file <artifact.html>`) — it carries a lossless comment digest plus the agent-instruction block; the raw `#hyp-comments` island is omitted unless the never-grow fallback reports `fallback: true` and emits the source verbatim. Read it, NEVER edit it or save it back.
