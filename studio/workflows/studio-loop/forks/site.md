...
Run `beat-01-message-lock.md` MANDATORY SEQUENCE step 1 (discovery — brief intake) as written: confirm `{project_name}` + `{audience_mode}` (here `site-marketing`), resolve and HALT on `{output_folder}`, read the brief / project-memo / entity root-level `.md` files, ingest any prior site as CONTENT INPUT ONLY (never a restyling base). Build the data wishlist; every gap becomes an Open Data Gap. This file does not restate that step.
...
| 4 | Charts — unchanged from the deck | Any chart is hand-authored inline SVG/CSS with an action-title — NO charting library. A library import is a defect, same as the deck. |
...
This fork concretizes the `artifact: site` structure beat + output contract + downstream wiring. It does NOT build: the app fork (`forks/app.md`, separate); production code wiring (coding agents own it); CMS/deployment; SEO copy beyond §A's narrative (site-path-spec Out of Scope). The owner entry surface and any standalone site command are install-set decisions deferred to a post-v1 decision (architecture §2), not built here.
