...
Run `beat-01-message-lock.md` MANDATORY SEQUENCE step 1 (discovery — brief intake) as written: confirm `{project_name}` + `{audience_mode}` (here `app-product`), resolve and HALT on `{output_folder}`, read the product brief / project-memo / entity root-level `.md` files, ingest any prior app as CONTENT INPUT ONLY (never a restyling base). Build the data wishlist; every gap becomes an Open Data Gap. This file does not restate that step.
...
2. Write each screen's UX companion doc to `{output_folder}/artifacts/ux/{screen-slug}.md` in the UX companion-docs contract format. The content spec's per-screen section points at the screen's UX doc; the UX docs + the (later) HTML are the coding-agent handoff package.
...
| 4 | UX companion doc travels with each screen | Each HTML screen ships WITH its UX companion doc (§C / UX companion-docs contract). The package — HTML screens + UX docs + the flow map — is the coding-agent handoff; the contract's self-sufficiency rule binds (a fresh coding agent wires from the package alone, asking the designer nothing). The Designer keeps each screen's HTML and its UX doc in sync: a screen whose designed states diverge from its UX doc's named states is a defect. |
| 5 | Imagery — real provenance only | Imagery comes via the image-gen capability (`{rbtv_path}/studio/capabilities/image-gen/`) OR owner-supplied assets. NEVER fabricate stock-lookalike photos passed as real photos of real things, people, or places. A screen needing an image whose asset is absent HALTS to the owner naming the missing asset — never an invented "real" photo. (The image-gen capability's FIXTURE provider is the quota-independent path — the contract may invoke the capability without assuming live image-gen quota.) Prompt/style/placement craft lives at `{rbtv_path}/studio/capabilities/image-gen/image-craft.md`. |
| 6 | Charts — unchanged from the deck | Any chart is hand-authored inline SVG/CSS with an action-title — NO charting library. A library import is a defect, same as the deck. |
...
- The coding-agent handoff is the OUTPUT, not a beat: the accepted package (HTML screens + UX companion docs + flow map) satisfies the UX companion-docs contract's self-sufficiency bar (a fresh coding agent wires the real app reading ONLY the package — app-path-spec behavior row 5). Production code wiring, backend, data models are OUT OF SCOPE here — the coding agent owns them (app-path-spec Out of Scope).
...
This fork concretizes the `artifact: app` discovery beats (§A goals · §B user-flow · §C UX) + output contract (§D) + downstream wiring (§E). It does NOT build: the site fork (`forks/site.md`, separate); production code wiring, backend, or data models (coding agents own them — app-path-spec Out of Scope); native-platform UI; accessibility certification (designed-for, not certified). The owner entry surface and any standalone app command are install-set decisions deferred to a post-v1 decision (architecture §2), not built here.
