...
It writes `<deck>.lean.html` — comments are represented by a readable, lossless digest plus the agent-instruction block; the raw `#hyp-comments` island is not copied except for the sanctioned never-grow fallback path, which emits the source verbatim and reports `fallback: true`. The visual layer (CSS, inline SVG, fonts, vendor JS) is stripped, every `id`/`class`/section is kept. Use it to read the comment set and to scan every slide for ripple. It is a READ surface only — never edit it, never save it back into hypresent. Use `python {rbtv_path}/studio/hypresent/tools/hypresent.py read --file <deck.html> --comment <id> --self` to inspect a commented element by literal `data-hyp-cid`, and `python {rbtv_path}/studio/hypresent/tools/hypresent.py read --file <deck.html> --selector "<css>"` for a one-off element read not tied to a comment.
...
- Operating through the app/bridge: the reply-under-agent-name and the untag-on-apply run through `{rbtv_path}/studio/hypresent/tools/hypresent.py reply --file <deck.html> --comment-id <id> --reply "<text>" --author "{agent-name} ({role} agent)"` appends the reply; `--clear-agent` drops that thread's `data-hyp-agent` tag on apply (Per-Comment Procedure steps 2–3). The tool round-trips the real runtime, so the `#hyp-comments` island, the element's `data-hyp-agent`, and the head instruction block stay consistent, and the runtime re-anchors every kept thread on save. The retired wrapper's `reply_comment: ERROR —` prefix is now `hypresent reply: ERROR —`; consumers grepping the old prefix must update. After the run, verify every kept thread is anchored (none showing `unanchored`).
