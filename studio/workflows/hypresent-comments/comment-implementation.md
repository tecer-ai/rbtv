...
- Operating through the app/bridge: the reply-under-agent-name and the untag-on-apply run through `{rbtv_path}/studio/hypresent/tools/reply_comment.py` — `--comment-id <id> --reply "<text>" --author "{agent-name} ({role} agent)"` appends the reply; `--clear-agent` drops that thread's `data-hyp-agent` tag on apply (Per-Comment Procedure steps 2–3). The tool round-trips the real runtime, so the `#hyp-comments` island, the element's `data-hyp-agent`, and the head instruction block stay consistent, and the runtime re-anchors every kept thread on save. After the run, verify every kept thread is anchored (none showing `unanchored`).
