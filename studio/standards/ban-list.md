...
The catalogue of attractors every studio output must avoid: the model's training-mean defaults plus the living corrections mined from the retiring `deck-design` data files. An attractor on this list reads as AI slop — generic, template-default, or credibility-destroying — and must never appear in a generated artifact.
