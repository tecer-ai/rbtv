...
| G-2 | A deck with no mandatory `@media print` block | Without it, colors and page breaks are unreliable in Decktape/Chromium export | Mandatory `@media print`: `-webkit-print-color-adjust: exact`, `.slide { page-break-inside: avoid; }`, `@page { size: landscape; margin: 0; }` (non-deck output). For a **deck**, `@page` MUST be `{ size: 1280px 720px; margin: 0; }` — one `@page` rule that replaces the landscape rule (see G-3 and `convention-spec.md` §10.3); the other two properties are unchanged. | P-3 |
| G-3 | A generated deck whose slides are not fixed at 1280×720 | Without a fixed slide size, slide-snapping and clean decomposition into library fragments are non-deterministic; `aspect-ratio` alone is G-1-unsafe in print | Every deck slide: `.slide { width: 1280px; height: 720px; }` (or the deck's equivalent fixed box). Pairs with G-2's deck `@page { size: 1280px 720px; }` — the two together guarantee one slide per page at the fixed size. Escape: HTML output that is a dashboard or app (neither website nor pitch) is exempt — see `convention-spec.md` §10.5 for the non-deck escape. | convention-spec §10.2 |
| G-4 | A generated deck whose slide `<section>` elements carry no `data-hyp-*` export-metadata attributes | Without per-slide export metadata, decomposing the deck into library fragments requires manual cleanup; metadata embedded in a `<script>` or `<head>` block is stripped when the slide is isolated into a bare `<section>` fragment | Every top-level deck slide (`<section class="slide …">`) MUST carry the full eight-attribute export-metadata set (`data-hyp-slide-id`, `data-hyp-section`, `data-hyp-title`, `data-hyp-audience`, `data-hyp-lang`, `data-hyp-summary`, `data-hyp-kind`, and optionally `data-hyp-provenance`). Attributes live on the `<section>` itself — NOT in `<head>`/`<script>` — so they survive decomposition. See `convention-spec.md` §10.1 for the full attribute contract and §10.4 for the fragment-invariant rationale. Escape: dashboard/app output is exempt from this rule (§10.5). | convention-spec §10.1 |
