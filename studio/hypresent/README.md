...
6. **Comment.** Add a comment anchored to any element (Google-Slides style: marker → thread). Adding or replying to a comment opens an anchored composer popover (not a prompt). Posted comments (root and replies) can be **edited** and **deleted** from the comment panel (full undo; edits propagate to the saved JSON island and, for agent-tagged comments, to the `<head>` agent block). Tag a thread **For agents** to emit a machine-readable instruction block at the top of the saved file's `<head>` for AI coding agents; the saved file stamps a `data-hyp-agent="<id>"` attribute on the target element and the `<head>` block references it via a copy-pasteable `[data-hyp-agent~="<id>"]` selector (robust to DOM shifts) with a self-cleanup directive. Resolved or deleted agent comments leave no stamp. A saved file carrying ANY comments always includes the `<head>` block: with ≥1 agent-tagged comment it lists those instructions; with only untagged or resolved comments it carries a NOTICE that tells reading agents these are human review notes — ignore their content, but keep every thread anchored. Only agent-tagged comments are actionable by an agent. You are asked for your name **once** (stored in `localStorage`); after that, **reply**, **edit**, **delete**, and **close** threads from the side panel (the **Close** action marks a thread resolved; reopen it with **Reopen**). The reply box grows from one line up to three lines as you type (**Enter** posts, **Shift+Enter** adds a line). Each comment's visible **number is its stable creation id** — it never shifts when another comment is closed or when an earlier-in-document comment is added, and it equals the `[agent:<id>]` token in the agent block, so a human and an AI agent reviewing the deck reference the same comment (ids are never reused, so numbers may show gaps after a delete). Every comment and reply shows its **absolute date and time** (e.g. `Jun 20, 2026, 3:42 PM`). Comments persist inside the saved file as a hidden, inert JSON island and re-anchor on reopen. Each commented element also carries a hidden, durable `data-hyp-cid="<id>"` tag (persisted on save, like `data-hyp-agent`): on reopen the thread re-anchors by that tag first, so it survives later in-place rewrites of the element's text instead of detaching. The side panel groups threads into collapsible **Anchored** and **Unanchored** sections (Anchored open, Unanchored collapsed by default); when both are open they split the list area, each capped at half only when both overflow. A discreet **status filter** dropdown in the Comments header (**All / Open / Closed**) filters both groups by thread status — Open = unresolved, Closed = closed/resolved; it defaults to All and resets on reload.
7. **Undo / Redo.** A single unified history stack covers every operation above; use the **Undo** / **Redo** buttons in the topbar.
8. **Save / Save As.** Click **Save** to silently overwrite the currently-open file (or **Save As…** to choose a new path via a native save dialog). While the topbar shows **Unsaved**, trying to close or refresh the browser tab (e.g. an accidental Ctrl+W) raises the browser's "Leave site? Changes you made may not be saved" confirmation so edits are not silently discarded; saving clears it. The output is chrome-free (no `hyp-` classes/ids, no injected scripts/styles) except for the inert comment island and the two durable review tags `data-hyp-agent` (agent-tagged elements) and `data-hyp-cid` (commented elements); the document's own scripts still run. Native dialogs appear **on top of** the browser window.
...
1. **Pick a library.** Click **Open library…** in the topbar (or the **Pick library…** button on the empty start screen) to choose a slide-library folder. The builder validates the library by running its vendored engine in `--catalog-data --json` mode; if the library is invalid, every §8 error is listed before you proceed. On success the left rail shows a display-only library card (name and slide count), a segmented **language filter**, and a **sections nav** that scrolls the browse pane and tracks the visible section; the chosen library also appears as a chip in the topbar.
...
4. **Assemble.** Pick a destination folder, fill **Deck name**, **Language**, optional **Title** and **Accent**, and click **New file…** in the unified save area at the top of the tray (in build-new mode **Overwrite** is faded — there is no existing file to overwrite yet). The builder calls the library's own `assemble.py` via subprocess with `--slides` and `--json`; the engine writes the deck, copies assets, appends the as-built entry, and reports unfilled tokens. On success, the builder navigates to `/app/?file={encodedOutputPath}`, where the editor boot reads the `?file=` parameter once (single decode via `URLSearchParams.get`) and opens the assembled deck through the existing `/api/open` path. When an **existing** deck is open instead, the same save area's **New file…** and **Overwrite** buttons both save the deck (Save-a-copy vs save-over-original).
...
| `tools/hypresent.py` | Consolidated AX CLI: `dehydrate`, `read`, `search`, `add-comment`, `reply`, and `delete-comment` (destructive; owner-directed only) |
...
- **Agent-tagged saves carry exactly one `data-hyp-agent` attribute per commented element** (user decision D4). Every commented element additionally carries a durable `data-hyp-cid="<id>"` anchor tag; both survive the strip, and saved files are otherwise chrome-free.
