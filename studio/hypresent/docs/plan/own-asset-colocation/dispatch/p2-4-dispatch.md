...
allowlist:
  ...
  - C:/Users/henri/Documents/second-brain/3-resources/tools/rbtv/studio/hypresent/app/js/builder/builder-main.js
  - C:/Users/henri/Documents/second-brain/3-resources/tools/rbtv/studio/hypresent/tests/test_deck_api.py
sandbox: workspace-write
commit_policy: none
test_command: NONE — the CONDUCTOR runs pytest and the headed warning proof (your sandbox cannot execute repo Python; do NOT attempt, do NOT report as a blocker)
forbidden_ops:
  - git push
...
- Work-target: the rbtv repo `C:\Users\henri\Documents\second-brain\3-resources\tools\rbtv` (branch `master`); the hypresent app dir is `studio/hypresent/` inside it. ALL task-body paths anchor there.
- The own-asset colocation feature is committed at `3ce0400`. The own-asset loop in `handle_deck_save` already computes each preserved section's referenced assets and silently `continue`s when the source file is absent — your change surfaces that case, never alters copy/rename/rewrite behavior.
- `[FULL READ]` before executing: `studio/hypresent/docs/plan/own-asset-colocation/decisions.md` (especially the 2026-06-12 owner option-B ruling) and `studio/hypresent/docs/plan/own-asset-colocation/specs/own-asset-colocation-spec.md`.
- The working tree carries uncommitted files from this run (`tests/e2e/test_pb11_deck_save.py`, `tests/e2e/p2_3_live_repro.py`) and possibly foreign parallel-session files — do not touch them.

## Allowed Paths

Frontmatter `allowlist` = complete write surface: `studio/hypresent/server/deck_api.py` (✎), `studio/hypresent/app/js/builder/deck-save.js` (✎), `studio/hypresent/app/js/builder/builder-main.js` (✎ — ADX-2: the status-bar message construction lives here; extend it for the warning fragment), `studio/hypresent/tests/test_deck_api.py` (✎ — add the three unit tests in the task's Test Plan).

## Forbidden Paths

Everything else — `server/recompose.py`, `server/server.py`, `tests/e2e/**`, `docs/**`, `app/**` beyond deck-save.js + builder-main.js.
...
- Implement Behavior items 1-4 of the task body exactly; minimal surgical diff; match each file's existing style.
- `assets_missing` entries are the deck-relative refs as written in the HTML (e.g. `assets/gsmm-logo.png`), deduplicated, stable (first-seen) order; emitted for OWN assets in preserved `existing` sections only — never library refs.
- Do not restructure the existing loop — add the collection at the existing missing-source skip point.
- In `deck-save.js`, extend the existing status message construction; non-blocking (success styling stays success; append a warning fragment when `assets_missing.length > 0`).

## Validation

You cannot run pytest (sanctioned skip — report `SKIPPED_COUNT: 1`, reason: sandbox cannot execute repo python; conductor runs it). Make the three new unit tests correct by inspection against the existing test conventions in `tests/test_deck_api.py`. The conductor runs the suite + a headed warning probe after your return.
