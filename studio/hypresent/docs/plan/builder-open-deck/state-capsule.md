...
- **REFRESH CANDIDATE (YES) — offered to owner at this phase-3-build/checkpoint boundary.** Context is full after a long run (resume + p3-1 + html→studio rename + p3-2 + p3-3 with a kimi→qwen re-route). If owner accepts a fresh session: the new conductor reads THIS capsule and dispatches p3-checkpoint (B10). If owner declines: continue in-session, dispatch B10 now. Nothing is in flight; all build committed + certified — a clean boundary either way.
