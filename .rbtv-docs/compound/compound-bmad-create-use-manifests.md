...
- Rule applies to output of `BMAD/.cursor/commands/bmad-rbtv-create.md` (Builder agent).
- Manifests already in use (e.g. `rbtv-manifest.csv`, `subagents-manifest.csv`) must be referenced rather than duplicating their content in generated files.
...
- **Builder agent** (`_bmad/rbtv/agents/builder.md`) — Produces agents, workflows, steps, tasks, commands, configs, knowledge, registries.
- **Templates** (`_bmad/rbtv/workflows/build/templates/`) — Used by Builder; may contain or suggest hardcoded lists.
- **rbtv-manifest.csv** — Canonical list of commands, skills, workflows, tasks, subagents; should be the reference for "what exists".
...
1. **Rule to be added (when implementing)**  
   ...
   - **Text (or equivalent):**  
     ...
     - "ALWAYS refer to the appropriate manifest (e.g. rbtv-manifest.csv, subagents-manifest.csv) as the single source of truth; use manifest path and column names (e.g. 'id', 'path') in instructions."  
...
2. **Manifest-first checklist (when implementing)**  
   - In Builder activation or build knowledge: before generating a file that needs to reference commands/skills/rules/subagents, the Builder must:  
     - Check if a manifest for that catalog exists (e.g. rbtv-manifest.csv, subagents-manifest.csv).  
...
| Related files | `BMAD/.cursor/commands/bmad-rbtv-create.md`, `_bmad/rbtv/agents/builder.md`, `_bmad/rbtv/rbtv-manifest.csv`, `_bmad/rbtv/subagents-manifest.csv`. |

---

## Rationale

- **Single source of truth:** Manifests (CSV or similar) already exist for rbtv (rbtv-manifest.csv, subagents-manifest.csv). Using them in generated files keeps catalogs up to date in one place.  
...
| File | Relationship |
|------|--------------|
| BMAD/.cursor/commands/bmad-rbtv-create.md | Command that loads Builder; rule applies to its output. |
| _bmad/rbtv/agents/builder.md | Builder agent; candidate location for rule or reference to rule. |
| _bmad/rbtv/rbtv-manifest.csv | Example manifest for commands, skills, workflows, tasks, subagents. |
| _bmad/rbtv/subagents-manifest.csv | Example manifest for subagent ids; referenced by plan-creation-rules. |
| _bmad/rbtv/workflows/planning/plan/data/plan-creation-rules.md | Example of referencing manifest instead of hardcoding (subagents). |
...
- Compound workflow: `_bmad/rbtv/workflows/doc/compound/workflow.md`.
