...
## Patterns from System Audit (Tasks 2 & 3)

> **Source:** RBTV System Audit (commits 4ea0f9a, 05b7637). These patterns were discovered by auditing M1-M4 components already built by this plan. Errors found and fixed in completed milestones are documented below as do/dont patterns so that pending tasks (M5, M6) avoid repeating them.

### Milestone Transition Patterns

**The problem found:** When a milestone's FINAL framework synthesis step completes, it must advance `currentMilestone` in project-memo.md frontmatter. M2→M3 (Pre-mortem synthesis) and M3→M4 (Tone of Voice synthesis) both forgot this, breaking cross-milestone auto-routing.

| # | DONT | DO |
|---|------|-----|
| 1 | Forget to update `currentMilestone` in the last framework's synthesis step | In the LAST framework's synthesis step of each milestone, **always** set `currentMilestone` to the next milestone (e.g., `currentMilestone: m6-mvp` when M5 is complete) |
| 2 | Assume `stepsCompleted` alone is sufficient for milestone routing | `stepsCompleted` tracks framework completion; `currentMilestone` tracks which milestone to route to — both are required |

**Affected pending tasks:** p6-15 (Sean Ellis PMF is the final M5 framework — its synthesis MUST set `currentMilestone: m6-mvp`)

### Menu Code Patterns

**The problem found:** M4 step-01-init.md used `[B]` for both "Build Prototype" and "Back" in the same menu, causing ambiguous routing.

| # | DONT | DO |
|---|------|-----|
| 3 | Reuse single-letter menu codes within the same menu for different items | Ensure every menu code is unique within its menu; if a framework uses `[B]`, use `[BK]` for Back |
| 4 | Use codes that are substrings of other codes without clear disambiguation | Prefer 2-letter codes for frameworks (e.g., `[MT]`, `[SS]`) and distinct codes for navigation (`[S]`, `[BK]`, `[X]`) |

**Affected pending tasks:** p6-9 (M5 milestone) and p6-17 (M6 milestone) — verify no code collisions between framework codes and navigation codes

### Synthesis Return Routing

**The problem found:** M4 Design Context synthesis said "return to milestone selection (bi-business-innovation)" but should have said "return to M4 Prototypation milestone menu". These are different destinations.

| # | DONT | DO |
|---|------|-----|
| 5 | Say "return to milestone selection" in a framework synthesis — this skips the milestone menu | Always route framework synthesis back to the PARENT MILESTONE menu (e.g., "return to M5 Market Validation milestone menu") |

### Cross-Milestone Strategic Context Loading

**The problem found:** M4 had zero dependency on M2 validation outputs. User Flow & IA, Design Context, and Conversion Optimization were built without reference to TRL feasibility, Leap of Faith assumptions, or Unit Economics pricing. This meant prototype decisions were ungrounded in validation results.

| # | DONT | DO |
|---|------|-----|
| 6 | Build downstream milestone framework init steps that ignore upstream milestone outputs | Load relevant upstream milestone outputs as context in init steps: M2 validation outputs in M4 init, M4 design outputs in M5 init, M1-M5 context in M6 init |
| 7 | Treat milestone boundaries as walls — M1 stands alone, M2 stands alone, etc. | Context flows downstream: each milestone's frameworks should reference the most relevant outputs from prior milestones |
| 8 | Build conversion/marketing frameworks without grounding in messaging and customer forces | Load Messaging Architecture (message hierarchy) and JTBD (customer forces) when building conversion, sales, or marketing frameworks |
| 9 | Assume all upstream context is always available — some frameworks may not have been completed | Use "if available" pattern: load upstream outputs when they exist, proceed with founder knowledge fallback when they don't, but WARN about the gap |

**Specific upstream context to load per milestone:**

| Building frameworks in... | Load from... | Key outputs to reference |
|--------------------------|--------------|--------------------------|
| M4 (Prototypation) | M2 (Validation) | TRL posture, LoF assumptions to test, UE pricing model |
| M4 (Prototypation) | M3 (Brand) | Messaging Architecture, Brand Positioning |
| M5 (Market Validation) | M4 (Prototypation) | Prototype design outputs, user flows, conversion hypotheses |
| M5 (Market Validation) | M2 (Validation) | Assumption backlog (to know what to validate) |
| M5 (Market Validation) | M3 (Brand) | Messaging Architecture (for testing messaging with customers) |
| M6 (MVP) | M5 (Market Validation) | Persevere/pivot/kill decision, PMF score, validated pricing |

**Affected pending tasks:**
- p6-10 to p6-15 (M5 frameworks): Should load M4 prototype/design outputs and M2 assumption backlog as context
- p6-12 (Smoke Test) and p6-15 (Sean Ellis PMF): MUST load M4 prototype outputs since they test the prototype directly
- p6-17 (M6): Already notes loading M1-M5 context; should specifically reference M5 persevere decision
