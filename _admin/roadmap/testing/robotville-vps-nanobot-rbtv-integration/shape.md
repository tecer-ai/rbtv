...
### 2026-02-14 - p6-5 Deploy Command Routing and Rules Added to Bootstrap Files

- **Decision:** Add deploy command routing to `_mobile/TOOLS.md` and deployment safety rules to `_mobile/SOUL.md` as the bootstrap-level integration for the robotville.ai deployment pipeline.
- **TOOLS.md changes:** (1) Updated routing rules to recognize `deploy` as a standalone action command distinct from agent persona commands. (2) Added "Deploy Commands" section with two entries: `deploy site` (Active — deploys home page via `netlify deploy --dir=... --prod`) and `deploy docs {project}` (Deferred — awaiting deploy workflow creation from original p6-4 scope). Includes execution steps for `deploy site` and a note that `deploy docs` is not yet implemented.
- **SOUL.md changes:** Added "Deployment Rules" section between Context Window Resilience and Security and Safety, codifying three rules: (1) deploy ONLY on explicit user command (never automatic), (2) confirm target/source/destination with user before executing, (3) report deploy URL and status after completion.
- **Alignment with p6-4 deferral:** The original p6-4 deploy command/workflow was deferred (depends on project docs to deploy). The TOOLS.md routing documents `deploy docs` as "Deferred" so the bootstrap file is pre-wired for future activation without requiring another TOOLS.md update when the workflow is built.
- **Validation:** Both files maintain consistent structure with existing sections. Deploy routing in TOOLS.md aligns with Netlify CLI contract from p6-2 (`netlify deploy --dir=<path> --prod`). SOUL.md rule prevents the failure mode where framework completion auto-triggers deployment.

### 2026-02-14 - p6-5 Pre-Push Safety Audit — Five Issues Found and Fixed
...
### 2026-02-14 - Quality Review: p6-3 Scope Reduction and p6-4 Task File Stale

- **Decision:** Pre-push quality review identified that p6-3 delivered a waitlist landing page instead of the full home page with `/docs/` and `/app/` navigation specified in the task. This was a practical simplification — no content exists for `/docs/` or `/app/` yet — but the plan YAML and task file were not updated to reflect the reduced scope. Similarly, p6-4.task.md still described the original deploy command/workflow scope instead of the waitlist work that was actually executed.
- **Fix — p6-4.task.md:** Rewritten to describe the actual waitlist email capture work (Netlify Forms, honeypot, success page). Original scope noted as deferred in frontmatter.
...
- **Design adaptation:** Artprize-shadows visual language (minimal, confident, spacious, gallery-like) translated to Robotville: deep navy (`#011B50`) hero section, warm off-white (`#ECEAE4`) content sections, purple (`#412762`) accent/CTAs, pill-shaped buttons (`border-radius: 50px`), smooth transitions (`cubic-bezier(0.32, 0.94, 0.6, 1)`), generous section padding. System fonts only (no external dependencies). Glow-style shadows on dark sections. Opacity-based interaction states.
- **Content sourcing:** Hero headline from WB press release ("napkin to reality"); subtitle synthesized from lean canvas UVP and PRD executive summary; "What it is" from PRD executive summary + lean canvas differentiation; "How it works" pipeline from PRD user journeys + project-memo solution description; "Who it's for" from PSF customer segment + lean canvas early adopters + WB customer examples; "Why Robotville" differentiators from lean canvas section 3 (vs ChatGPT, vs consultants, vs vibe coding).
- **Pages delivered:**
  - `_mobile/_docs/netlify-placeholder/index.html` — Production home page with nav, hero, what/how/who/differentiators/waitlist/footer sections.
  - `_mobile/_docs/netlify-placeholder/docs/index.html` — `/docs/` placeholder ("Project documents appear here when deployed via the `deploy docs` command").
  - `_mobile/_docs/netlify-placeholder/app/index.html` — `/app/` placeholder ("Project applications appear here when deployed via the Mentor workflow").
  - `_mobile/_docs/netlify-placeholder/success.html` — Updated for visual consistency with new design (same nav, color palette, typography, layout).
- **Netlify Forms preserved:** Waitlist form retains `name="waitlist"`, `data-netlify="true"`, `netlify-honeypot="bot-field"`, `action="/success"`, hidden honeypot field.
- **Responsive:** Three breakpoints (64em, 48em, 30em). Grid layouts collapse to single column, form stacks, pipeline adapts. No horizontal scroll on mobile.
- **Zero external dependencies:** System fonts, no JavaScript, no build step. Deploys via `netlify deploy --dir=... --prod`.
