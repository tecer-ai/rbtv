...
The repo is module-first: resolve the owning module BEFORE the type path. Read `{rbtv_path}/admin/install/module-manifest.json` for the module list and `{rbtv_path}/modules/{module}.md` for each module's scope. If no existing module fits, confirm the target module with the user.

| Component | Destination |
|-----------|-------------|
| Workflow | `{rbtv_path}/{module}/workflows/{name}/` |
| Agent/Persona | `{rbtv_path}/{module}/personas/{name}.md` |
| Rule | `{rbtv_path}/{module}/rules/{name}.md` |
| Task | `{rbtv_path}/{module}/tasks/{name}.xml` |
| Skill (thin loader) | `{rbtv_path}/{module}/skills/{name}/SKILL.md` |
| Knowledge/Data | `{rbtv_path}/{module}/workflows/{parent}/data/` |
