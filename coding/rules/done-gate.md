...
An agent MUST NOT declare a coding task done until it has exercised every contracted outcome criterion the way the owner will use the feature and exhibited the evidence. Done is purchased with user-fidelity evidence — nothing else buys it. And when a contracted outcome runs through a surface the agent cannot drive or observe, the agent MUST build a test seam for that surface as part of the feature — so the outcome is exercisable at the Fidelity Floor without faking the owner's gesture.

## Trigger and Scope

**Activates when:** a coding task starts (Contract) and a done-claim is about to be made (Exercise + Exhibit). Default-ON for every coding task — feature, bug fix, behavior change — in any workspace where this rule is installed. At Contract, the same scope also triggers the per-criterion drivability check (§ The Gate — Contract row + Drivability Check).

**Does NOT apply to:**

- Non-code work (documents, plans, vault content) — out of scope entirely.
- The exempt list — the ONLY exit for coding tasks: typo and comment fixes, formatting-only changes, internal cleanups with zero behavior change (renames, dead-code removal), config changes invisible at runtime. Fails safe: not on the list → gated, AND not exempt here → the drivability check applies too.
- Drivability only: a criterion whose surface the agent can ALREADY drive and observe with a real gesture — a normal in-page control, a CLI flag, a file it reads/writes. Mark it `drivable` and move on; no seam needed.

**Boundary:** internal correctness (unit tests, type checks, builds) remains the agent's normal testing job. This rule governs ONLY the done boundary — tests green, builds passing, and code review do NOT buy done. The drivability check governs what you BUILD so the outcome is drivable; HOW you then exercise and prove it is the Exercise/Exhibit job below.
...
The sheet carries TWO row families, both created at Contract time:
