...
Copies .cursor/ folder from rbtv/_config/ to the project root (three folders up from this script).
...
def get_paths():
    """Get source and destination paths. Root is three folders up from this script."""
    script_dir = Path(__file__).parent.resolve()  # _bmad/rbtv/_config/
    rbtv_dir = script_dir.parent  # _bmad/rbtv/
    root = rbtv_dir.parent.parent  # project root (three folders up)
...
def main():
    ...
    print(f"  From: {paths['config'] / '.cursor' / 'mcp.json'}")
    print(f"  To:   {root / '.cursor' / 'mcp.json'}")
    
    cursor_mcp_stats = merge_mcp_json(
        paths["config"] / ".cursor" / "mcp.json",
    ...
    cursorignore_stats = merge_cursorignore(paths["config"], root)
