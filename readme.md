...
2. Run the installer:
   ...
   - Orchestration model packages (when the `orchestration` module is selected): which per-model CLI dispatch packages — `kimi-code-cli`, `codex-cli`, `claude-code-cli`, `opencode` — to make available in this workspace. These are doc packages read just-in-time from the RBTV source (not copied into `.claude/`); the installer records the selection in `rbtv.json` `model_packages` — the single election authority `route.py` reads; nothing is baked into the shared repo, and the `rbtv-orchestrating` skill recalls what is routable here on demand via `python {rbtv_path}/orchestration/models/route.py --availability` (which prints the elected and not-elected package ids). It also reconciles the workspace's `.claude/settings.local.json` permission allowlist from each package's manifest `permission_rules` (e.g. `Bash(kimi:*)` / `PowerShell(kimi:*)`): an elected CLI package's entries are added so a conductor session may spawn that worker in-session (D17), a deselected package's entries are removed, and only the exact manifest-declared strings are ever touched — hand-added entries survive. Scripted: `--model-packages kimi-code-cli,codex-cli` (omit to keep the prior selection or elect all; `--model-packages ""` elects none). A **configurable** package (manifest `configurable_model.is_configurable`, e.g. `opencode`) surfaces its native backends as separately-electable, provider-path-labeled rows so the owner can elect a subset — e.g. only the DeepSeek-via-opencode backends, kept distinct from the direct `deepseek-api` package; scripted via `--model-variants opencode=deepseek-flash,deepseek-pro` and listed via `--list-model-backends`. A proper-subset election is recorded in `rbtv.json` `model_variants` (a full election records none; an absent entry routes all backends — back-compat), and `route.py` routes only the elected backends. The installer also runs an advisory manual-render freshness check (warns if any generated manual is stale relative to its sources; never aborts the install). Electing a package that carries a mirror config also **renders its worker mirror** in the target workspace (guidance file, per-model config dir, and skill loaders under `.agents/`) via the mirror driver at `orchestration/models/mirror/driver/`; deselecting a package removes its mirror artifacts (ref-counted — shared files such as `AGENTS.md` are only removed when all packages that share them are deselected). Mirror state is persisted in a single `model_mirror` block in `rbtv.json`. Use `--mirror` to refresh mirror artifacts only (skips component install, reads the existing `rbtv.json` election). Use `install.py --mirror --check` to ask whether the mirror is current WITHOUT refreshing it: a read-only probe that writes nothing, names every managed file missing or fallen behind its source, and exits 1 on drift / 0 in sync (gating-ready for CI and pre-flight checks) — worth having because mirrors are gitignored and refresh only when the installer runs, so drift is per-machine and invisible to git. `--check` applies only with `--mirror`, and is mutually exclusive with `--uninstall` (exit 2). Use `--exclude PATH [PATH ...]` to skip paths: no guidance file is rendered beside any `CLAUDE.md` inside an excluded path; passing `--exclude` REPLACES the recorded list in `rbtv.json` (`model_mirror.excluded_paths`), omitting it PRESERVES the recorded list; applies on both a full install and `--mirror`. Excluding an already-mirrored path now DELETES that path's existing guidance files (banner-guarded deletion — hand-authored files are spared; never runs under `--check`). On top of whatever a workspace configures, the walk ALWAYS skips `.rbtv/goals/` (the driver default `ALWAYS_EXCLUDED_PREFIXES` in `driver/guidance.py`): a goal folder's routers — `CLAUDE.md` AND `AGENTS.md` — are written by the goals-tree scaffold (owner ruling Q18, 2026-08-09), so the mirror neither renders over them nor records them as managed. It is a driver DEFAULT rather than a per-workspace `excluded_paths` entry because `--exclude` REPLACES that list and `.rbtv/` exists in every workspace rbtv serves; and it deliberately does NOT feed the prune-on-exclude deletion — the scaffold's router header carries the same DO-NOT-EDIT sentinel, so the banner-guard would not spare it. Uninstall is the mirror image and DOES consult the same constant: a workspace upgraded from a pre-exclusion driver still carries those routers in its recorded managed-file list, so `install.py --mirror --uninstall` (and a deselection teardown) partitions them OUT of its delete set instead of deleting them (`UninstallResult.protected`) — they stay on disk, un-managed, and are reported under a `protected` label beside the deleted/spared counts. Use `install.py --mirror --uninstall` for a full mirror teardown: removes ALL generated mirror artifacts for the target (guidance files, the shared `.agents/` library, and per-model config dirs) — except any recorded path under `ALWAYS_EXCLUDED_PREFIXES`, which is protected (left on disk, un-managed) rather than deleted — and clears the `model_mirror` block from `rbtv.json`; `--uninstall` applies only with `--mirror`. The orchestration model packages also include **API-worker packages** (`deepseek-api`, `gemini-api`, `claude-code-native`, `manus-api`) that run via a shared API runner (`orchestration/models/_api/run.py`) with keys resolved from the `env_file` path in `rbtv.json` — distinct from the CLI dispatch packages (kimi-code-cli, codex-cli, claude-code-cli, opencode), which invoke local executables rather than calling provider APIs. (`opencode` is the OSS multi-provider CLI worker serving four backends — `z1` (zai-coding-plan/glm-5.2), `sakana` (fugu-ultra), and the `deepseek-flash`/`deepseek-pro` code executors inherited from the retired qwen-code-cli; per-backend API keys via the manifest `auth.env_var` override — and `z1` also resolves from a stored `opencode auth login` credential via `auth.credential_store`, either path satisfying availability (ruling 2026-07-15); NO native sandbox — worktree-mandatory confinement.) (`manus` is the agentic-web worker — autonomous server-side browser/tool loop, per-task cost, raw-dump return only.)
