...
| **studio** | Design and communication module — the four-beat studio loop (message-lock · art-direction · generate · human-gate) covering deck, site, and app artifacts; the Strategist persona (investor + client audience modes); Vivian the Designer (`rbtv-designing`); standards bundle (ban-list + flaw-checklist); design-state schema; reference-set scaffold; design-token extraction from live sites; reference-image forensics into regeneration prompts (`/rbtv-vision-to-json`); browser automation; AI image generation; exemplar-screenshot capture; motion/interaction reference extraction; and the hypresent presentation engine | [modules/studio.md](./modules/studio.md) |
