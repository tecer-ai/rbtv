# V3-T8 — R3 test: delete/undo/thread-fate/guards/no-keyboard

You are Kimi, a code executor. Do EXACTLY what this file says. Read nothing else. Output ONLY the file in the allowlist. Depends on V3-T7 (R3 feature landed).

## D14 addendum (binding)
On a blocking product bug, STOP and write `docs/verification/v3/v3-t8-BUG.md` — never patch product files. NEVER create files at the workspace root — scratch goes under a tempdir.

## Goal
CREATE `tests/e2e/test_r3_delete.py` (port 8793): delete outcome (DOM node removed), undo (restored at original index), thread → unanchored + re-anchor on undo, last-region block (via `doc_eval` DOM surgery reducing the body to ONE tagged region), edit-active guard, no-keyboard-path (Delete/Backspace do nothing — U14), Moveable teardown after delete, no console errors.

## How the feature works (from V3-T7 + existing)
- `conftest_helpers.py`: `start_server`, `stop_server`, `copy_fixture`, `open_via_dialog_ui`, `doc_eval`, `preset_author`, `FIXTURE`. (`preset_author(page)` EXISTS in conftest_helpers.py — it adds an `addInitScript` that pre-seeds the author name in localStorage; it MUST be called BEFORE `goto` so the init script fires before the first navigation, suppressing the one-time author-name prompt — RV13.)
- Toolbar: `#delete-btn` (parent doc). Undo is the toolbar button whose text is "Undo".
- Bridge commands available via `page.evaluate` on the iframe runtime are NOT directly callable from the test; instead drive the UI (`#delete-btn`, Undo) and assert DOM via `doc_eval`. To probe `comments-read`/`get-selection` results, use the existing comment panel UI and DOM, or call the bridge through the parent shell's `bridge` — NOT accessible from the test. Use DOM-observable outcomes only.
- Select an element with a REAL mouse click at its on-screen center (no dispatchEvent).
- To add a comment: select an element, click `#comment-btn` (opens the composer popover `.hyp-comment-composer`), type in its textarea, press Ctrl+Enter. The author prompt is skipped by pre-seeding localStorage (`H.preset_author` adds an init script — call it BEFORE `goto`).

## CREATE `tests/e2e/test_r3_delete.py`
```python
import os, sys
...
> **Note on E-R3-5 (last-region block):** the fixture's body has multiple `section.slide` regions, so a single delete never naturally hits the guard. `test_last_region_block` (above) makes the guard observable by `doc_eval` DOM surgery on the TEMP fixture copy: it removes every body-level `[data-hyp-id]` sibling except the first (the survivor keeps its boot-time registry entry, so `byId` still resolves it), then attempts a toolbar delete and asserts the survivor STILL EXISTS and the `#shell-status` shows "Cannot delete the last remaining region." Fail-loud preconditions guard the surgery (≥2 regions before; exactly 1 after). The surgery is on the in-iframe document of the temp copy only — the original gitignored fixture file is never touched (RV01).
...
1. `python -m unittest discover -s tests/e2e -p "test_r3_*.py" -v` → all pass on port 8793, NO skips.
2. The delete test asserts the element is GONE (`querySelector('[data-hyp-id="…"]')` null) and restored on undo with the same id + previous sibling.
3. The no-keyboard test asserts Delete/Backspace leave the element intact.
3a. `test_last_region_block` reduces the body to ONE tagged region via `doc_eval` surgery (fail-loud preconditions: ≥2 before, ==1 after), attempts a toolbar delete, and asserts the region STILL EXISTS and `#shell-status` contains "Cannot delete the last remaining region" (E-R3-5 — RV01).
4. No `dispatchEvent` for selection.
5. `git diff` shows ONLY the one new file.
6. Write evidence to `docs/verification/v3/v3-t8-run.txt`.

## Do NOT
- Do NOT modify any product file. If delete does not work or undo does not restore, write `docs/verification/v3/v3-t8-BUG.md` and STOP.
