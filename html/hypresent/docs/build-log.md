...
## Session 4 — prez-builder v1

Shipped the RBTV slide-library convention, the vendored assembly engine, the hypresent builder page, and the tecer library migration in one coordinated cut. Canonical specs (`convention-spec.md` with ADX amendments and `fixture-spec.md`) now govern library layout, manifest parsing, asset resolution, the library-YAML as-built grammar, and the cold-agent interface. The engine (`html/slide-library/engine/assemble.py`) implements the 24-rule validation table, the `--json` envelope, `--catalog-data`, `--no-log`, and the vendored copy model; `install-engine.py` re-vendors it into a target library and syncs the `engine_version` stamp. The builder page (`app/builder.html` plus seven `app/js/builder/*` modules plus `server/builder_api.py`) consumes the library's own engine via subprocess, renders section-grouped browse with IO-gated srcdoc previews, provides a pointer-events drag-reorder tray, and hands the assembled deck off to the editor via `?file=`. The migration converted the live tecer library to the v1 convention, vendored the engine, and merged to tecer-biz `main` after round-trip and DT5 reproduction validation.
