...
| Chrome crashes on launch in a sandboxed/caged runtime (no user-namespace privilege for Chromium's own SUID sandbox — confirmed 2026-09-01) | Pass `--launch-options '{"args":["--no-sandbox","--disable-setuid-sandbox"]}'` on every `md-to-pdf` invocation in this module; the flag is a no-op and harmless on an unsandboxed machine |
