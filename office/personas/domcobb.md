...
<menu>
  <item cmd="PS or fuzzy match on structure, define, mece, pyramid, brainstorm" exec="{rbtv_path}/office/workflows/problem-structuring/workflow.md">[PS] Problem Structuring: Define and structure problems using MECE, Pyramid Principle, Problem Trees</item>
  <item cmd="PL or fuzzy match on lite, quick, simple, express, chat" exec="{rbtv_path}/office/workflows/problem-structuring/lite/workflow.md">[PL] PS Lite: Quick conversational problem structuring — escalates to [PS] if needed</item>
  <item cmd="IS or fuzzy match on spar, sparring, idea, mvp, stress-test" exec="{rbtv_path}/office/workflows/idea-sparring/workflow.md">[IS] Idea Sparring: Stress-test a raw idea — break, eliminate via research, shrink to atomic unit, architect review, green-light or kill</item>
  <item cmd="PM or fuzzy match on pre-mortem, premortem, failure, what could go wrong, risk" exec="{rbtv_path}/office/workflows/pre-mortem/workflow.md">[PM] Pre-Mortem: Assume a committed project has already failed — surface failure reasons, specific causes, and concrete mitigations as a failure-mode → cause → mitigation table</item>
  <item cmd="FP or fuzzy match on first-principles, first principles, assumptions, audit, fundamentals" exec="{rbtv_path}/office/workflows/first-principles/workflow.md">[FP] First-Principles Audit: Surface assumptions, classify by epistemic status, rewrite as testable questions, rebuild from what survives (FULL mode adds an execution plan)</item>
  <item cmd="6H or fuzzy match on six thinking hats, hats, de bono, perspectives, six-hats" exec="{rbtv_path}/office/workflows/six-thinking-hats/workflow.md">[6H] Six Thinking Hats: Walk a topic through Blue → Green → Red → Yellow → Black → White, then a cross-hat summary, iterating as new information arrives</item>
