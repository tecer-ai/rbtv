...
| Rules directory | `.cursor/rules/` | Corrected from compound doc's `_config/.cursor/rules/` which doesn't exist |
| Universal template location | `workflows/_shared/templates/shape-template.md` | Shared workflows directory; decouples template from plan-lifecycle ownership |
...
|---|----------|-----------------|-----------------|------------------|
| 1 | Path correction | Accepted | Discovered `_config/.cursor/rules/` doesn't exist; rules live at `.cursor/rules/` | Use corrected path for both new rules |
...
**Decision:** Created context preservation rule in both locations: canonical source at `_config/claude/rules/bmad-rbtv-context-preservation.md` (Claude Code) and workspace copy at `.cursor/rules/bmad-rbtv-context-preservation.mdc` (Cursor). Body content identical; only difference is `alwaysApply: true` in `.mdc` frontmatter.

**Rationale:** Matches the existing pattern for all 7 other RBTV rules. The plan's shape.md originally referenced `.cursor/rules/` as the target, but that directory doesn't exist and isn't where any other rule lives. Following existing conventions over the plan's stated path.

### 2026-03-17 — P2 Discovery: memory system rule missing workspace .mdc

**Discovery:** P1's memory system rule was created only at `_config/claude/rules/bmad-rbtv-memory-system.md` — it has no `.cursor/rules/bmad-rbtv-memory-system.mdc` workspace copy. All other RBTV rules have both. This means the memory system rule is not active in Cursor sessions.
...
**Decision:** Moved universal shape template from `_shared/templates/shape-template.md` to `workflows/_shared/templates/shape-template.md`. Removed the empty `_shared/` directory.
...
**Discovery:** The shape.md Key Decisions table specified `_shared/templates/shape-template.md` as the universal template location. The P2 executor actually created the file at `workflows/_shared/templates/shape-template.md`. The context preservation rule (both canonical and workspace copies) references the actual path. P3-1 and P3-3 initially used the planned path from shape.md — corrected to match the actual file location and the CP rule's references.
...
2. **`.cursor/rules/` doesn't exist:** Plan file checkpoint prompts and some task files reference this path. Canonical source is `_config/claude/rules/`, workspace copy is `.cursor/rules/`. These appear in the plan file body (not editable) and completed task files (historical). **Not fixed — documented.**
3. **`_shared/templates/` vs `workflows/_shared/templates/`:** Plan file P2 checkpoint and P4 checkpoint reference the planned path, not the actual path. Appears in plan file body (not editable). **Not fixed — documented.**
...
| `workflows/plan-lifecycle/templates/shape-template.md` | Current shape template to generalize | p2-1 |
| `workflows/plan-lifecycle/steps-c/step-04-generate-artifacts.md` | Step to modify for merge logic | p3-1 |
| `workflows/plan-lifecycle/data/plan-creation-rules.md` | Rules to update shape.md references | p3-2 |
| `workflows/plan-lifecycle/workflow.md` | Update shapeTemplateFile path | p3-3 |
| `.gitignore` | Add `.claude/memory/` exclusion | p1-3 |
| `_config/bootstrap.py` | Ensure installer protects memory directory | p1-4 |
