...
Rename `tasks/judge.xml` to `tasks/quality-review.xml` to align task file naming with the skill/subagent id.
...
| `tasks/judge.xml` | File to rename |
...
1. Copy `tasks/judge.xml` to `tasks/quality-review.xml`.
2. Delete `tasks/judge.xml`.
...
|--------|----------|--------|
| Renamed task file | `tasks/quality-review.xml` | XML (unchanged content) |
