...
| 8 | TEA references check | Search RBTV for `tea/` refs. TEA jumped 0.1.1 → 1.5.2; internal structure likely changed. Only actionable if RBTV actually references TEA | Only breaks if RBTV has TEA path refs (unlikely but must verify) | 2 | 5 |
...
Claude Code executes this upgrade from the **RBTV repo root** (``), NOT from the parent BMAD project root. Two reference directories provide the comparison basis:
...
| Beta.6 | 2026-02-04 | Cross-file reference validator; path standardization to `{project-root}/`; removed Excalidraw workflows |
...
bmm/workflows/2-plan-workflows/create-prd/workflow.md
...
1. Check each of the three paths in `_admin/docs/BMAD-v6.0.4/`:
   - `bmm/workflows/1-analysis/create-product-brief/workflow.md` — does `workflow.md` still exist or was it renamed?
   - `bmm/workflows/2-plan-workflows/create-prd/workflow.md` — same check
   - `bmm/workflows/2-plan-workflows/create-ux-design/workflow.md` — same check
...
```
core/workflows/advanced-elicitation/workflow.xml
```
...
**Required action before upgrading:**
1. Check `_admin/docs/BMAD-v6.0.4/core/workflows/advanced-elicitation/`
...
**What RBTV does:** The installer's `add_rbtv_to_help_catalog()` function appends a row to `_config/bmad-help.csv` with these columns:
...
**Required action before upgrading:**
1. Inspect `_admin/docs/BMAD-v6.0.4/_config/bmad-help.csv`
...
### 4. `bmm/config.yaml` Field Names — MEDIUM RISK

**What RBTV does:** The installer's `normalize_bmad_output_paths()` function reads and rewrites three fields in `bmm/config.yaml`:
- `output_folder`
- `planning_artifacts`
...
**Required action before upgrading:**
1. Inspect `_admin/docs/BMAD-v6.0.4/bmm/config.yaml`
...
### 5. `_config/manifest.yaml` Format — LOW RISK

**What RBTV does:** The installer's pre-flight version check reads `_config/manifest.yaml` and expects an `installation.version` field.

**What BMAD changed:** No documented change to manifest format between Beta.4 and v6.0.4, but it should be verified.

**Required action before upgrading:**
1. Inspect `_admin/docs/BMAD-v6.0.4/_config/manifest.yaml`
...
### 6. `` Directory Structure — LOW RISK
...
**What BMAD changed:** No core structural changes to `core/` or `bmm/`. The `` naming convention (vs the earlier `.bmad/` hidden folder) was already established by Beta.4. SDET module was added in Beta.3 (but RBTV doesn't reference it).

**Required action before upgrading:**
1. Verify `core/` and `bmm/` still exist in `_admin/docs/BMAD-v6.0.4/`
...
**Required action before upgrading:**
1. Search RBTV codebase for any references to `tea/`:
   ```bash
   grep -r "tea" agents/ workflows/ tasks/ --include="*.md" --include="*.xml"
   ```
2. If references are found: compare the referenced paths between `_admin/docs/BMAD-mirror/tea/` and `_admin/docs/BMAD-v6.0.4/tea/` — the major version jump (0.1.1 → 1.5.2) likely changed internal structure
...
ls _admin/docs/BMAD-v6.0.4/bmm/workflows/1-analysis/create-product-brief/
ls _admin/docs/BMAD-v6.0.4/bmm/workflows/2-plan-workflows/create-prd/
ls _admin/docs/BMAD-v6.0.4/bmm/workflows/2-plan-workflows/create-ux-design/
ls _admin/docs/BMAD-v6.0.4/core/workflows/advanced-elicitation/

# Compare against mirror (current baseline)
ls _admin/docs/BMAD-mirror/bmm/workflows/1-analysis/create-product-brief/
ls _admin/docs/BMAD-mirror/bmm/workflows/2-plan-workflows/create-prd/
ls _admin/docs/BMAD-mirror/bmm/workflows/2-plan-workflows/create-ux-design/
ls _admin/docs/BMAD-mirror/core/workflows/advanced-elicitation/

# Check config file field names (v6.0.4)
grep -E "output_folder|planning_artifacts|implementation_artifacts" _admin/docs/BMAD-v6.0.4/bmm/config.yaml
grep "output_folder" _admin/docs/BMAD-v6.0.4/core/config.yaml

# Check manifest format (v6.0.4)
cat _admin/docs/BMAD-v6.0.4/_config/manifest.yaml

# Check output folder name (v6.0.4 vs mirror)
grep "output_folder" _admin/docs/BMAD-v6.0.4/core/config.yaml
grep "output_folder" _admin/docs/BMAD-mirror/core/config.yaml

# Check bmad-help.csv header (v6.0.4 vs mirror)
head -1 _admin/docs/BMAD-v6.0.4/_config/bmad-help.csv
head -1 _admin/docs/BMAD-mirror/_config/bmad-help.csv
```
...
cp -r _admin/docs/BMAD-mirror/rbtv /tmp/rbtv-slot-backup
...
mkdir -p _admin/docs/BMAD-mirror/rbtv
...
| TEA major version jump (0.1.1 → 1.5.2) | Medium | Medium | Search RBTV for `tea/` refs; compare TEA paths between mirror and v6.0.4 |
