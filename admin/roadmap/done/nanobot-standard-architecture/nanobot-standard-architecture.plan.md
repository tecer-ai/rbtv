...
todos:
  ...
  - id: p4-3
    ...
    status: completed
  - id: p4-4
    content: "p4-4: Migrate cross-module path references (~60 files) from {project-root}/core/ and /bmm/ patterns to {bmad_core} and {bmad_bmm} variables"
    status: completed
  - id: p4-5
...
- `p4-4`: Migrate cross-module path references (~60 files) from `{project-root}/core/` and `/bmm/` patterns to `{bmad_core}` and `{bmad_bmm}` variables *(taskFile: phase-4/p4-4.task.md)*
