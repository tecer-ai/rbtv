...
2. UPDATE CONFIG
   Run task: {project-root}/tasks/update-bmad-config.xml
...
6. RESTORE CONFIG
   Run task: {project-root}/tasks/restore-bmad-config.xml
...
BMAD workflows in the new conversation read `{project-root}/{module}/config.yaml` during agent activation (step 2 of every BMAD agent). The config fields that control file discovery are:
