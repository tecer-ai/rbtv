...
| File | Issue |
|------|-------|
| `workflows/bi-business-innovation/` | **Gap:** No backward reconciliation step. Workflow moves forward only. No mechanism to flag when a later framework contradicts an earlier one. |
...
1. **New Rule**: Post-completion cross-framework consistency gate step in the BI workflow
   ...
   - **Location:** New step file in `workflows/bi-business-innovation/steps-c/`
...
| Aspect | Details |
|--------|---------|
| File(s) to modify | Each BI framework's completion/synthesis step file (the step that sets `status: completed` and updates project-memo). These are the `step-05-synthesis.md` (or equivalent final step) files within each framework's workflow directory under `workflows/bi-business-innovation/`. |
...
| Related files | `workflows/bi-business-innovation/workflow.md` (may need a note in the workflow architecture section documenting this behaviour). `project-memo.md` template (already tracks `stepsCompleted` per milestone — the count is derivable from this). |
...
| File | Relationship |
|------|--------------|
| `workflows/bi-business-innovation/workflow.md` | Parent workflow — documents milestone/framework structure |
| `workflows/bi-business-innovation/steps-c/step-05-synthesis.md` (per framework) | Target modification — completion steps where the invitation is added |
| `core/workflows/party-mode/workflow.md` | Leveraged infrastructure — Party Mode is the review mechanism |
