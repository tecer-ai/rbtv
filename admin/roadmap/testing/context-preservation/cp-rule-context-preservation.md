...
6. **Context Preservation Rule — Detect, Discover, Confirm, Capture (SELECTED)**
   ...
   - **Location:** New rule at `_bmad/rbtv/_config/.cursor/rules/bmad-rbtv-context-preservation.mdc`
...
| Aspect | Details |
|--------|---------|
| File(s) to create | `_bmad/rbtv/_config/.cursor/rules/bmad-rbtv-context-preservation.mdc` (new rule) |
...
| Shared template location | `_bmad/rbtv/_shared/templates/shape-template.md` (universal shape template) |
...
**PRD:** `_bmad/rbtv/_admin/roadmap/todos/_claude-code-workspace/agent-architecture/prd-rbtv-memory-system.md`
...
- [ ] New rule file created at `_bmad/rbtv/_config/.cursor/rules/bmad-rbtv-context-preservation.mdc`
...
2. **`outputPath` stale** — Frontmatter says `_bmad/rbtv/_admin/roadmap/todos` but file lives at `_claude-code-workspace/context-preservation/`. Caused by post-creation reorganization. Update frontmatter to match.
