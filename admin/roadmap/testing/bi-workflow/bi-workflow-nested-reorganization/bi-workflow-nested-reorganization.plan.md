...
| Only relative and hardcoded paths change            | `../bi-mN/` and `_bmad/rbtv/workflows/bi-mN/` patterns change; variable-resolved paths stay the same                     |
...
| Master folder does not move                         | `bi-business-innovation/` stays at `_bmad/rbtv/workflows/bi-business-innovation/` — only its children change             |
...
**Working directory:** `_bmad/rbtv/workflows/`
...
> ## Work to Evaluate
>
> 27 `git mv` operations executed in `_bmad/rbtv/workflows/` to nest bi-m* folders under `bi-business-innovation/`.
>
> ## Quality Criteria
>
> 1. Run `ls _bmad/rbtv/workflows/` — no `bi-m*` folders remain at the top level (only `bi-business-innovation/` and non-bi folders)
> 2. Run `ls _bmad/rbtv/workflows/bi-business-innovation/` — contains `bi-m1/`, `bi-m2/`, `bi-m3/`, `bi-m4/`, plus the original master files (workflow.md, data/, steps-c/, templates/)
> 3. Run `ls _bmad/rbtv/workflows/bi-business-innovation/bi-m1/` — contains `bi-m1-five-whys/`, `bi-m1-lean-canvas/`, `bi-m1-competitive-landscape/`, `bi-m1-jobs-to-be-done/`, `bi-m1-problem-solution-fit/`, `bi-m1-working-backwards/`, plus hub files (workflow.md, data/, steps-c/)
...
**Files to edit (all paths relative to `_bmad/rbtv/workflows/bi-business-innovation/`):**
...
> ## Work to Evaluate
>
> Path reference updates in `_bmad/rbtv/workflows/bi-business-innovation/` master workflow files: `steps-c/step-02-milestone-select.md`, `data/founder-process.md`, and `workflow.md`.
>
> ## Quality Criteria
>
...
**Identical pattern to Phase 3, but for M2.** All paths relative to `_bmad/rbtv/workflows/bi-business-innovation/`.
...
**Identical pattern to Phase 3, but for M3.** All paths relative to `_bmad/rbtv/workflows/bi-business-innovation/`.
...
**Identical pattern to Phase 3, but for M4.** All paths relative to `_bmad/rbtv/workflows/bi-business-innovation/`.
...
- `{project-root}/_bmad/rbtv/tasks/update-bmad-config.xml` — **NO CHANGE** (task file, not a workflow)
- `{project-root}/_bmad/rbtv/tasks/restore-bmad-config.xml` — **NO CHANGE**
...
> ## Work to Evaluate
>
> Path reference updates across all 4 milestone hubs and 23 framework workflow folders in `_bmad/rbtv/workflows/bi-business-innovation/`.
>
> ## Quality Criteria
>
...
1. `**_bmad/rbtv/tasks/mentor-help.xml`** — Dynamic path construction for milestone workflows.

  | Old Pattern                                               | New Pattern                                                                      |
  | --------------------------------------------------------- | -------------------------------------------------------------------------------- |
  | `{project-root}/_bmad/rbtv/workflows/bi-m{N}/workflow.md` | `{project-root}/_bmad/rbtv/workflows/bi-business-innovation/bi-m{N}/workflow.md` |
  | `{project-root}/_bmad/rbtv/workflows/bi-m{N}/`            | `{project-root}/_bmad/rbtv/workflows/bi-business-innovation/bi-m{N}/`            |

   Note: `{project-root}/_bmad/rbtv/workflows/bi-business-innovation/data/founder-process.md` does NOT change (master folder didn't move).
2. `**_bmad/_config/bmad-help.csv`** — Verify the BI entry path. The current path `_bmad/rbtv/workflows/bi-business-innovation/workflow.md` should NOT change (master folder didn't move). Confirm only.
3. `**_bmad/rbtv/_config/bootstrap.py**` — Verify the BI registration path. The current path `_bmad/rbtv/workflows/bi-business-innovation/workflow.md` should NOT change. Confirm only.
4. `**_bmad/rbtv/agents/vivian.md**` and `**_bmad/rbtv/agents/paul.md**` — Check for any bi-m* path references and update if found.
...
> ## Work to Evaluate
>
> Path reference updates in runtime-critical files: `_bmad/rbtv/tasks/mentor-help.xml`, verification of `_bmad/_config/bmad-help.csv` and `_bmad/rbtv/_config/bootstrap.py`, and agent files.
>
> ## Quality Criteria
>
> 1. `mentor-help.xml` paths include `bi-business-innovation/` prefix before `bi-m{N}/`
> 2. `bmad-help.csv` BI entry still points to `_bmad/rbtv/workflows/bi-business-innovation/workflow.md` (unchanged)
> 3. `bootstrap.py` BI registration still points to `_bmad/rbtv/workflows/bi-business-innovation/workflow.md` (unchanged)
...
1. `**_bmad/rbtv/_admin/roadmap/`** (~62 files) — Search for `bi-m1/`, `bi-m2/`, `bi-m3/`, `bi-m4/` and prefix with `bi-business-innovation/` where they reference workflow source paths.
...
Run these grep commands from repository root (`_bmad/rbtv/`):
...
2. `rg "\.\./bi-m[1-4]/" _bmad/rbtv/workflows/bi-business-innovation/` — Verify remaining `../bi-m` references are valid (hub→framework = `../bi-mN-{name}/`, which is correct for step files going up to hub then into sibling framework). Flag any that look like the old hub-as-sibling pattern.
3. `rg "\.\./bi-business-innovation/" _bmad/rbtv/workflows/bi-business-innovation/` — Must return zero matches (old pattern where hubs referenced master as sibling).
4. `rg "bi-m[1-4]-[a-z]" _bmad/rbtv/workflows/bi-business-innovation/ --glob "*/workflow.md"` — In hub workflow.md files, framework references must use `./bi-mN-{name}/` (child), not `../bi-mN-{name}/` (old sibling).
...
> ## Quality Criteria
>
> 1. `rg "workflows/bi-m[1-4]/" _bmad/rbtv/ --glob "!.git"` — every match includes `bi-business-innovation/` prefix
> 2. `rg "\.\./bi-business-innovation/" _bmad/rbtv/workflows/bi-business-innovation/` — returns zero matches
> 3. No `{bmad_rbtv}`, `{bmad_core}`, `{bmad_bmm}`, `{bmad_output}`, `{outputFolder}`, `{projectMemo}` variable paths were modified anywhere
> 4. `ls _bmad/rbtv/workflows/` shows no `bi-m`* folders at the top level
