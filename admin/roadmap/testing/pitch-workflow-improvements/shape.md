...
The "Files to Load During Execution" table lists `_bmad/rbtv/workflows/pitch/data/html-patterns.md` and `_bmad/rbtv/workflows/pitch/data/html-components.md` for task p1-1. The actual file paths are `_bmad/rbtv/workflows/_shared/pitch-data/html-patterns.md` and `_bmad/rbtv/workflows/_shared/pitch-data/html-components.md`. The compound doc and step-07 frontmatter both reference the correct `_shared/pitch-data/` path. Non-blocking — the task file's Context Files table also has the correct compound doc path which contains the correct paths.
...
|----------|----------------------|
| `_bmad/rbtv/_admin/roadmap/todos/_claude-code-workspace/pitch-workflow/cp-workflow-pitch-asset-handling-and-layout-overflow.md` | Logo patterns, background image constraints, design constraint rows 13–16, step-07 verification rows |
| `_bmad/rbtv/_admin/roadmap/todos/_claude-code-workspace/pitch-workflow/cp-workflow-pitch-output-and-pdf-validation.md` | Folder conventions (client/fundraising), step-10 PDF validation with Decktape + Playwright, step-01 output path prompt |
| `_bmad/rbtv/_admin/roadmap/todos/_claude-code-workspace/pitch-workflow/cp-workflow-pitch-narrative-sync.md` | Agent sync rules (leo, roelof, vivian), edit step sync enforcement, step-07 post-gen check, CSS-only exemption |

### Files to Load During Execution

| File | Purpose | When |
|------|---------|------|
| `_bmad/rbtv/workflows/_shared/pitch-data/html-patterns.md` | Target file for pattern additions | p1-1 |
| `_bmad/rbtv/workflows/_shared/pitch-data/html-components.md` | Reference for component structure | p1-1 |
| `_bmad/rbtv/workflows/pitch/steps-c/step-07-generate.md` | Target for verification rows and sync check | p1-2, p3-5 |
| `_bmad/rbtv/workflows/pitch/steps-c/step-01-init.md` | Target for output path prompt | p2-2 |
| `_bmad/rbtv/workflows/pitch/steps-c/step-09-synthesis.md` | Chain to step-10 | p2-4 |
| `_bmad/rbtv/workflows/pitch/workflow.md` | Register step-10 | p2-5 |
| `_bmad/rbtv/agents/leo.md` | Target for sync rule | p3-1 |
| `_bmad/rbtv/agents/roelof.md` | Target for sync rule | p3-1 |
| `_bmad/rbtv/agents/vivian.md` | Target for sync rule + INPUT update | p3-2 |
| `_bmad/rbtv/workflows/pitch/steps-e/step-e01-load.md` | Target for narrative loading | p3-3 |
| `_bmad/rbtv/workflows/pitch/steps-e/step-e02-edit.md` | Target for sync enforcement | p3-4 |
