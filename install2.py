...
D4  HARNESSES — all three by default (claude, codex, opencode); `--harness`
    filters. The `kimi` harness is RETIRED (owner ruling 2026-08-21, matching
    the team-kit retirement of 2026-08-18): kimi-code-cli is leaving this
    system and kimi models are reached through opencode instead. A run naming
    it is refused as an unknown harness. Nothing kimi-exclusive was ever
    written — its MATRIX column pointed at claude's and codex's own files — so
    no artifact is orphaned by the removal, and a stale `kimi` in an older
    state book is simply filtered out (every read intersects HARNESSES).
...
D8  `agents.md` AND THE GUIDANCE SURFACE (rewritten 2026-08-10 to conform to
    ...
    auto-inject no rule folder — of D4's harnesses, Codex ONLY (CMP-12 also
    lists Qwen and Kimi there; neither is selectable here). It is emitted into a
...
# auto-injects) and not opencode (reads `.claude/` natively). Qwen and kimi are
# on this list in CMP-12 and absent from D4's harnesses.
...
# that `materialize-seats.py` carries. A KIMI column was measured on this box
# and is GONE with D4's retirement of that harness — it never carried a path of
# its own (skill/rule reused claude's and codex's files, command/sub-agent were
# None), so removing it changes nothing on disk.
...
def resolve_basis(stored, override: str | None) -> str | None:
    ...
    `claude.md`, `Claude.md` and `NONE` all land where the user meant. Only the
    canonical form is ever stored or compared downstream."""
    value = override if override is not None else stored
    if value is None or value == "" or value.casefold() == BASIS_NONE:
        return None
    value = {n.casefold(): n for n in GUIDANCE_NAMES}.get(value.casefold(),
                                                         value)
...
def plan_files(records: dict[str, dict], catalog: dict[str, dict]
    ...
    if hooks:
        ...
        # opencode has no hooks surface — nothing is minted for it here.
...
def selftest() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        h3, r3 = _mk("ws-two-share", ["codex", "opencode"],
                     "CLAUDE.md")
        check("H3 — two harnesses sharing AGENTS.md get ONE file per folder",
        ...
        answers = ["CLUADE.md", "CLAUDE.mdx", "CLAUDE.md"]
        asked: list[str] = []
        ...
        try:
            prompt_basis(ask=lambda _p: "CLUADE.md", tries=1)
            single = "no refusal"
        except Refuse as exc:
            single = exc.code
        ...
        try:
            resolve_basis(None, "CLUADE.md")
            direct = "no refusal"
        except Refuse as exc:
            direct = exc.code
        check("7.623b — NON-interactive resolve_basis is unchanged: no retry "
              "exists there", direct == "guidance-basis-invalid", direct)
        check("7.623b — CASE is not a typo: any spelling of a known basis (or "
              "of `none`) resolves to the CANONICAL one",
              prompt_basis(ask=lambda _p: "claude.md", tries=1) == "CLAUDE.md"
              and resolve_basis(None, "agents.MD") == "AGENTS.md"
              and resolve_basis(None, "NONE") is None)
        ...
        if verb == "install":
            s.add_argument("--harness", default=",".join(HARNESSES),
                           help="comma-separated subset of "
                                + ",".join(HARNESSES))
            # No argparse `choices`: it compares case-SENSITIVELY, and a basis
            # is matched case-insensitively (`resolve_basis`). The type hook
            # canonicalizes here so the stored value is always canonical, and
            # `resolve_basis` still refuses anything that is not a basis.
            s.add_argument("--guidance-basis", default=None,
                           type=lambda v: {n.casefold(): n for n in
                                           (*GUIDANCE_NAMES, BASIS_NONE)
                                           }.get(v.casefold(), v),
                           metavar="{%s}" % ",".join((*GUIDANCE_NAMES,
                                                      BASIS_NONE)),
