...
def do_scan(catalog: dict[str, dict], shadowed: list[dict]) -> dict:
    ...
    return {"ok": True, "components": entries, "shadowed": shadowed,
            "no_manifest": [e["id"] for e in entries
                            if e["note"] == "component has no exposure manifest"],
...
def _fixture(root: Path) -> None:
    ...
    (dup / COMPONENT_NAME).write_text("# dupcomp\n", encoding="utf-8")
...
def selftest() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        check("it is INSTALLABLE despite having no manifest, and says so",
              ...
              and "_hub/skills/vendored" not in data["no_manifest"]
              and [e for e in data["components"]
                   if e["id"] == "_hub/skills/vendored"][0]["methods"]
              == ["skill"], str(data["no_manifest"]))
        ...
        for method, cid in expect_hub.items():
            ...
            check(f"H-discover-{method} — id {cid}",
                  ...
                  str(rec))
        check("H-discover-path-file — suffix kept, still D9 (mints nothing)",
        ...
        check("H-realize-config — MCP key claimed",
              ...
              .get("mcpServers", {}))
        check("H-realize-path — catalogued, D9 mints nothing",
              any(r["method"] == "path" and r["part"] == "hubbin.py"
        ...
        four = ["claude", "codex", "opencode", "kimi"]
        ...
        rst = read_state(rw)
        check("H-rewrite — keys move, module becomes _hub, harnesses intact",
              set(rst["components"])
              == {"_hub/skills/vendored", "_hub/skills/hubskill"}
              and rst["components"]["_hub/skills/vendored"]["module"] == HUB_DIR
              and rst["components"]["_hub/skills/vendored"]["harnesses"] == four
        ...
        if live_book.is_file():
            ...
            skill_harnesses = {
                ...
                if cid.startswith(f"{SKILLS_DIR}/")}
            rewrite_legacy_skill_ids(raw)
            old_ids = set(raw["components"])
            old_files = {cid: list(rec.get("files") or [])
                         for cid, rec in raw["components"].items()}
            ...
            check("U-live still-present cids materialise FULL parts + keep files",
                  all("parts" in got["components"][cid]
                      and "files" in got["components"][cid]
                      and set(got["components"][cid]["parts"])
                      == {r["id"] for r in catalog_parts_map(live_cat)[cid]}
                      for cid in present),
                  str(present[:5]))
            check("U-live shared_claims unchanged",
                  got["shared_claims"] == old_claims)
            check("U-live known_files dual-read equals v1 rec.files ∪ guidance",
                  known_files(got) == set(sum(old_files.values(), []))
                  | set(raw.get("guidance_files") or []))
            four = ["claude", "codex", "opencode", "kimi"]
            three = ["claude", "codex", "opencode"]
            rewritten = {
                f"{HUB_DIR}/{HUB_ID_FOLDER['skill']}/{old.split('/', 1)[1]}": hs
                for old, hs in skill_harnesses.items()}
            check("U-live R6 — 7 _skills keys rewritten, harnesses intact",
                  len(rewritten) == 7
                  and all(cid in got["components"] for cid in rewritten)
                  and sum(1 for hs in rewritten.values() if hs == four) == 2
                  and sum(1 for hs in rewritten.values() if hs == three) == 5
                  and all(got["components"][cid]["harnesses"] == hs
                          and got["components"][cid].get("module") == HUB_DIR
                          for cid, hs in rewritten.items()),
