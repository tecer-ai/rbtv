...
D8  `agents.md` AND THE ROOT GUIDANCE SURFACE — this installer NEVER writes the
    target's `CLAUDE.md` / `AGENTS.md`. Those are hand-authored or owned by the
    old installer's mirror driver. Instead every `agents.md` row and every
    `rule` copy is named in ONE installer-owned, prefix-named file,
...
# `path` (d-tool-inventory-exposure-rows). A method outside this set refuses.
CANONICAL_METHODS = (
...
def plan_files(records: dict[str, dict], catalog: dict[str, dict]
    ...
    report: dict = {"skipped_path_rows": [], "no_realization": []}
    ...
    for cid in sorted(records):
        ...
        for row in exposure_rows(comp):
            ...
            if method in INVENTORY_METHODS:
                ...
                report["skipped_path_rows"].append(
                    {"component": cid, "part": pid, "entry_point": entry_rel})
                continue
            if not entry_rel:
                raise Refuse(
                    "entry-point-missing",
...
def do_install(target: Path, catalog: dict[str, dict], picked: list[str],
               harnesses: list[str], dry_run: bool) -> dict:
...
def print_result(data: dict) -> None:
    ...
    for row in report.get("skipped_path_rows") or []:
        print(f"  · skipped `path` row {row['component']}/{row['part']} "
              f"({row['entry_point']}) — tool inventory, mints nothing")
...
def selftest() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        check("`path` row minted nothing",
              not (target / f".claude/skills/{PREFIX}fixtool").exists()
              and len(res["report"]["skipped_path_rows"]) == 1)
