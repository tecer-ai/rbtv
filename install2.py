...
def plan_mirror(target: Path, basis: str | None) -> dict[str, str]:
    ...
    try:
        ...
        raise Refuse(
            ...
            f"the guidance basis {basis!r} is not readable as UTF-8 text "
            f"({exc}) — a mirror of it would be garbage; refusing before any "
            f"write. Turn the mirror off with `--guidance-basis {BASIS_NONE}` "
            "if this file is not meant to be guidance",
            str(source)) from exc
...
def apply(target: Path, files: dict[str, str], claims: list[dict], state: dict,
    ...
    if collisions:
        ...
        mirrors = [rel for rel in collisions if rel in GUIDANCE_MIRROR]
        if mirrors:
            raise Refuse(
                "guidance-mirror-collision",
                f"{', '.join(mirrors)} already exists at the install root and "
...
def _add_mirror(target: Path, state: dict, files: dict, owners: dict,
                report: dict, override: str | None) -> frozenset[str]:
    ...
    Returns the paths `apply` must never delete: the CURRENT basis. The book can
    hold that same name from an earlier run when it was the generated mirror
    (the flip: yesterday's mirror is today's hand-authored basis), and deleting
    it as stale would destroy user-authored guidance. The book itself is
    corrected by `_rebook`, which recomputes `guidance_files` from the new plan.
    """
    ...
    return frozenset([basis]) if basis else frozenset()
...
def do_uninstall(target: Path, catalog: dict[str, dict], picked: list[str],
    ...
    if records:
        ...
        try:
            ...
            # BOTH root guidance files off the delete set — un-managed on disk
            # beats deleted, and the next install re-books whatever is real.
            protect = frozenset(GUIDANCE_MIRROR)
