...
| O9 | **`admin/roadmap/` archive files cause false-positive in Task 18 Step 6 grep** — The archive contains historical task files with `{bmad_output}` and `_bmad/rbtv` references. Task 18 Step 6 greps the entire `H:/BMAD/_bmad/rbtv` expecting zero matches. Archive files make it fail. | LOW | **(a)** Add `--exclude-dir=admin/roadmap` to the verification grep in Task 18 Step 6. **(b)** Also rewrite archive files (not necessary, they don't execute). | **(a)** (2026-04-17) — Add `--exclude-dir=admin/roadmap` to Task 18 Step 6's verification grep. |
...
| Ana-nested workflows | `doc-compound-learning`, `doc-context-handoff`, `add-prompting-knowledge` |
...
| Old path | New path |
|---|---|
| `_bmad/rbtv/_config/claude/skills/bmad-rbtv-<name>/` | `rbtv/skills/<name>/` |
| `_bmad/rbtv/_config/claude/commands/` | `rbtv/commands/` (NEW — no existing commands) |
| `_bmad/rbtv/_config/claude/rules/bmad-rbtv-<name>.md` | `rbtv/rules/<name>.md` |
| `_bmad/rbtv/_config/claude/agents/bmad-rbtv-<name>.md` | `rbtv/subagents/<name>.md` (designer, web-research; others dropped) |
| `_bmad/rbtv/_config/bootstrap.py` | `rbtv/install.py` (rewritten) |
| `_bmad/rbtv/_config/bootstrap/` | `rbtv/admin/install/installer/` |
| `_bmad/rbtv/_config/config.yaml` | `rbtv/admin/install/defaults.yaml` (rewritten) |
| `_bmad/rbtv/_admin/` | `rbtv/admin/` (no underscore, contents reorganized) |
| `_bmad/rbtv/agents/<name>.md` | `rbtv/agents/<name>/<name>.md` |
| `_bmad/rbtv/workflows/<wf>/` (single-agent-owned) | `rbtv/agents/<agent>/workflows/<wf>/` |
| `_bmad/rbtv/workflows/<wf>/` (shared) | `rbtv/workflows/<wf>/` |
| `_bmad/rbtv/tasks/<name>.xml` (single-agent-owned) | `rbtv/agents/<agent>/tasks/<name>.xml` |
| `_bmad/rbtv/tasks/<name>.xml` (shared) | `rbtv/tasks/<name>.xml` |
...
|---|---|---|
| `{project-root}/_bmad/rbtv/` | `{rbtv_path}/` | Absolute path to RBTV repo in target |
...
```bash
git -C "H:/BMAD/_bmad/rbtv" checkout -b feat/rbtv-standalone
git -C "H:/BMAD/_bmad/rbtv" tag rbtv-standalone-phase-0-start
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" status
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" push -u origin feat/rbtv-standalone
git -C "H:/BMAD/_bmad/rbtv" push origin rbtv-standalone-phase-0-start
```
...
**Files:**
- Move: `_bmad/rbtv/_config/claude/skills/` → `_bmad/rbtv/skills/`
- Move: `_bmad/rbtv/_config/claude/rules/` → `_bmad/rbtv/rules/`
- Move: `_bmad/rbtv/_config/config.yaml` → `_bmad/rbtv/admin/install/defaults.yaml` (deferred rewrite to Task 15)
- Move: `_bmad/rbtv/_config/bootstrap/` → `_bmad/rbtv/admin/install/installer/` (deferred rewrite to Task 18)
- Move: `_bmad/rbtv/_config/bootstrap.py` → `_bmad/rbtv/install.py` (deferred rewrite to Task 19)

- [ ] **Step 1: Create destination directories**

```bash
mkdir -p "H:/BMAD/_bmad/rbtv/admin/install"
```

- [ ] **Step 2: Tag phase start**

```bash
git -C "H:/BMAD/_bmad/rbtv" tag rbtv-standalone-phase-1-start
```

- [ ] **Step 3: Move skills directory**

```bash
git -C "H:/BMAD/_bmad/rbtv" mv _config/claude/skills skills
```

- [ ] **Step 4: Move rules directory**

```bash
git -C "H:/BMAD/_bmad/rbtv" mv _config/claude/rules rules
```

- [ ] **Step 5: Create commands directory (empty, files added in later tasks)**

```bash
mkdir -p "H:/BMAD/_bmad/rbtv/commands"
```

- [ ] **Step 6: Move config.yaml to admin/install/defaults.yaml**

```bash
git -C "H:/BMAD/_bmad/rbtv" mv _config/config.yaml admin/install/defaults.yaml
```

- [ ] **Step 7: Move bootstrap package**

```bash
git -C "H:/BMAD/_bmad/rbtv" mv _config/bootstrap admin/install/installer
```

- [ ] **Step 8: Move bootstrap.py to root as install.py**

```bash
git -C "H:/BMAD/_bmad/rbtv" mv _config/bootstrap.py install.py
```

- [ ] **Step 9: Remove now-empty _config directory**

```bash
rm -rf "H:/BMAD/_bmad/rbtv/_config"
```

- [ ] **Step 10: Commit**

```bash
git -C "H:/BMAD/_bmad/rbtv" add -A
git -C "H:/BMAD/_bmad/rbtv" commit -m "refactor: flatten _config wrapper — skills/rules/commands at root, install in admin/install/"
```
...
**Files:**
- Move: `_bmad/rbtv/_admin/` → `_bmad/rbtv/admin/` (merging with existing `admin/install/` created in Task 1)
...
```bash
git -C "H:/BMAD/_bmad/rbtv" mv _admin/roadmap.md admin/roadmap.md 2>/dev/null || echo "roadmap.md not present, skipping"
```

Use this pattern for any top-level files in `_admin/` (roadmap.md, vision.md, or similar). Check with:

```bash
ls "H:/BMAD/_bmad/rbtv/_admin"
```

For each top-level `.md` file, move it:

```bash
git -C "H:/BMAD/_bmad/rbtv" mv _admin/<filename> admin/<filename>
```

- [ ] **Step 2: Move _admin/claude to admin/claude (temporary — processed in Task 3)**

```bash
git -C "H:/BMAD/_bmad/rbtv" mv _admin/claude admin/claude
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" mv _admin/<subdir> admin/<subdir>
```

- [ ] **Step 4: Remove now-empty _admin/**

```bash
rmdir "H:/BMAD/_bmad/rbtv/_admin"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add -A
git -C "H:/BMAD/_bmad/rbtv" commit -m "refactor: rename _admin to admin (Python-importable)"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" mv admin/claude/rules/admin-rbtv-config-source.md rules/source-of-truth.md
```
...
Body should retain the original content but with path references updated: `{rbtv_path}/` replaces any `{bmad_rbtv}/` or `{project-root}/_bmad/rbtv/` patterns.
...
```bash
ls "H:/BMAD/_bmad/rbtv/admin/claude/rules/admin-rbtv-component-patterns.md"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add rules/source-of-truth.md
git -C "H:/BMAD/_bmad/rbtv" commit -m "refactor(rules): extract source-of-truth rule from _admin/claude/"
```
...
```bash
mkdir -p "H:/BMAD/_bmad/rbtv/subagents"
```

- [ ] **Step 2: Move and rename the two kept subagents**

```bash
git -C "H:/BMAD/_bmad/rbtv" mv admin/claude/agents/bmad-rbtv-designer.md subagents/designer.md
git -C "H:/BMAD/_bmad/rbtv" mv admin/claude/agents/bmad-rbtv-web-research.md subagents/web-research.md
```

- [ ] **Step 3: Delete the two dropped subagents**

```bash
git -C "H:/BMAD/_bmad/rbtv" rm admin/claude/agents/bmad-rbtv-quality-review.md
git -C "H:/BMAD/_bmad/rbtv" rm admin/claude/agents/bmad-rbtv-context-distill.md
```
...
```bash
ls "H:/BMAD/_bmad/rbtv/admin/claude/agents"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add -A
git -C "H:/BMAD/_bmad/rbtv" commit -m "refactor(subagents): migrate dispatchable subagents to rbtv/subagents/, drop context-distill and quality-review"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" tag rbtv-standalone-phase-2-start
```

- [ ] **Step 2: Create subdirectories**

```bash
mkdir -p "H:/BMAD/_bmad/rbtv/agents/ana" "H:/BMAD/_bmad/rbtv/agents/domcobb" "H:/BMAD/_bmad/rbtv/agents/fernando" "H:/BMAD/_bmad/rbtv/agents/george-orwell" "H:/BMAD/_bmad/rbtv/agents/paul" "H:/BMAD/_bmad/rbtv/agents/leo" "H:/BMAD/_bmad/rbtv/agents/roelof" "H:/BMAD/_bmad/rbtv/agents/vivian"
```

- [ ] **Step 3: Move each agent file**

```bash
git -C "H:/BMAD/_bmad/rbtv" mv agents/ana.md agents/ana/ana.md
git -C "H:/BMAD/_bmad/rbtv" mv agents/domcobb.md agents/domcobb/domcobb.md
git -C "H:/BMAD/_bmad/rbtv" mv agents/fernando.md agents/fernando/fernando.md
git -C "H:/BMAD/_bmad/rbtv" mv agents/george-orwell.md agents/george-orwell/george-orwell.md
git -C "H:/BMAD/_bmad/rbtv" mv agents/paul.md agents/paul/paul.md
git -C "H:/BMAD/_bmad/rbtv" mv agents/leo.md agents/leo/leo.md
git -C "H:/BMAD/_bmad/rbtv" mv agents/roelof.md agents/roelof/roelof.md
git -C "H:/BMAD/_bmad/rbtv" mv agents/vivian.md agents/vivian/vivian.md
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add -A
git -C "H:/BMAD/_bmad/rbtv" commit -m "refactor(agents): nest each agent into its own directory"
```
...
**Files:**
- Move: `workflows/doc-compound-learning/` → `agents/ana/workflows/doc-compound-learning/`
- Move: `workflows/doc-context-handoff/` → `agents/ana/workflows/doc-context-handoff/`
...
```bash
mkdir -p "H:/BMAD/_bmad/rbtv/agents/ana/workflows"
```

- [ ] **Step 2: Move workflows**

```bash
git -C "H:/BMAD/_bmad/rbtv" mv workflows/doc-compound-learning agents/ana/workflows/doc-compound-learning
git -C "H:/BMAD/_bmad/rbtv" mv workflows/doc-context-handoff agents/ana/workflows/doc-context-handoff
git -C "H:/BMAD/_bmad/rbtv" mv workflows/add-prompting-knowledge agents/ana/workflows/add-prompting-knowledge
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add -A
git -C "H:/BMAD/_bmad/rbtv" commit -m "refactor(agents/ana): nest doc workflows under ana"
```
...
```bash
mkdir -p "H:/BMAD/_bmad/rbtv/agents/domcobb/workflows"
```

- [ ] **Step 2: Move + rename**

```bash
git -C "H:/BMAD/_bmad/rbtv" mv workflows/problem-structuring agents/domcobb/workflows/problem-structuring
git -C "H:/BMAD/_bmad/rbtv" mv workflows/ps-lite agents/domcobb/workflows/problem-structuring-lite
git -C "H:/BMAD/_bmad/rbtv" mv workflows/prompting-assistance agents/domcobb/workflows/prompting-assistance
git -C "H:/BMAD/_bmad/rbtv" mv workflows/ai-web-project agents/domcobb/workflows/ai-web-project
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add -A
git -C "H:/BMAD/_bmad/rbtv" commit -m "refactor(agents/domcobb): nest problem-structuring workflows, rename ps-lite → problem-structuring-lite"
```
...
```bash
mkdir -p "H:/BMAD/_bmad/rbtv/agents/fernando/workflows"
```

- [ ] **Step 2: Move + rename the workflow**

```bash
git -C "H:/BMAD/_bmad/rbtv" mv workflows/build-rbtv-component agents/fernando/workflows/create-component
```

- [ ] **Step 3: Move the component-patterns file into workflow's data/**

```bash
mkdir -p "H:/BMAD/_bmad/rbtv/agents/fernando/workflows/create-component/data"
git -C "H:/BMAD/_bmad/rbtv" mv admin/claude/rules/admin-rbtv-component-patterns.md agents/fernando/workflows/create-component/data/component-patterns.md
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add -A
git -C "H:/BMAD/_bmad/rbtv" commit -m "refactor(agents/fernando): nest create-component workflow, promote component-patterns to workflow data"
```
...
```bash
mkdir -p "H:/BMAD/_bmad/rbtv/agents/george-orwell/workflows"
mkdir -p "H:/BMAD/_bmad/rbtv/agents/george-orwell/tasks"
```

- [ ] **Step 2: Move + rename workflow**

```bash
git -C "H:/BMAD/_bmad/rbtv" mv workflows/essay agents/george-orwell/workflows/writing
```

- [ ] **Step 3: Move task**

```bash
git -C "H:/BMAD/_bmad/rbtv" mv tasks/critical-essay-review.xml agents/george-orwell/tasks/critical-essay-review.xml
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add -A
git -C "H:/BMAD/_bmad/rbtv" commit -m "refactor(agents/george-orwell): nest writing workflow (renamed from essay) and critical-essay-review task"
```
...
```bash
mkdir -p "H:/BMAD/_bmad/rbtv/agents/paul/workflows"
mkdir -p "H:/BMAD/_bmad/rbtv/agents/paul/tasks"
```

- [ ] **Step 2: Move + rename workflow**

```bash
git -C "H:/BMAD/_bmad/rbtv" mv workflows/bi-business-innovation agents/paul/workflows/business-innovation
```

- [ ] **Step 3: Move task**

```bash
git -C "H:/BMAD/_bmad/rbtv" mv tasks/mentor-help.xml agents/paul/tasks/mentor-help.xml
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add -A
git -C "H:/BMAD/_bmad/rbtv" commit -m "refactor(agents/paul): nest business-innovation workflow (renamed from bi-business-innovation) and mentor-help task"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" mv workflows/plan-lifecycle workflows/planning
git -C "H:/BMAD/_bmad/rbtv" mv workflows/pre-product-discovery workflows/product-discovery
git -C "H:/BMAD/_bmad/rbtv" mv workflows/design-token-extraction workflows/design-extraction
```
...
```bash
ls "H:/BMAD/_bmad/rbtv/workflows"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add -A
git -C "H:/BMAD/_bmad/rbtv" commit -m "refactor(workflows): rename shared workflows — planning, product-discovery, design-extraction"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" tag rbtv-standalone-phase-3-start
```

- [ ] **Step 2: List current bmad-rbtv-* skill directories**

```bash
ls "H:/BMAD/_bmad/rbtv/skills"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" mv skills/bmad-rbtv-plan skills/planning
git -C "H:/BMAD/_bmad/rbtv" mv skills/bmad-rbtv-help skills/help
git -C "H:/BMAD/_bmad/rbtv" mv skills/bmad-rbtv-quality-review skills/quality-review
git -C "H:/BMAD/_bmad/rbtv" mv skills/bmad-rbtv-tone-extraction skills/tone-extraction
git -C "H:/BMAD/_bmad/rbtv" mv skills/bmad-rbtv-visual-design-extraction skills/design-extraction
git -C "H:/BMAD/_bmad/rbtv" mv skills/bmad-rbtv-web-research skills/web-research
git -C "H:/BMAD/_bmad/rbtv" mv skills/bmad-rbtv-reorganize-memory skills/reorganize-memory
git -C "H:/BMAD/_bmad/rbtv" mv skills/bmad-rbtv-client-pitch skills/client-pitch
git -C "H:/BMAD/_bmad/rbtv" mv skills/bmad-rbtv-create-component skills/create-component
git -C "H:/BMAD/_bmad/rbtv" mv skills/bmad-rbtv-designer skills/designer
git -C "H:/BMAD/_bmad/rbtv" mv skills/bmad-rbtv-doc skills/doc
git -C "H:/BMAD/_bmad/rbtv" mv skills/bmad-rbtv-domcobb skills/domcobb
git -C "H:/BMAD/_bmad/rbtv" mv skills/bmad-rbtv-essay skills/writing
git -C "H:/BMAD/_bmad/rbtv" mv skills/bmad-rbtv-investor-pitch skills/investor-pitch
git -C "H:/BMAD/_bmad/rbtv" mv skills/bmad-rbtv-meeting-summarizer skills/meeting-summarizer
git -C "H:/BMAD/_bmad/rbtv" mv skills/bmad-rbtv-mentor skills/mentor
git -C "H:/BMAD/_bmad/rbtv" mv skills/bmad-rbtv-product-discovery skills/product-discovery
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add -A
git -C "H:/BMAD/_bmad/rbtv" commit -m "refactor(skills): strip bmad-rbtv prefix, rename plan→planning, essay→writing, visual-design-extraction→design-extraction"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" mv rules/bmad-rbtv-atomic-files.md rules/atomic-files.md
git -C "H:/BMAD/_bmad/rbtv" mv rules/bmad-rbtv-audio-aware.md rules/audio-aware.md
git -C "H:/BMAD/_bmad/rbtv" mv rules/bmad-rbtv-background-agents.md rules/background-agents.md
git -C "H:/BMAD/_bmad/rbtv" mv rules/bmad-rbtv-bash-patterns.md rules/bash-patterns.md
git -C "H:/BMAD/_bmad/rbtv" mv rules/bmad-rbtv-chat-discipline.md rules/chat-discipline.md
git -C "H:/BMAD/_bmad/rbtv" mv rules/bmad-rbtv-context-preservation.md rules/context-preservation.md
git -C "H:/BMAD/_bmad/rbtv" mv rules/bmad-rbtv-git-file-ops.md rules/git-file-ops.md
git -C "H:/BMAD/_bmad/rbtv" mv rules/bmad-rbtv-memory-system.md rules/memory-system.md
git -C "H:/BMAD/_bmad/rbtv" mv rules/bmad-rbtv-reasoning.md rules/reasoning.md
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add -A
git -C "H:/BMAD/_bmad/rbtv" commit -m "refactor(rules): strip bmad-rbtv prefix from all rule files"
```
...
```bash
grep -rn "bmad-rbtv-" "H:/BMAD/_bmad/rbtv"
```
...
```bash
grep -rn "workflows/plan-lifecycle\|workflows/essay\|workflows/build-rbtv-component\|workflows/bi-business-innovation\|workflows/pre-product-discovery\|workflows/design-token-extraction\|workflows/problem-structuring\|workflows/ps-lite\|workflows/prompting-assistance\|workflows/ai-web-project\|workflows/doc-compound-learning\|workflows/doc-context-handoff\|workflows/add-prompting-knowledge" "H:/BMAD/_bmad/rbtv"
```
...
| `workflows/doc-compound-learning/` | `agents/ana/workflows/doc-compound-learning/` |
| `workflows/doc-context-handoff/` | `agents/ana/workflows/doc-context-handoff/` |
| `workflows/add-prompting-knowledge/` | `agents/ana/workflows/add-prompting-knowledge/` |

- [ ] **Step 3: Fix agent path references**

```bash
grep -rn "agents/ana\.md\|agents/domcobb\.md\|agents/fernando\.md\|agents/george-orwell\.md\|agents/paul\.md\|agents/leo\.md\|agents/roelof\.md\|agents/vivian\.md" "H:/BMAD/_bmad/rbtv"
```
...
```bash
grep -rn "tasks/critical-essay-review\.xml\|tasks/mentor-help\.xml" "H:/BMAD/_bmad/rbtv"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add -A
git -C "H:/BMAD/_bmad/rbtv" commit -m "refactor: fix internal path references after rename and nesting"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" tag rbtv-standalone-phase-4-start
```

- [ ] **Step 2: Search for BMAD module references**

```bash
grep -rn "bmad_core\|bmad_bmm\|bmad_cis\|_bmad/core\|_bmad/bmm\|_bmad/cis" "H:/BMAD/_bmad/rbtv"
```
...
```bash
grep -rn "bmad_output" "H:/BMAD/_bmad/rbtv"
```
...
```bash
grep -rn "{project-root}/_bmad/rbtv" "H:/BMAD/_bmad/rbtv"
```
...
```bash
grep -rn "\[AE\]\|Advanced Elicitation" "H:/BMAD/_bmad/rbtv"
```
...
```bash
grep -rn "\[PM\]\|\[P\] Party\|Party Mode" "H:/BMAD/_bmad/rbtv"
```
...
```bash
grep -rn "Advanced Elicitation\|Party Mode" "H:/BMAD/_bmad/rbtv"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add -A
git -C "H:/BMAD/_bmad/rbtv" commit -m "refactor: remove Advanced Elicitation and Party Mode menu items (BMAD dependencies)"
```
...
```bash
cat "H:/BMAD/_bmad/rbtv/agents/ana/ana.md"
```
...
```bash
grep -n "bmad_bmm\|_bmad/bmm" "H:/BMAD/_bmad/rbtv/agents/ana/ana.md"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add -A
git -C "H:/BMAD/_bmad/rbtv" commit -m "refactor(agents/ana): replace BMM references with plugin invocations"
```
...
```bash
grep -n "bmad_cis\|_bmad/cis" "H:/BMAD/_bmad/rbtv/agents/domcobb/domcobb.md"
```
...
```markdown
| [PV] | Problem Solving | Load `{project-root}/_bmad/cis/workflows/problem-solving/workflow.yaml` |
```
...
```bash
grep -rn "bmad_cis\|_bmad/cis" "H:/BMAD/_bmad/rbtv"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add -A
git -C "H:/BMAD/_bmad/rbtv" commit -m "refactor(agents/domcobb): replace CIS reference with plugin invocation"
```
...
| Find | Replace |
|---|---|
| `{project-root}/_bmad/rbtv/workflows/` | `{rbtv_path}/workflows/` |
| `{project-root}/_bmad/rbtv/agents/` | `{rbtv_path}/agents/` |
| `{project-root}/_bmad/rbtv/tasks/` | `{rbtv_path}/tasks/` |
| `{project-root}/_bmad/rbtv/` | `{rbtv_path}/` |
...
```bash
grep -rln "^main_config:\|^advancedElicitationTask:\|^partyModeWorkflow:\|^parentWorkflow:\|^validateWorkflow:" "H:/BMAD/_bmad/rbtv"
```
...
```bash
grep -rn "bmad_core\|bmad_bmm\|_bmad/core\|_bmad/bmm" "H:/BMAD/_bmad/rbtv"
```
...
```bash
grep -rn "bmad_core\|bmad_bmm\|bmad_cis\|bmad_rbtv\|bmad_output\|{project-root}/_bmad\|main_config:\|advancedElicitationTask:\|partyModeWorkflow:" "H:/BMAD/_bmad/rbtv"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add -A
git -C "H:/BMAD/_bmad/rbtv" commit -m "refactor: replace path variables with standalone placeholders, remove BMAD config loading"
```
...
```bash
cat "H:/BMAD/_bmad/rbtv/skills/meeting-summarizer/SKILL.md"
```
...
```bash
ls "H:/BMAD/_bmad/rbtv/workflows/meeting-summarizer/"
```
...
```bash
grep -n "bmad\|project-root\|_bmad" "H:/BMAD/_bmad/rbtv/workflows/meeting-summarizer/universal-prompt.md"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add workflows/meeting-summarizer/workflow.md
git -C "H:/BMAD/_bmad/rbtv" commit -m "feat(workflows): transform meeting-summarizer into BMAD architecture (workflow + thin-loader skill)"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" tag rbtv-standalone-phase-5-start
```

- [ ] **Step 2: Delete dropped skill directories**

```bash
git -C "H:/BMAD/_bmad/rbtv" rm -r skills/help
git -C "H:/BMAD/_bmad/rbtv" rm -r skills/quality-review
git -C "H:/BMAD/_bmad/rbtv" rm -r skills/reorganize-memory
```

- [ ] **Step 3: Commit**

```bash
git -C "H:/BMAD/_bmad/rbtv" commit -m "feat: drop skills help, quality-review, reorganize-memory (low usage or Claude Code native)"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" rm rules/memory-system.md
```

- [ ] **Step 2: Commit**

```bash
git -C "H:/BMAD/_bmad/rbtv" commit -m "feat: drop memory-system rule (replaced by Claude Code auto-memory)"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" rm tasks/help.xml
git -C "H:/BMAD/_bmad/rbtv" rm tasks/quality-review.xml
git -C "H:/BMAD/_bmad/rbtv" rm tasks/context-distill.xml
git -C "H:/BMAD/_bmad/rbtv" rm tasks/check-bmad-compat.xml
git -C "H:/BMAD/_bmad/rbtv" rm tasks/restore-bmad-config.xml
git -C "H:/BMAD/_bmad/rbtv" rm tasks/update-bmad-config.xml
```

- [ ] **Step 2: Verify remaining task files**

```bash
ls "H:/BMAD/_bmad/rbtv/tasks"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" commit -m "feat: drop BMAD-only and obsolete task XML files"
```
...
```bash
find "H:/BMAD/_bmad/rbtv/admin/claude" -type f
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" rm -r admin/claude 2>/dev/null || rmdir "H:/BMAD/_bmad/rbtv/admin/claude"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add -A
git -C "H:/BMAD/_bmad/rbtv" commit -m "chore: remove now-empty admin/claude directory"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" tag rbtv-standalone-phase-6-start
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add admin/install/defaults.yaml
git -C "H:/BMAD/_bmad/rbtv" commit -m "feat(install): rewrite defaults.yaml for standalone operation"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add admin/install/module-manifest.yaml
git -C "H:/BMAD/_bmad/rbtv" commit -m "feat(install): create module-manifest.yaml"
```
...
```bash
grep -n "bmad\|_bmad\|project-root" "H:/BMAD/_bmad/rbtv/skills/playwright-cli/SKILL.md"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add skills/
git -C "H:/BMAD/_bmad/rbtv" commit -m "feat(skills): write standalone skill templates with path placeholders"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add commands/
git -C "H:/BMAD/_bmad/rbtv" commit -m "feat(commands): write command source templates for all skills"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add rules/output-resolution.md skills/output-routing commands/output-routing.md workflows/output-routing
git -C "H:/BMAD/_bmad/rbtv" commit -m "feat(output-routing): add rule + skill + command + workflow for CLAUDE.md-based runtime output resolution"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add admin/install/installer/manifest.py
git -C "H:/BMAD/_bmad/rbtv" commit -m "feat(install): manifest parser"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add admin/install/installer/context.py
git -C "H:/BMAD/_bmad/rbtv" commit -m "feat(install): install context resolver"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add admin/install/installer/generator.py
git -C "H:/BMAD/_bmad/rbtv" commit -m "feat(install): loader generator with overwrite-scoped cleanup"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add admin/install/installer/state.py
git -C "H:/BMAD/_bmad/rbtv" commit -m "feat(install): rbtv.yaml state reader/writer"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add admin/install/installer/__init__.py admin/install/installer/cli.py
git -C "H:/BMAD/_bmad/rbtv" commit -m "feat(install): CLI with module selection (output paths resolved at runtime per D7)"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add install.py
git -C "H:/BMAD/_bmad/rbtv" commit -m "feat(install): rewrite install.py as thin entrypoint to admin/install/installer"
```
...
```bash
mkdir "H:/BMAD/_bmad/rbtv-install-test"
```

- [ ] **Step 2: Copy RBTV repo into the test target**

```bash
cp -r "H:/BMAD/_bmad/rbtv" "H:/BMAD/_bmad/rbtv-install-test/rbtv"
```

- [ ] **Step 3: Run the installer**

```bash
python "H:/BMAD/_bmad/rbtv-install-test/rbtv/install.py" --target "H:/BMAD/_bmad/rbtv-install-test" --modules core,innovation,work-productivity,writing --non-interactive
```
...
```bash
ls "H:/BMAD/_bmad/rbtv-install-test/.claude/skills"
```
...
```bash
ls "H:/BMAD/_bmad/rbtv-install-test/.claude/commands"
```

Expected: 14 `rbtv-*.md` files.

```bash
ls "H:/BMAD/_bmad/rbtv-install-test/.claude/rules"
```

Expected: 10 `rbtv-*.md` files (9 original + `rbtv-output-resolution.md` added per D7/Task 26A).

```bash
ls "H:/BMAD/_bmad/rbtv-install-test/.claude/agents"
```

Expected: 2 `rbtv-*.md` files — `rbtv-designer.md`, `rbtv-web-research.md`.

```bash
cat "H:/BMAD/_bmad/rbtv-install-test/rbtv.yaml"
```
...
```bash
cat "H:/BMAD/_bmad/rbtv-install-test/.claude/skills/rbtv-planning/SKILL.md"
```

Verify:
- `{rbtv_path}` is replaced with a **vault-relative** path like `rbtv` (NOT `H:/BMAD/_bmad/rbtv-install-test/rbtv`). If it shows an absolute path, the generator is not using `ctx.rbtv_relative` — bug in `generator.py:_resolve_bake_value`.
...
```bash
cat "H:/BMAD/_bmad/rbtv-install-test/.claude/skills/rbtv-meeting-summarizer/SKILL.md"
```

Confirm the baked path ends with `workflows/meeting-summarizer/workflow.md`. Then verify the target exists:

```bash
ls "H:/BMAD/_bmad/rbtv-install-test/rbtv/workflows/meeting-summarizer/workflow.md"
```
...
```bash
cat "H:/BMAD/_bmad/rbtv-install-test/.claude/skills/rbtv-playwright-cli/SKILL.md" | grep -i "references/"
```
...
```bash
ls "H:/BMAD/_bmad/rbtv-install-test/rbtv/skills/playwright-cli/references/"
```
...
```bash
cat "H:/BMAD/_bmad/rbtv-install-test/.claude/agents/rbtv-designer.md" | head -20
```
...
```bash
python "H:/BMAD/_bmad/rbtv-install-test/rbtv/install.py" --target "H:/BMAD/_bmad/rbtv-install-test" --modules core --non-interactive
```
...
```bash
rm -rf "H:/BMAD/_bmad/rbtv-install-test"
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" tag rbtv-standalone-phase-7-start
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add README.md
git -C "H:/BMAD/_bmad/rbtv" commit -m "docs: add README for standalone RBTV"
```
...
```bash
cat "H:/BMAD/_bmad/rbtv/admin/roadmap.md" 2>/dev/null || echo "File does not exist — will create."
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" add admin/roadmap.md
git -C "H:/BMAD/_bmad/rbtv" commit -m "docs: update roadmap with standalone milestone and deferred work"
```
...
**Files:**
- Move: `H:/BMAD/_bmad/rbtv/` → `H:/BMAD/projects/second-brain/3. Resources/rbtv/`
- Modify: `H:/BMAD/projects/second-brain/.gitignore`

- [ ] **Step 1: Tag phase start**

```bash
git -C "H:/BMAD/_bmad/rbtv" tag rbtv-standalone-phase-8-start
```
...
```bash
git -C "H:/BMAD/_bmad/rbtv" push origin feat/rbtv-standalone
```
...
```bash
mv "H:/BMAD/_bmad/rbtv" "H:/BMAD/projects/second-brain/3. Resources/rbtv"
```
...
- `H:/BMAD/_bmad/rbtv/` no longer exists (moved to vault)
...
- `H:/BMAD/_bmad/` — BMAD module parent — may have `bmm/`, `cis/`, `core/` siblings to RBTV (RBTV already removed)
