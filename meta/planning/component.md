...
- Judge/eval pool — the two closing-seat definitions (`prompts/dod-judge.md` + `tasks/judge-milestone.md`, `prompts/unblock-checker.md` + `tasks/check-unblocked.md`, rows in `seats.csv`) the `drafter` shops into every PRODUCED taskforce as its closing seats; they hold no `planning.csv` node.
