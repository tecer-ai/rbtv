...
2. Read the seeded verdict. On `verdict: FAIL`, ask the coordination CLI what the escalation gate would decide for the seeded milestone — `coordinate fail-status <milestone-id> --json` — and read `at_bar` and `escalated` off its answer. DERIVE NOTHING YOURSELF: the count and the bar it is measured against are one authority, and a second derivation here is free to disagree with the one that actually enforces the halt. Nothing is stored either way — the verb recomputes both from the verdict log at the moment you ask.
...
4. On `verdict: FAIL`, act on the two flags together — `at_bar OR escalated` → QUEUE NOTHING, otherwise queue exactly one gap-fill. Both flags, never one: `escalated` alone would let a judge that skipped its escalation step silently un-halt the run, and `at_bar` alone would ignore a threshold lowered after the row already existed.
   - **`at_bar` or `escalated` true** — queue NOTHING. The refusal IS the halt: the escalation row stands, and no pass opens at this contract until an owner answer appears on the channel. Record the halted state explicitly — a `coordinate send <your-own-seat-name> --type note` naming the milestone, the bar, and which of `at_bar`/`escalated` fired. Never silence: a halt nobody wrote down is indistinguishable from a pass-opener that never ran.
   - **both false** — queue exactly ONE gap-filling pass for the seeded milestone at the SAME done contract, with pass-kind `gap-fill` (§ How to queue a pass). Idempotent: a gap-fill pass already queued or open for this verdict means queue nothing.
...
## Outcome
After every verdict, exactly the right passes are queued — on acceptance the newly unblocked milestones (none missed, none duplicated, none re-judged for size); on a FAIL below the bar exactly one gap-filling pass; on a FAIL at the bar (or already escalated) none, the halt recorded.
...
- Never queue an `initial` pass for a milestone whose `after` set is not fully accepted, and never queue any pass for a milestone with a pass already open or queued. Never queue anything for a milestone whose `fail-status` reports `at_bar` or `escalated` until an owner answer appears on the channel.
