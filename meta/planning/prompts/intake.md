...
<procedure source="prompts/interviewer.md#user-stories-gate">
...
The block above is forge's CREATE-mode gate, carried verbatim from its one home and read with three substitutions: CREATE mode stands where the block reads a workflow-producing use case, the piece set this request creates stands where it reads the produced workflow, and `forge-spec.md` stands where it reads `goal.md`. Step 6 fires it.
