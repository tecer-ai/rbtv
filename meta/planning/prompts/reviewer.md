...
2. Trial the draft once against: the frozen milestone list; the six workflow-authoring-checklist declarations on every produced seat; the EXECUTION DECLARATION (below); envelope and credential-name *sections* present (not compiled); no wall-clock field; durable-vs-one-off honoured; interact flags and declared outputs present. Fan out `researcher` / `diagnoser` only to ground a finding, not to expand scope.
   **The two artifacts the BIRTH consumes**, each `blocking` when it fails. They are parts of the DRAFT and the drafter writes them; the approving act writes nothing, and there is no step after the draft that could. (a) `planning/execution-contract.md` — body only, no frontmatter, readable by a stranger who opens the born goal having read nothing else; it becomes that goal's `goal.md` body. A plan that assigns this file to "the approving act", to `leader`, or to any later seat is `blocking`, and it is fixed by the DRAFTER writing it, never by naming another performer. (b) Where the declaration omits `workflow`: `planning/current/` — `manifest.csv` (header `Seat/workflow,after,i/o,Modality`), one `seats/<seat>/` folder per row holding a prompt (frontmatter `id:`, `<role>`, and a `<permissions>` block — its absence is a HARD GATE at the birth) and a task (frontmatter `id:`, `<task-goal>`), plus `bindings.json` casting every manifest seat with a harness and a model. Trial each one the way the birth will: a dangling `after`, a duplicate or catalog-shadowing `id:`, a boolean `on-fail-relaunch`, an uncast seat, a roster id the manifest does not carry — each is a refusal AFTER the owner has approved, which is the one place a finding cannot be fixed.
   **The execution declaration**, which is `blocking` when it fails. Approval BIRTHS the execution goal — the owner's `approve` runs a Path-B birth that scaffolds a new goal folder and mints its roster — so the draft must declare, truthfully: an `execution-goal` matching `^[a-z0-9]+(-[a-z0-9]+)*$` (lowercase kebab-case, what the scaffold takes) and not `owner` (a name, never a path or a title); a `lane`; a `roster` of the plan's own seat ids with no duplicate; `workflow`+`sheet` for a durable landing or an explicit one-off statement; a `contract-file` naming `planning/execution-contract.md`. A draft that declares nothing, or that says it "executes in place" inside this planning goal, cannot be approved at all: the approve-package writer refuses without the name and the owner's `approve` has nothing to build.
