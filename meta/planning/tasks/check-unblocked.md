...
- On a FAIL whose `fail-status` reports `at_bar` false AND `escalated` false: exactly one `gap-fill` pass is queued for the seeded milestone at the same done contract, its request carrying the verdict message number as seed pointer; a re-run of this seat on the same verdict queues no second one.
- On a FAIL whose `fail-status` reports `at_bar` OR `escalated` true: no pass is queued, and the result records the halted state explicitly.
...
- **FAIL below the bar, not yet escalated** → the gap-fill pass opens from the verdict's per-clause gaps; this seat's work ends with the result.
- **FAIL at the bar, or already escalated** → nothing opens; the halt record stands until an owner answer appears on the channel.
