...
- A MULTI-SEAT PHASE HANDS ARTIFACTS ACROSS THE GOAL'S `planning/` WORKSPACE — the one goal subtree the cage opens read-write to every seat (owner ruling d-s31-planning-workspace-shared-rw). A product no successor reads stays in the seat's own folder; a product a successor reads goes under `planning/` at a literal path both units name. A peer's SEAT FOLDER is absent under the seats tmpfs — never route a cross-seat read through one. The planning workflow itself is the live instance: five seats handing `planning/facts-brief.md` → `planning/design.md` → `planning/draft-plan.md` → `planning/review-package.md` → `planning/approval-digest.md` down the chain, each at a literal path both units name.
- A CONSUMER MUST NOT TREAT EXISTENCE AS PRODUCTION. A declared or workspace file can exist EMPTY from spawn onward, so "the file exists" proves nothing. Every produced artifact carries a first-line marker its consumer checks (the D13 patch plan's is its `PATCH-PLAN` first line); an empty or markerless file is a non-report, never a pass.
- Only ONE. A role that needs two goal-folder products is two roles, or one product it has not named yet.
- ONE PRODUCT IS NOT ONE FILE. `goal-writes` names the seat's PRODUCT — the thing its check-out is verified against — and a product that is a set of files under `planning/` is still one product, declared by the file a consumer opens FIRST. The planning drafter is the live instance: its `goal-writes` is `planning/draft-plan.md`, and the same act also lays down `planning/execution-contract.md` and the `planning/current/` seat set the birth mints from — parts of that one plan, handed across the shared `planning/` workspace above, not a second declaration and not a second seat. Splitting them into a second seat would put one plan in two authors' hands. What this rule forbids is a role with two UNRELATED products (a report AND a config), never a product with parts.
...
- ONE exemption inside `path`: the standing `rbtv:ignite/coord/coordinate` checkout grant, which belongs to the run protocol and never to the role. Any use of `coordinate` BEYOND checkout — an owner-channel ask, a fail-status query, a registration act — is described like every other instrument.
