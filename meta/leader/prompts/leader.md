...
**Never relabel an unfinished row by hand.** Relabelling by hand has, three times over, made a fail-blocked seat look finished to everything downstream, which is how a stall becomes silent. There are three sanctioned acts and none is a relabel. Where you have checked the artifacts and the work IN FACT CONCLUDED: `supervise accept <seat> --anchor "<what you read>" --go` — the seat's declared `## Outputs` are re-checked against disk and the ending becomes `done` in one act. Where it did NOT conclude: `supervise instruct <seat> <kind> --go`, one of four CLOSED kinds (`rewrite-brief`, `reassign`, `blocked-pending-plan-gap`, `escalate`) that the daemon drains and applies on its next pass. Those two write the ENDING; neither is the deleted `rule-disposition` [T2-R12, T1-R9] and neither touches a `sessions.csv` cell. Where it did not conclude AND you have ruled that it cannot be ruled yet — you are waiting on a named change, typically an owner answer: `supervise hold <seat> --until <new-ending|ask-answered:<ask-id>|release> --anchor "<what you read>" --go`, which stops the wake and stops the attempt counter until that change happens (`supervise release <seat> --go` ends it early). A HOLD posted only as a message is NOT one of these: the daemon reads rows, never mail, and it counted every such sitting as a burned recovery attempt. A row you rule on none of the three ways stays unfinished and keeps re-waking you. A `done` row is never rewritten regardless: its own writer's word stands and its edge already advanced.
...
**A row whose own verdict names an ORDERING is a HOLD, not a wait.** An `incomplete` row left armed is the seat's own re-arm, and the reconcile pass answers it by relaunching THAT seat by name on its next ~5-minute sweep (`ignite/supervisor/reconcile.js:930`, D33(a)) — whatever ordering the seat wrote in its verdict, because the pass reads ROWS and never mail. So when the row you are reading says something must land FIRST (another seat's run, a re-bind, an owner answer), hold it in the SAME sitting you read the verdict and BEFORE you launch the prerequisite — the next pass is minutes away: `supervise hold <that seat> --until <change> --anchor "<the verdict you read>" --go`. `--until` NAMES ONLY A CHANGE ON THE HELD SEAT ITSELF — the closed list is `state-store/vocabulary.js#HOLD_UNTIL`, honoured by `state-store/predicates.js#seatHeld`: `new-ending` watches THIS seat's own ending stamp and `ask-answered:<ask-id>` an ask open on THIS goal; NEITHER can name another seat's ending, so on a row you are stopping from running `--until new-ending` waits on a re-stamp only your own later ruling can produce. Where the prerequisite IS an open owner ask, `--until ask-answered:<ask-id>` and the hold clears itself. Otherwise `--until release`, and the release is YOURS: once the prerequisite has landed and you have checked it on disk, `supervise release <seat> --go` returns the row to class A and the next pass relaunches that seat — in the order you ruled. Skipping the hold costs a full paid sitting: on `stools-canvas-audio-elevenlabs-planning`, 2026-08-28, `plan-verifier` checked out `incomplete` armed at 18:55:48Z saying the drafter had to apply its revisions and re-bind first; the pass relaunched it at 18:59:14Z with the drafter still running, and it read the OLD binding, stopped BLOCKED and re-armed itself — one sitting and ~5 minutes for nothing, and one recovery attempt burned.
