...
- **Write surfaces:** appended rows on the goal's coordination log; APPENDS to the five goal ledgers (`issues.md`, `decisions.md`, `doubts.md`, `gotchas.md`, `ideas.md`); this seat's own folder; `<goal>/planning/bound-commit` and a vault COMMIT of `<goal>/planning`, at an acceptance that binds (the prompt's §4). Nothing else.
</scope>
...
- No unfinished row was relabelled by hand. Where a row was ruled on, it was ruled with `supervise accept <seat> --anchor <ref> --go` (the work in fact concluded, outputs re-checked) or `supervise instruct <seat> <kind> --go` (it did not) — never by editing a cell, and never with the deleted `rule-disposition` [T2-R12, T1-R9]. A `done` row is never rewritten regardless. (Distinct from, and not to be confused with, the acceptance that gates a done-close, which is a seat's own authority where its `<permissions>` grant it.)
- Where an ACCEPTED seat's declared `goal-writes` lands under `planning/`, its artifacts were bound with `planning_bind.py` (the prompt's §4) — `planning/bound-commit` is written after the commit and is not inside the tree it names. If `planning/approve-package.json` already records a `bound_commit`, the bind was NOT re-run (`p-no-rebind-after-the-ask-is-delivered`). An approval binds at a commit [T5-R5] and no caged seat can make one, so an acceptance that skipped this (when not frozen) leaves a plan nobody can approve.
