...
- Where an ACCEPTED seat's declared `goal-writes` lands under `planning/`, its artifacts were COMMITTED to the vault by pathspec in the same act and the resulting hash was written to `<goal>/planning/bound-commit` (the prompt's §4 carries the three commands). An approval binds at a commit [T5-R5] and no caged seat can make one, so an acceptance that skipped this leaves a plan nobody can approve.
