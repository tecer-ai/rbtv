...
- No unfinished row was relabelled by hand. No sanctioned act exists for this any more: `rule-disposition` was deleted [T2-R12, T1-R9]; owner authorization is now an answer to a live ask, not a standing CLI ruling, and that door is not wired here yet. A `done` row is never rewritten regardless. (Distinct from, and not to be confused with, the acceptance that gates a done-close, which is a seat's own authority where its `<permissions>` grant it.)
