...
| **A task will be dispatched to a CLI model** — a chosen worker is `kimi-code-cli`, `codex-cli`, `claude-code-cli`, `qwen-code-cli`, … | **The model's rendered manual** — open it JIT at first dispatch to that model for the exact invocation shape (command, flags, work-dir, prompt transport, exit handling, resume). **Availability guard:** the manual exists only when that model package is ELECTED (`rbtv.json` `model_packages`) and present AND the render step (P3) has produced it — if the package is not elected, or `models/{model}/` is absent, the CLI dispatch is not available; routing routes to an Agent-tool Claude tier instead or halts (routing card §1). | `models/{model}/manual.md` |
...
> **Model packages installed:** claude (native, from Claude Code), codex (CLI), deepseek (API), gemini (API), kimi-code (CLI)
> **Not elected:** claude-code (CLI), manus (API), qwen-code (CLI)
...
| **codex-cli** | Code-executing CLI worker (`codex exec`) — separate-process execution. Routes 6 (model, effort) variants: gpt-5.5 `low-reasoning`/`default`/`high-reasoning` + gpt-5.4 `gpt-5.4-low`/`gpt-5.4`/`gpt-5.4-high` (`-m gpt-5.4`). Only gpt-5.5/medium `default` is validated end-to-end (~10 bounded-code dispatches, 2026-06-11); the other five share the CLI contract but are `probe-pending` (gpt-5.4 = conduct-only 2026-06-11 reasoning battery, ran all 3 efforts exit-0 but UNGRADED). |
