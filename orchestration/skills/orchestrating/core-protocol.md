...
The always-loaded heart of `rbtv-orchestrating`. SKILL.md loads THIS file at the start of any orchestration and keeps it in context for the whole run. It carries three things and nothing else: the **invariants** every orchestration obeys, the **situation table** that routes the run to the right card just-in-time, and the **capability summary** of the catalog. The cards hold the situational mechanics; this core holds only what is true in every situation.

Read it once, hold it all run. When a situation arises, the situation table names the ONE card to open for it — open that card, follow it, return here for the next situation. Never front-load the cards; never improvise a step the table maps to a card.
...
| 9 | **Depth cap ≤ 2 conductor levels** | Agent-tool sub-agents CANNOT spawn sub-agents (the nesting wall). A second conductor level is achieved ONLY through CLI workers as separate OS processes (`cast` of codex / claude-cli / opencode). The top conductor plus at most one CLI-driven sub-conductor — never stack deeper. |
...
Reading the table: most situations map to a card. A CLI dispatch opens the **dispatch-wrapper** card — the invocation shape is `cast <harness> <model> <effort> <launch-root> -f <task file>` (that card). Do not open anything under a models tree. **Parallel waves:** when several workers return concurrently, each return is processed INDEPENDENTLY through the same Verification gate (one return = one gate run); the per-wave reconciliation — committing at wave boundaries to restore git-diff resolution — is the Routing card's wave-commit discipline (routing §8, which also sized the wave). A situation not in this table is either an invariant question (answer it from the invariants above) or a sign the run has drifted off-protocol — stop and reconcile, do not invent a step.

---

## Capability summary — what is routable here

Routable set = catalog ∩ availability. Catalog = `cast route --catalog` (the live roster). Availability-now = binary installed per `cast doctor` logic, plus API-key resolution for `carrier: api` rows. `rbtv.json` `model_packages` is not the authority. Recall the roster on demand with `cast route --catalog` (add `--json` for the machine-readable view).

Per-harness capability lines — a STATIC reference roster of what this skill can carry, NOT a list of what is available now. A line's presence in this table says nothing about routability: routability is decided ONLY by catalog ∩ availability (recalled via `cast route --catalog`). Routing reads the catalog row for the full field set — these lines are the at-a-glance recall, not the routing inputs:

| Harness / carrier | reasoning / coding / cost (1–7; 7=strongest/priciest) | routable_for | One-line capability (recall only — catalog is authoritative) |
|-------------------|-------------------------------------------------------|--------------|----------------------------------------------------------------|
| **claude / agent-tool** | fable-5 7/7/7 · opus-5 7/6/6 · sonnet-5 6/5/5 · haiku-4-5 3/2/3 | all leaves | Agent-tool Claude tiers — in-session default carrier; `cast` refuses to launch these rows; no native CLAUDE.md/rules load (parent must inline); nesting wall (cannot spawn sub-agents). |
| **claude / cli** | fable-5 7/7/7 · opus-5 7/6/6 · sonnet-5 6/5/5 · haiku-4-5 3/2/3 | all leaves | `cast claude …` process worker — natively loads workspace `CLAUDE.md`/rules; usable as a CLI-driven sub-conductor (process boundary clears the nesting wall). 5-rung effort ladder for fable/opus/sonnet; haiku is inert. |
| **codex / cli** | gpt-5.5 7/7/7 · gpt-5.6-* per catalog | all leaves | Code-executing CLI worker (`cast codex …`) — separate-process execution. Effort rides `model_reasoning_effort`. |
| **opencode / cli** | glm-5.2 · fugu-ultra · deepseek-v4-* · kimi k2.7/k3 · grok/gemini rows per catalog | per row | Code-executing CLI worker (`cast opencode …`). Kimi models ONLY via opencode — kimi-code-cli is retired. NO native sandbox — worktree-mandatory launch root (G1 exception: launch root IS the target). |
| **api / api** | deepseek-v4-flash 4/3/1 · deepseek-v4-pro 5/4/1 · gemini-3.5-flash 6/3/4 · gemini-3.1-flash-lite 2/1/2 · manus-autonomous 1/1/1 | text / web-research per row | API workers — `cast` refuses to launch; hand off to `cast api`. `routable_for` omits code roles except where the catalog says otherwise. Gemini is the only web-capable chat worker (`--grounded`). |

A line here is recall, not permission: routing routes only on catalog ∩ availability rows. `cast route --catalog` is the single source for what is routable in this workspace.
