...
This template is instantiated mechanically at spine init — copy the fenced block, substitute `{run-name}` and the timestamp, write it to the spine location, and initialize the Resume Point: **Last completed** = "intake complete", **Next dispatch** = the first task/batch routing will assign, **Last update** = the init timestamp. All FOUR leading callouts (mutable-overwrite + audience + regenerate-don't-edit + computed-claims) are part of the file and MUST survive instantiation — they keep the mutability boundary, the not-a-worker-surface rule, the regenerate-never-edit-in-place discipline, and the compute-every-claim obligation visible at the point of use. The count is stated so a conductor copying literally cannot drop one: name every callout you add here.
