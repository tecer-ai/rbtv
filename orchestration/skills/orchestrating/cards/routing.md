...
> **⚠ Fable 5 availability (access-gated; marked 2026-06-14).** Fable 5 (`claude:fable`) may be gated behind the limited "Fable Mythos" access rollout. When a headless `claude -p --model claude-fable-5` returns *"Claude Fable 5 is currently unavailable — Fable Mythos access"* (observed 2026-06-14), Fable is NOT dispatchable: **both Fable-pinned roles above (Conductor and Final-plan reviewer) fall back to Opus** (`claude:opus`), the senior available Claude, until access is granted. Fable stays the named default pin — do NOT remove it; restore it the moment a probe succeeds. A workspace WITH Fable access is unaffected (the probe succeeds and the pins resolve to Fable).
