...
The boundedness table above names a TIER ("the cheapest capable (model, variant)", "a mid-tier Claude", "a top-tier Claude"). Resolving that tier to ONE concrete `(model, variant)` is a deterministic pure function, NOT a judgment pick. Do NOT hand-execute the enumerate→filter→rank stages — CALL the router script, which implements them over the live manifests on disk:
...
| **2. Call the router** | `echo '{…profile…}' \| python {rbtv_path}/orchestration/models/route.py` (or `--profile {file}`); add `--explain` to print the enumerate→filter→rank trace for the run-log. Stdlib only — no network, clock, or randomness. |
...
**Specification of record — the algorithm the script implements (authority lives here).** `route.py` is the mechanization; THIS card is the source of truth for the algorithm it runs. On any script-vs-card divergence the CARD TEXT WINS and a defect is filed against the script — never hand-edit a verdict to match the card, and never let a script behavior silently redefine the algorithm. Selector fields per `models/manifest-schema.md` §2: `reasoning_tier`, `context_window`, `cost_class`, `code_competence`, `web_access`, `evidence_status`. The full behavior + acceptance contract is `specs/router-script-spec.md` in the token-efficiency-refactor plan folder (the spec is the build contract; this card is the algorithm authority).

| Stage | Rule (the contract `route.py` implements) |
|-------|------|
| **1. Enumerate** | List EVERY installed `(model, variant)` from the live `models/` folder — Claude (Agent-tool, §4), claude-cli, kimi, codex, the installed API packages — never a bare provider name. A model with one operating profile still contributes one variant. |
...
**Total order — never collapses.** Stages 1-3 are a PROVABLY TOTAL order: the five keys break every tie in sequence, and Key 5 closes it absolutely. Key 4 (`model-name`) and Key 5 (`variant-name`) together form the `(model-name, variant-name)` pair, which is UNIQUE per enumerated row by construction — every enumerated `(model, variant)` row carries a distinct variant name within its model, so two distinct survivors ALWAYS differ on Key 4 or Key 5. Therefore two distinct variants can NEVER tie on all five keys, and the selector is NEVER non-deterministic for any legal manifest set — including future manifests where two variants of one model differ only on a non-rank field (e.g. `context_window`): they still differ on `variant-name`, so Key 5 separates them. The variant field-count discipline (`models/manifest-schema.md` §2 — a mode changing no routing-relevant field is not a separate variant) remains a variant-AUTHORING rule that keeps the enumerated set lean; the TOTALITY guarantee no longer rests on it but on Key 5's unique pair. If zero variants survive Stage 2, degrade per §1 (next-capable / Agent-tool Claude fallback) or HALT — NEVER silently pick an incapable variant.

**Name the chosen pair — never a bare provider.** Every logged or summarized assignment names the chosen `(model, variant)` — e.g. `claude:sonnet`, `deepseek:v4-flash` — NEVER just "Claude" or "DeepSeek". A bare-provider label in the run-log or intake budget summary is the collapse this selector exists to prevent.

**Text-leaf clause — where API chat workers compete.** API chat workers carry `code_competence: none`, so Stage 2's `code_competence ≥ needed` filter removes them from EVERY code leaf automatically. They survive ONLY on TEXT leaves — partially-bounded text synthesis, mechanical text transforms, desk-research synthesis over inlined sources — where they compete on `cost_class` against the Claude tiers. NEVER route a code leaf to an API chat worker; the filter already bars it, and a manual override of that bar is a routing defect.
...
|------|-------|----------------|
| **Conductor / orchestrator** | **Fable 5** — the pair `claude:fable` (carrier-resolved per §4). The default conductor tier. The most senior Claude conducts; it never routes down to a worker tier. | Owner mandate (Brief B Part 5): Fable 5 is `rbtv-orchestrating`'s default conductor/orchestrator, replacing the opus-class default the conductor previously implied. Conducting is the highest-judgment seat in the run. |
| **Final-plan reviewer** | **Fable 5** — the pair `claude:fable` — the default pin for the cold review of a generated plan before execution. | Locked owner mandate: Fable 5 cold-reviews the generated plan before any execution begins (decisions.md "Final plan reviewer"). |
...
| **Commits** | Routed through a worker invoking `rbtv-commit` — never folded into an executor's own run. Carrier: an **Agent-tool Claude** (sonnet floor) invoking `rbtv-commit`; CLI executors are not the commit worker. | Commit hygiene, message quality, exclusion lists, and no-push guarantees require the skill; CLI workers are kept OFF commits deliberately. |
...
**Within the top Claude tier — Fable 5 vs opus.** Both are top-tier Claude; the cut between them is a within-tier tiebreaker (boundedness-first stays the master cut — §2). The **conductor and the final-plan-reviewer roles take Fable 5** (the seats above). **Cost-sensitive senior work takes opus** — a high-judgment WORKER dispatch (an architecture spec, a rule edit, a judgment-dense leaf) where the run's budget answer favors the cheaper top-tier Claude; opus carries `cost_class: high` while Fable 5 carries `cost_class: premium` (the §2a selector therefore never auto-picks Fable for a worker leaf — Fable is reached only via these pins). For the finer within-tier "best at" disambiguation between any two same-tier variants, consult each `(model, variant)`'s `specialty` manifest field (p2-1) and the overlap rows in `orchestration/docs/routing-matrix-reference.md` — a within-tier tiebreaker, never a new master cut.
...
**Two distinct Claude carriers — both enumerated as separate `(model, variant)`.** Claude is enumerable through TWO packages, NOT one: `models/claude/` (the **Agent-tool** in-session carrier, dispatched via the Agent tool — `claude:opus` / `claude:sonnet`) and `models/claude-cli/` (the **`claude -p` process** carrier — `claude-cli:opus` / `claude-cli:sonnet`). The §2a selector enumerates BOTH as distinct pairs; they differ in routing-relevant properties (the Agent-tool carrier does NOT natively load workspace `CLAUDE.md`/rules and cannot spawn sub-agents — the nesting wall; the process carrier loads workspace rules natively and clears the nesting wall as a separate OS process). NEVER collapse the two Claude carriers into one entity.

| Tier the tree chose | Carrier |
|---------------------|---------|
| A Claude tier (mid or top) | Default to **`claude`** — the **Agent-tool** in-session carrier (`models/claude/`; no guidance-file dependency, no process overhead). Use **`claude-cli`** (the `claude -p` process carrier, `models/claude-cli/`) instead ONLY when a process-boundary sub-conductor is needed (the worker must itself drive CLI workers — the nesting wall forces a process) or when native workspace-rule loading is required. Both are enumerated distinctly by §2a; this row picks BETWEEN the two carriers for the chosen tier. |
| A code-executing CLI leaf | Choose **kimi vs codex** by manifest fit, not preference: kimi is the validated bounded-code executor (default for bounded code); codex is the live-proven-once alternative — pick on the manifest's code-competence / OS-quirk / cost-class fields for the specific task. Absent a distinguishing manifest reason, default to the validated worker (kimi) and log the choice. |
