...
| **3. Rank by total order** | Order the survivors by, in strict priority: (1) `cost_class` ascending (`cheapest` < `low` < `mid` < `high`); (2) `evidence_status` (`validated` > `probe-pending`); (3) `reasoning_tier` fit — closest-not-over (the lowest tier that still meets the requirement, never a tier above what the leaf needs); (4) model-name lexical (ascending). Pick the top. |
...
**Total order — never collapses.** Stages 1-3 are a TOTAL order: the four keys break every tie in sequence, so two distinct variants can never both rank first. The selector is non-deterministic ONLY if a tie survives ALL FOUR keys, which a distinct-variant manifest set forbids (a mode changing no routing-relevant field is not a separate variant — `models/manifest-schema.md` §2 variant field-count discipline). If zero variants survive Stage 2, degrade per §1 (next-capable / Agent-tool Claude fallback) or HALT — NEVER silently pick an incapable variant.
