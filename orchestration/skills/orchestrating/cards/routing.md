...
### 2a. The deterministic `(model, variant)` selector — enumerate → filter → rank

The boundedness table above names a TIER ("the cheapest capable (model, variant)", "a mid-tier Claude", "a top-tier Claude"). Resolving that tier to ONE concrete `(model, variant)` is a deterministic pure function, NOT a judgment pick — run it on the installed manifests (selector fields per `models/manifest-schema.md` §2: `reasoning_tier`, `context_window`, `cost_class`, `code_competence`, `web_access`, `evidence_status`):
...
| **3. Rank by total order** | Order the survivors by, in strict priority: (1) `cost_class` ascending (`cheapest` < `low` < `mid` < `high`); (2) `evidence_status` (`validated` > `probe-pending`); (3) `reasoning_tier` fit — closest-not-over (the lowest tier that still meets the requirement, never a tier above what the leaf needs); (4) model-name lexical (ascending); (5) `variant-name` lexical (ascending). Pick the top. |
...
(This clause is the workflow-side resolution of the haiku conflict; this card governs routing. The rule-side clause in `core/rules/sub-agents.md` MUST carry this exact substance — any divergence between the two is a build defect to fix, not an authority split. p4-7 lands the matching rule-side wording.)
