...
| Step | Action |
|------|--------|
| List the elected packages | The routable set is the workspace ELECTION (`rbtv.json` `model_packages`), NOT raw folder presence — `route.py` enumerates only elected packages. `{rbtv_path}/orchestration/models/` is the shippable CATALOG (a superset: every package the repo ships); a package present there but NOT elected does not route this run (it appears under `not_elected` when the conductor runs `python {rbtv_path}/orchestration/models/route.py --availability`). Folder presence is still a necessary guard — an elected package whose folder or `manifest.yaml` is absent cannot route. **Skip underscore/known-infra dirs** (`_api/`, `_fixture/`, `mirror/`) and any dir carrying no `manifest.yaml` — never enumerated as a worker. A CONFIGURABLE package confined to a backend subset (`rbtv.json` `model_variants`, e.g. `{opencode: [z1, deepseek-flash]}`) enumerates ONLY its elected backends — `route.py` skips the rest at enumerate; an absent entry routes all that package's backends. |
...
| Recall the election live | Recall the elected set on demand with `python {rbtv_path}/orchestration/models/route.py --availability` — it reads `rbtv.json` `model_packages` (the authority) and prints `{"elected": [<ids>], "not_elected": [<ids>]}`. Nothing about the election is baked into the shared repo; the live read is the source. On any election-vs-catalog mismatch the ELECTION wins (extra folder packages are catalog, not routable); note the mismatch in `run-log.md`. Because the recall reads `rbtv.json` directly each time, it cannot go stale across workspaces. |
...
| **4. PIN/STAKES apply AFTER** | Stakes tier-up and the §3 pinned-role floors apply ONLY after the cheapest pick — they may raise it UPWARD (reviewer ≥ sonnet; conductor → fable, opus fallback; etc.), never below the floor. Pinned-role floors read the `executor_reasoning` INTEGER from the profile. Effort = f(boundedness) is set from `reasoning_modes` AFTER the pin (`_resolve_effort`): fully-bounded → `low`; partially-bounded → `medium`; unbounded → `high`/`max`; single-mode workers are a no-op. |
...
A task declares its **known input** — `known_input_size` (tokens) = the dispatch prompt + the files the task is required to read — and the §2a GATE refuses any worker whose window is too small to hold it comfortably, so a large-footprint task never lands on a worker that overflows mid-run (Kimi/Claude auto-compaction; Codex `"ran out of room"` crash). The router CONSUMES `known_input_size`; the PLANNER produces it (measure the read-set, chars→tokens ÷3 round up) — never measured here.
...
|------|-------|----------------|
| **Conductor / orchestrator** | **fable** — the pair `claude-code-native:fable` (carrier-resolved per §4), falling back to **opus** (`claude-code-native:opus`) when fable is unavailable. The default conductor tier. The most senior available Claude conducts; it never routes down to a worker tier. | Re-added 2026-07-07 (supersedes D15, 2026-06-17): access reopened — an Agent-tool `model: fable` sub-agent dispatched successfully (merge-refactor-unification run, logged as that run's D2 deviation). Conductor pin re-points to **fable**, the senior-most available Claude, with opus as the fallback when fable is unavailable. Conducting is the highest-judgment seat in the run. |
| **Final-plan reviewer** | **fable** — the pair `claude-code-native:fable`, falling back to **opus** (`claude-code-native:opus`) when fable is unavailable — the default pin for the cold review of a generated plan before execution. | Re-added 2026-07-07 (supersedes D15, 2026-06-17): same fable re-add rationale as conductor above. Fable cold-reviews the generated plan before any execution begins; opus reviews when fable is unavailable. |
...
>
> **✅ Fable 5 re-added (2026-07-07) — supersedes the drop above.** Access reopened: an Agent-tool `model: fable` sub-agent was dispatched successfully (merge-refactor-unification run, logged as that run's D2 deviation). The `fable` variant is back in both Claude worker manifests (`available: true`), and the conductor / final-plan-reviewer pins are re-pointed to **fable** (`claude-code-native:fable`), with **opus** as the fallback when fable is unavailable.

> **⚠ Debug floor de-carrier-locked (D17, 2026-06-17).** The `debug` pin no longer requires Claude membership — it admits any elected, available, code-eligible executor with **reasoning ≥ 7**. With the current catalog that is exactly `{claude-code-native:fable, claude-code-cli:fable, claude-code-native:opus, claude-code-cli:opus, codex-cli:gpt-5.5}` (the fable pairs re-added 2026-07-07); reasoning-6 workers (sonnet, kimi, gpt-5.4) and non-code API workers (deepseek/gemini/manus) stay barred. **Observable default unchanged:** opus (cost 6) outranks the cost-7 peers (fable, gpt-5.5) under the §2a cost-ascending RANK, so a standard debug task still picks `claude-code-native:opus`; a cost-7 peer wins debug only when opus is unavailable/capped.
...
| **Pre-dispatch mirror REFRESH** | Presence is not currency. A guidance file that exists can still be stale: the mirror refreshes ONLY when the installer runs, and every `AGENTS.md` is gitignored — so a skill or rule edited since the last run reaches a claude worker and NOT the codex/kimi/opencode worker beside it, with nothing in git ever revealing the drift. Before the FIRST CLI-worker dispatch of a run, refresh the mirror at the launch root: `python <rbtv_path>/install.py --mirror --non-interactive --target <launch-root>` (`rbtv_path` from the launch root's `rbtv.json`; it is idempotent write-if-changed, ~2-3s steady-state). Refresh ONCE per distinct launch root per run, not per dispatch. **NEVER pass `--exclude`** — omitting it preserves the workspace's recorded exclusions; passing it REPLACES them and starts rendering mirrors into paths the owner excluded. A refresh that fails does NOT block the dispatch: report it and dispatch anyway (a stale mirror still carries the previous render's rules; a blocked run carries none), but say so in the run's state. Skip entirely when the launch root has no `rbtv.json` or elects no mirrorable worker — that workspace has no mirror by design. A team-kit run needs no manual step: `coord.py launch` and `close-seat --renew` already refresh every non-claude seat's root before the pane opens. |
| **Pre-dispatch pinned-flag check** | Before dispatching ANY CLI worker, every non-trivial flag the dispatch pins (approval / sandbox / permission / output-path) MUST be verified present in the live `<cli> <subcommand> --help` — EVERY dispatch, not only on a version mismatch, and against the EXACT subcommand the dispatch uses (`codex exec --help`, not `codex --help`). ⚠ **THIS IS NO LONGER PROSE YOU RUN BY HAND WHEN THE PAIR HAS A PROFILE (task 7.54).** For a `(model, variant)` whose manifest records a `launch_profile`, it is dispatch-wrapper §5a **gate 5** — one call to `orchestration/capabilities/dispatch-resolve`, which derives the pinned flags from the resolved profile's own argv and checks them against the live `--help` (`E_PINNED_FLAG_ABSENT`). The per-harness knowledge that used to live in each delta's Pre-flight has ONE home: the shared resolver. For the 9 elected pairs with NO profile yet (task 7.86 authors them), run the gate from that worker's delta Pre-flight exactly as before. Either way, a pinned flag absent from live `--help` → STOP, do not dispatch; re-resolve the flag at the delta source, re-render the manual, re-run the gate — never hand-edit the rendered manual or pass an ad-hoc flag. A removed/renamed flag is a hard arg-parse error (exit 2) pre-spend if dispatched. (codex 0.137.0 dropped `--ask-for-approval` from `exec` at the same version family the manual pinned — caught at p5-2.) ⚠ **An unreadable `--help` is NOT a pass** (`E_PREFLIGHT_UNAVAILABLE`, a distinct code from a missing flag): `opencode run --help` writes to stderr and exits 0, which the shared `readHelp` cannot read today — that pair fails closed, and it is filed, not worked around. |
...
| Rule | Detail |
|------|--------|
| Route by manifest | Send the brief to an installed worker whose manifest declares `web_access: true`. Match the brief to a worker that can actually reach the web. (NOTE: the code-CLI workers are NOT web workers — opencode is `web_access: false` (webfetch only, no search), kimi's web tools are strippable but it is not the research default; route web research to a web-declaring worker or the Agent-tool path. D-exec-11, 2026-06-10.) |
