...
|------|-------|----------------|
| **Conductor / orchestrator** | **fable** — the pair `claude-code-native:fable` (carrier-resolved per §4), falling back to **opus** (`claude-code-native:opus`) when fable is unavailable. The default conductor tier. The most senior available Claude conducts; it never routes down to a worker tier. | The conductor pin is fable, with opus as fallback when fable is unavailable. Conducting is the highest-judgment seat in the run. |
| **Final-plan reviewer** | **fable** — the pair `claude-code-native:fable`, falling back to **opus** (`claude-code-native:opus`) when fable is unavailable — the default pin for the cold review of a generated plan before execution. | Fable cold-reviews the generated plan before any execution begins; opus reviews when fable is unavailable. |
...
> Fable is available for the conductor / final-plan-reviewer pins, with opus as fallback when unavailable. The `fable` variant is present in both Claude worker manifests (`available: true`).
