...
| Sketch the provisional map | Routing has not run yet, so the map is PROVISIONAL, but it STILL names a concrete `(harness, model)` plus `effort` plus `carrier` per work-cluster — NEVER a bare provider. Populate the roster by running `cast route --catalog` LIVE — never from memory, never from an election list. Then assign each phase/work-cluster its DEFAULT pair by calling `cast route` (routing card §2a — one profile per cluster, carrying its boundedness band) against the live catalog ∩ availability: bounded code → the cheapest capable code-executor row, judgment-dense work → a top-tier Claude row, review → the pinned reviewer-floor row, text synthesis → the cheapest capable text row. The pairs shown in conversation are ILLUSTRATIVE — the binding pair for each cluster is whatever the live §2a run returns, never a hardcoded pair. The user approves a ceiling / swap POLICY against this sketch, not a finalized assignment; routing re-surfaces the binding map at step 8. |
| Spend forecast — per-row `cost` | Each work-cluster's line shows its assigned `(harness, model)` plus `effort` plus `carrier` and that row's `cost` integer read from the catalog (`1`–`7`, `7` = priciest, never auto-picked on a cost tie), NOT a dollar figure. The user swaps on the relative cost integer, not on an invented number. Flag any row whose catalog `evidence_status` is `probe-pending` (the API chat/agentic rows) as **ESTIMATE-ONLY** — its `cost` is authored from the routing-matrix reference, not pilot-validated spend. |
| Name pairs, never bare providers | The budget summary NAMES `(harness, model)` plus `effort` plus `carrier` on EVERY row — e.g. `claude/sonnet-5` `carrier=agent-tool`, `api/deepseek-v4-flash` `carrier=api` — and NEVER a bare provider like "a top-tier Claude" or "DeepSeek". A bare-provider row is the "Claude as one entity" collapse this enumeration exists to kill (routing §2a "never collapses"; the run-log and this summary name the same pair). |
| Timing | The map is provisional at intake (routing finalizes assignments) — present it as the cost policy the user is approving, and re-surface the final pair-named map at step 8. |

`cost` is an INTEGER (`1`–`7`, cost-ascending — `7` ranks above all others, reserved for premium-priced top-tier models e.g. Fable 5; cost-ascending selection never auto-picks the highest value on a tie — pinned-roles reachability), NEVER a dollar amount — this section's opening no-currency rule and core invariant 6 both hold: the forecast stays in relative cost tiers, never currency.
