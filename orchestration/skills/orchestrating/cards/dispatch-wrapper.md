...
The scaffold runs four scripted pre-flight gates before it writes anything; ANY failure ⇒ EXIT≠0 + a machine-readable error naming the gap, with NO file written (a broken dispatch is caught before spend). Gate 5 is the conductor's own — the scaffold does not run it:
...
| 5 | **Profile resolve + dispatch pre-flight** (task 7.54) — **conductor-run, not scripted into the scaffold** | a NAMED profile resolves through the ONE shared resolver, its pinned flags exist in the live `--help`, and a work-target was supplied — **one call**, `orchestration/capabilities/dispatch-resolve`, invoked by the conductor via the `rbtv-dispatch-resolve` skill. ⚠ Fires only when the conductor is dispatching against a profile it NAMED; no `(model, variant)` maps to one (see below), so the ordinary path is the package manual + that worker's delta Pre-flight. |

**Gate 5 — what it replaced, and what it does NOT cover.** The pinned-flag check used to be prose the conductor ran by hand (`routing.md` §4). Where a profile is in play it is a **resolver call the conductor makes itself, via the `rbtv-dispatch-resolve` skill** — no code path consumes it (owner ruling `d-r2-preflight-manual-plus-skill`, 2026-08-10, made permanent 2026-08-11): `preflightDispatch(config, profileName, { addDir, effort })` resolves the NAMED profile through `ignite/launch-profiles/` (task 7.42) and in the same call refuses on `E_ADD_DIR_ABSENT`, `E_ADD_DIR_RELATIVE`, `E_UNKNOWN_PROFILE`, `E_RAW_FLAG`, `E_UNKNOWN_EFFORT`, `E_PINNED_FLAG_ABSENT` or `E_PREFLIGHT_UNAVAILABLE`. (`E_SEATBINDS_PROFILE` was retired with the seat-binds deny-list, same ruling.) The conductor supplies a profile NAME, an abstract effort level and declared-slot values — **never argv, never flags.** Effort reaches the harness in that harness's own dialect via the profile's translation table (`high` → `--effort high` for claude, `-c model_reasoning_effort=high` for codex), never as a hand-written flag.

⚠ **THE CONFINEMENT SPLIT (row G1) IS ENFORCED HERE, AND ENFORCEMENT IS NOT CORRECTNESS.** `{extra_dir}` became an authorable slot at task **7.87**, but NO SHIPPED PROFILE DECLARES IT — so **the add-dir flag is still hand-composed**, and the result's `addDirResolved` (`false` = you still owe the flag) is read by the conductor off the skill's output, never by any dispatch code. Gate 5 does not make that correct; it makes its ABSENCE LOUD — a dispatch that forgot the work-target is refused rather than launching a worker rooted at its guidance root, which is how a bare self-commit once swept 5 foreign files. Never read gate 5 as "confinement solved": it is *enforced, and carried to 7.87*.

⚠ **COVERAGE IS ZERO PAIRS, BY RULING — the package manual is the dispatch home for EVERY elected pair.** `launch_profile` is RETIRED (owner ruling 2026-08-11, executing `d-r2-preflight-manual-plus-skill`): the 11 recorded values all named `cli-*` profiles `r-seats-only-architecture` retired and nobody authored, nothing read the field, and manual invocation of `rbtv-dispatch-resolve` is the permanent contract. ⇒ Run gate 5's pinned-flag check **from that worker's delta Pre-flight**, exactly as `routing.md` §4 prescribes for a pair without a profile. The 14 profiles that remain are DAEMON SEAT profiles — resolving one hands you the daemon's caged command, not a manual dispatch's (`dispatch-resolve.md` § What this lane does NOT give you). One live gap stays tripwired in the capability's probe: `opencode run --help` writes to stderr while exiting 0, which the shared `readHelp` cannot read.
...
**Capture-bound Manus dispatches:** when the dispatch is a capture-bound Manus task, add the pre-flight line that the prompt MUST demand the complete deliverable in the reply MESSAGE TEXT — an attachment-only return failed live (see the manus delta obligations).
