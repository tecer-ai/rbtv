...
**Standing obligation — a checkpoint verdict is COMPUTED, and this binds whoever runs the checkpoint.** Every verdict this card produces is a claim about STATE — "complete", "in flight", "the gate is green", "the move landed" — and each is checkable by one command. Compute it from that command AS YOU ASSERT IT; never assert it from having done the thing. This is `rbtv-deterministic-first`'s Compute gate binding the checkpoint — follow the rule, do not restate it — and it extends §1c's re-run-every-count-claim discipline from a worker's REPORTED count to the checkpoint's OWN verdict. It binds **whoever runs the checkpoint — conductor, reviewer, cold verifier, or any dispatched agent alike**; there is no conductor exemption. A conductor asserting "done" from memory is the same defect as a worker reporting a test it never ran, and the costlier one: a false "done" is what the next agent builds on (2026-07-15: two state claims asserted from memory shipped inside commits already declared done).
...
| **Cited-capture content inspection** | For each evidence capture the return cites in `validation`: OPEN it and inspect content — it MUST be non-empty AND consistent with the claim it backs. This reads the capture from disk, beyond the parseable-field checks above (a qwen worker cited two EMPTY captures while quoting their supposed content; the kimi "work right, message wrong" case is the same family). | Empty / missing / contradicting cited capture = **evidence-husk** → treat as the fabricated-evidence class; route to recovery, do NOT accept `DONE`. Direction matters: husk is a capture that shows LESS than the claim it backs (claims success, capture is empty/shows failure). A capture that shows MORE held than a *`failed`* verdict claims is the opposite case — a verifier measurement bug reconciled to its own pixels per §3d, NOT a husk. |
...
**Exception — a verdict that contradicts its OWN evidence: the pixels win.** When a cold verifier's `failed` verdict contradicts the verifier's OWN captured screenshot (it grades an overlay "still visible" while its own capture shows it closed, or "missing" while its own capture shows it present), reconcile the verdict to the verifier's evidence — this is a MEASUREMENT bug in the verifier's check (e.g. a presence-based "visible?" boolean defeated by an opacity / `pointer-events` hide), not a real defect, and not a §3d FAIL. "Disk = truth" applies to the verifier's own return: its captures arbitrate over its verdict, never the builder's claim — so this does NOT reintroduce self-grade trust. The conductor's reconciliation MUST compare every `failed` verdict against the captures the verifier filed BEFORE routing to recovery. This binds ONLY the verdict-vs-its-own-capture self-contradiction in the success direction (the capture shows MORE held than the verdict claims); a genuine mismatch where the captures SUPPORT `failed` still FAILS per the rule above, and a capture showing LESS than a *held*/*success* claim is the §1b evidence-husk case (route to recovery), not this one. (Reduce it at the source: fold the floor's user-perceived-visibility requirement into the cold-verifier brief so its checks read opacity/pixels, not DOM presence.)
