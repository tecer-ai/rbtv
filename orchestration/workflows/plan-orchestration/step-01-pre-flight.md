...
If `run_mode: autonomous`, copy the template at `{rbtv_path}/orchestration/workflows/plan-orchestration/templates/autonomous-run-log-template.md` to `{plan_dir}/autonomous-run-log.md` (filling `{plan-name}` in the frontmatter and H1). If the file already exists, leave it intact and append new entries at the bottom — never overwrite.
