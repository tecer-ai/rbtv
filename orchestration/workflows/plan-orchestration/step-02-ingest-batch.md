...
- **If it exists AND conforms to the current template** (has Resume Point, Run Configuration, Approved Delegation Map, Completed Batches, Active State, Resume Capsule, and Notes for Resuming Agent sections per `{rbtv_path}/orchestration/workflows/plan-orchestration/templates/orchestration-state-template.md`): read it. The Resume Point, Approved Delegation Map, and Completed Batches table tell you where to pick up. Use this to skip already-done work in batching (next step) and dispatch (step-03).
