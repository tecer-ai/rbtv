...
### `variants[].launch_profile` — **RETIRED** (owner ruling 2026-08-11)

**The field no longer exists.** Owner ruling 2026-08-11 (executing
`d-r2-preflight-manual-plus-skill`): `launch_profile` is retired and the 11 recorded values were
dropped from the four CLI manifests in the same change. Recorded here rather than deleted so a
reader of an older manifest, task, or card knows what the key meant and why it is gone.

**What it was** (task 7.54): an optional string on a variant naming THE launch profile that
`(model, variant)` resolved through — a key of `ignite/config/spawn-profiles.yaml`, resolved by the
shared resolver `ignite/launch-profiles/` (task 7.42).

**Why it went.** Task 7.86 repointed every recorded value at a conductor-lane `cli-*` twin it was to
author; `r-seats-only-architecture` (2026-08-06) then retired that whole family, and the twins were
never authored — so all 11 mappings named profiles the shipped config does not have. Nothing read
the field either: routing (`route.py`) never did (see §4's table row), and manual invocation of the
`rbtv-dispatch-resolve` skill — where the conductor names the profile itself — is the permanent
contract.

⇒ **Every variant dispatches through its package manual**, as the 9 uncovered pairs already did.
Those manuals stay: they are the only home for exit codes, recovery protocols, resume mechanics,
failure modes and the per-model task contract, none of which ever had a field in the profile schema
...
| `auth` | yes | map | Auth requirement before dispatch. Sub-fields: `required` (bool), `method` (e.g. `cli-login`, `api-key`, `none`), `interactive` (bool — does first-time auth need a human, i.e. a USER-EXECUTED-ONLY pre-flight), `env_var` (OPTIONAL string — for `method: api-key` only: the exact API-key env-var name the availability probe checks, overriding the package-id derivation. Needed when a multi-provider package's backends authenticate under provider-specific key names the package id cannot produce, e.g. opencode: z1 → `ZHIPU_API_KEY`, sakana → `SAKANA_API_KEY`. Omitted → `route.py` derives `{PROVIDER}_API_KEY` from the package id, the canonical path), `credential_store` + `credential_store_key` (OPTIONAL pair — for `method: api-key` only: declares that this variant's credential ALSO resolves from a harness's own stored CLI login, so availability = env-var resolves **OR** the stored credential exists. See §4 availability note). |
...
| ~~`launch_profile`~~ | **RETIRED** (owner ruling 2026-08-11 — see §1). No manifest carries it; the conductor names the profile itself when it invokes `rbtv-dispatch-resolve`. It was never an ELECTION input, so routing cognition (`route.py`) is unaffected. |
...
| `auth.method: api-key` availability | **Availability = the credential resolves**, not a USER-EXECUTED login. Check order: OS environment variable first (e.g. `DEEPSEEK_API_KEY`), then the `env_file` path recorded in `rbtv.json`. The env-var name derives from the package id unless the variant declares `auth.env_var` (the per-backend override for multi-provider packages). If the key is absent in both → the variant is unavailable for this dispatch UNLESS it declares a credential store (next row); the router logs it and degrades (does not halt). This is distinct from `cli-login` workers, whose availability requires a USER-EXECUTED-ONLY login pre-flight (`auth.interactive: true`). For `method: api-key` workers, `auth.interactive: false` — availability is a credential-resolution test the runner performs, never a human gesture. |
| `auth.credential_store` — the second resolution path | A `method: api-key` variant MAY declare `credential_store` (the harness whose stored login also authenticates it, e.g. `opencode`) + `credential_store_key` (the provider id to look up in that store, e.g. `zai-coding-plan`). Then **availability = the env-var path above resolves OR that stored credential exists** — either is sufficient (owner ruling 2026-07-15). Rationale: a harness with its own credential store authenticates a backend from that store with NO API-key env var exported, so keying availability on the env var alone reports a demonstrably-working backend unavailable. The env-var path is tried FIRST and is unchanged; the store is a fallback, never a replacement. `route.py` (`_check_stored_credential`) reads the store FILE directly — the same file the CLI's own `auth list` names as its source — rather than spawning the CLI, because the gate runs at routing time per variant; presence of the provider key in the store IS the credential (the payload shape is not inspected, as it differs across auth types). Store-path resolution lives in `route.py`'s `CREDENTIAL_STORE_RESOLVERS` (opencode: `$XDG_DATA_HOME/opencode/auth.json`, else `~/.local/share/opencode/auth.json`), so no machine path is ever baked into a manifest. An unknown store id, absent/unreadable store, or malformed JSON reads as "no stored credential" (False) — never a crash. **Opt-in:** a variant that does not declare `credential_store` is gated on the env var exactly as before. |
