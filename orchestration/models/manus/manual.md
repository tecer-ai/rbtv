...
| **File artifacts fetched (shipped 2026-06-10, D-exec-14)** | The client parses `assistant_message.attachments[]` from `task.listMessages` (no `verbose=true` needed) and downloads each attachment into `--output-folder`. Defensive dual-auth download (D3/D6): GET with `x-manus-api-key`; on `401`/`403` OR a login-redirect detected by content-type mismatch (a `200` whose body is `text/html` while the attachment declared a non-HTML type), retry the same URL without the header. Login-page HTML is never saved. A per-file download failure is NON-FATAL: recorded in `raw_response["artifact_errors"]` and surfaced as a runner `concerns` line. Live-validated 2026-06-10: a paid task’s CSV deliverable landed in `--output-folder` alongside `raw-output.md`. |
