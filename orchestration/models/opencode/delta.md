...
| Guidance file in the WORKTREE | worktree root has `AGENTS.md`? | It will NOT by default (the vault gitignores mirror-generated guidance and a fresh worktree checks out only tracked files; the worktree does carry the tracked `CLAUDE.md` source) → generate it with the modern installer: `python {rbtv_path}/install2.py install --target "<worktree>" --guidance-basis CLAUDE.md` (it renders `AGENTS.md` beside every `CLAUDE.md`, skipping nested repos), or inline the load-bearing rules in the prompt. ⚠ The old engine `orchestration/models/mirror/mirror.py` (and the manifest's `mirror_entry`) is RETIRED and REFUSES — owner ruling `d-hard-guard-retire-model-mirror`, 2026-08-10; it would overwrite the installer-owned guidance files. |
