...
| Guidance file | the LAUNCH ROOT (orchestrator root — the `-C`/`--cd` value) has `AGENTS.md`? | Checked at the launch root, NEVER the work-target (the work-target needs no guidance file — the mirror skips nested repos by design). Absent at the launch root → generate it with the modern installer: `python {rbtv_path}/install2.py install --target "<launch-root>" --guidance-basis CLAUDE.md` (it renders `AGENTS.md` beside every `CLAUDE.md`, skipping nested repos), or inline the load-bearing rules in the prompt. ⚠ The old engine `orchestration/models/mirror/mirror.py` (and the manifest's `mirror_entry`) is RETIRED and REFUSES — owner ruling `d-hard-guard-retire-model-mirror`, 2026-08-10; it would overwrite the installer-owned guidance files. Codex natively reads `AGENTS.md` from its working root. |
