...
**Routing implication:** codex CAN self-validate a capability-run leaf on Windows whose check uses `git`/`node`/`pwsh`/`cmd` (e.g. a `git status` clean-tree check or a `node` smoke). The author/validate split is therefore NO LONGER the blanket default — apply it ONLY when validation requires a class the approval router declines (notably **`python`/`pytest`-based** test or build runs) OR a **live-call** (real/network API). In those cases **split the dispatch** — codex authors the edit ONLY, and the CONDUCTOR runs the validation as an exit-probe (the Invariant-4 decide→resume shape; codex still authors any follow-on fix) — or route the leaf to kimi-code-cli. Do NOT blanket-split every codex capability-run on Windows. Two sub-points on the wider sandbox grammar:

- **Live-call / network is SEPARATELY UNTESTED here** — these probes forbade network. `-c sandbox_workspace_write.network_access=true` governs outbound network *within* the sandbox; the design-module-closeout reported a live API call blocked, and that live-call concern is NOT overturned by the subprocess-spawn finding. Treat a live-call (real/network API) leaf as still requiring the split or a network-enabled sandbox until separately tested.
...
| **Windows `workspace-write`: `python`/`pytest` + live-call need the split; other subprocesses RUN** | On Windows the sandbox edits files AND spawns subprocesses — `git`, `node` (incl. `node -e`), `pwsh`, `cmd`, `where.exe` all execute (TESTED 2026-06-14, two probes). The only block is codex's APPROVAL ROUTER declining `python`/`python -c` (`approval required by policy, AskForApproval=Never`), NOT a spawn-capability limit; the node `spawn setup refresh` infra failure no longer reproduces. So codex CAN self-validate a git/node/pwsh/cmd-based check — SPLIT the dispatch (codex edits only; conductor runs validation as an exit-probe) or route to kimi-code-cli ONLY for python/pytest-validated leaves or live-call (real/network API) leaves, never every capability-run. `danger-full-access` does NOT change the python decline (TESTED 2026-06-13 — the approval router is orthogonal to sandbox mode); live-call/network behavior is separately UNTESTED in these probes. See the sandbox-grammar section above. |
