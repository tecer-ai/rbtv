...
- `--sandbox danger-full-access` (which disables the sandbox entirely) does **NOT** lift the Windows subprocess block (TESTED 2026-06-13, codex-cli 0.137.0, Windows 11, owner-sanctioned probe): a headless `danger-full-access` + `approval_policy="never"` dispatch still refused `python --version`, declined router-level in 0ms with `approval required by policy, but AskForApproval is set to Never` — the approval-router decline (same `approval required by policy` family as the `workspace-write` denial), NOT the `spawn setup refresh` error. Because headless REQUIRES `approval_policy="never"` (any other policy stalls a non-TTY run), `danger-full-access` + `never` is the only headless-viable configuration and it still blocks the subprocess. Disabling the sandbox is therefore NOT a workaround; the author/validate split (or routing the run-leaf to kimi-code-cli) remains the default.
