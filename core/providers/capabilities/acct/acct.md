---
description: "Every AI provider account on this box: park logins in named slots, switch without re-logging-in, read each plan's limits"
---
# acct
...
1. **Switch** the harness between accounts of one provider **without re-logging-in**, so agents
   keep calling the plain `claude` / `codex` binary. No wrapper command, no
...
acct doctor [--json]              # what this box can run: harness binaries installed, then
                                  # every provider a credential exists for — including the ones
                                  # reached only through opencode's store (`cast doctor` runs
                                  # this same command)
...
```bash
ln -s "$(pwd)/3-resources/tools/rbtv/core/providers/capabilities/acct/tool/acct.py" ~/.local/bin/acct
```
...
| kimi | — | ✅ | Kimi models run through **opencode** (`kimi-for-coding`) since 2026-08-14 — the standalone `kimi` CLI and its OAuth login are gone, so there is one `sk-kimi` key in opencode's store and nothing to switch between |
...
`oauthAccount.accountUuid` (claude) or `tokens.account_id` (codex).
