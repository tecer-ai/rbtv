...
them without re-logging-in, and read each account's plan limits.
  ...
  acct providers                    where each usage figure comes from, per provider
  acct doctor [--json]              what this box can run: harnesses installed, providers
...
OPENCODE_STORE_KEYS = {"zai": "zai-coding-plan", "deepseek": "deepseek", "sakana": "sakana",
                       "google": "google", "kimi": "kimi-for-coding", "xai": "xai"}
...
# The harness binaries `cast` can launch. A provider is WHOSE entitlement burns; a harness is
# WHICH software spends it — the two lists overlap (claude, codex) without being the same
# question, so `providers` answers both and `cast doctor` reads this half back.
HARNESS_BINS = ("claude", "codex", "opencode")
...
(read-only; keys are never printed and are sent ONLY to that provider's own endpoint):
  ...
  kimi      usage only. Kimi models run through OPENCODE now (its `kimi-for-coding`
            provider, 2026-08-14) — the standalone `kimi` CLI and its OAuth login are gone,
            so there is nothing to switch between. Its sk-kimi key (kimi.com/code/console)
            lives in opencode's store and polls GET api.kimi.com/coding/v1/usages.
...
def harnesses():
    """{binary: {ok, path}} for every harness `cast` can launch — install state, nothing more."""
    return {h: {"ok": bool(shutil.which(h)), "path": shutil.which(h)} for h in HARNESS_BINS}
...
def cmd_ls(prov=None):
    ...
    codex merely unparked, bare `acct` listed claude and nothing else — a switchable
    provider that was logged in the whole time simply did not appear, and that silence read
...
def kimi_window(detail, label):
    """One {limit, used, resetTime} block — kimi sends the counts as STRINGS, and a plain
    `if not r.get("limit")` on them is not the trap; `int()` on them is what the old parser
    skipped by reading a different response shape entirely."""
    try:
...
def parse_kimi_code(d):
    """Kimi Code plan usage — measured shape of GET api.kimi.com/coding/v1/usages, 2026-08-14:
    `limits[]` rows each carry a `window` (duration + timeUnit) and a `detail` counter, and a
    top-level `usage` holds the plan-wide counter (whose own window the payload does not name,
    hence the bare `plan` label)."""
    windows = []
    for row in d.get("limits") or []:
        win = (row.get("window") or {})
        mins = (win.get("duration") or 0) * (60 if win.get("timeUnit") == "TIME_UNIT_HOUR" else 1)
        label = f"{mins // 60}h" if mins and mins % 60 == 0 else (f"{mins}m" if mins else "window")
        w = kimi_window(row.get("detail") or {}, label)
        if w:
            windows.append(w)
    plan = kimi_window(d.get("usage") or {}, "plan")
    if plan:
        windows.append(plan)
    if not windows:
...
def kimi_usage(key):
    ...
    return {"error": "no credential"} if key else \
        {"note": "no key in opencode's store · console only", "console": CONSOLE_URLS["kimi"]}
...
def usage_rows(provider=None, which=None):
    ...
    for prov in want:
        ...
        if prov == "codex":
            ...
            data = kimi_usage(opencode_key("kimi"))
...
def doctor_report():
    """What this box can actually run, in two halves: which HARNESS binaries are installed, and
    which PROVIDERS are enabled (a credential exists). The halves are separate questions — an
    installed opencode is worth nothing without a provider key, and a provider key routes through
    opencode whether or not it is one of the four harnesses.

    The provider half is read off the credential stores themselves, NOT off a hardcoded list:
    opencode routes to as many providers as its store holds, and a fixed list would silently
    omit a key added yesterday. Names, presence, and slot names only — never a key or a token.
    """
    provs = {}
    for p in PROVIDERS:
        provs[p] = {"enabled": bool(live_snapshot(p)), "via": "login",
                    "slots": slot_names(p), "active": active(p)}
    known = {v: k for k, v in OPENCODE_STORE_KEYS.items()}
    for store_key, entry in sorted(opencode_store().items()):
        name = known.get(store_key, store_key)
        # a switchable provider keeps its login row; the store entry is the same account's key
        provs.setdefault(name, {
            "enabled": bool((entry or {}).get("key") or (entry or {}).get("access")),
            "via": f"opencode store ({store_key})", "slots": [], "active": None})
...
def doctor_lines():
    rep = doctor_report()
    out = ["harnesses installed  (what `cast` launches)"]
    for h, r in rep["harnesses"].items():
        out.append(f"  {h:<11} {r['path'] if r['ok'] else 'MISSING'}")
    out.append("")
    out.append("providers enabled  (whose compute this box can buy)")
    for name, r in rep["providers"].items():
        tail = ""
        if r["slots"]:
            tail = (f" · {len(r['slots'])} slot(s): {', '.join(r['slots'])}"
                    f" · active {r['active'] or '(none saved — run `acct %s add <name>`)' % name}")
        elif r["via"] == "login" and r["enabled"]:
...
def selftest():
    ...
    # counts arrive as STRINGS and the 5h window is a 300-MINUTE row (measured payload)
    _kimi = parse_kimi_code({
        "usage": {"limit": "100", "used": "3", "resetTime": "2026-08-17T16:45:32.890734Z"},
        "limits": [{"window": {"duration": 300, "timeUnit": "TIME_UNIT_MINUTE"},
                    "detail": {"limit": "100", "used": "1",
                               "resetTime": "2026-08-14T13:45:32.890734Z"}}]})["windows"]
    ck("kimi-code", [(w["label"], w["pct"]) for w in _kimi] == [("5h", 1.0), ("plan", 3.0)])
    ck("kimi-code-resets", _kimi[0]["resets_at"] == int(
        datetime.fromisoformat("2026-08-14T13:45:32.890734Z").timestamp()))
    ck("kimi-code-empty", "error" in parse_kimi_code({"limits": [], "usage": {}}))
    ...
    # harness install map: a binary that cannot exist must read `missing`, not vanish from the list
    global HARNESS_BINS
    _real = HARNESS_BINS
    HARNESS_BINS = ("sh", "no-such-harness-xyz")
    try:
        ck("harness-map", harnesses() == {"sh": {"ok": True, "path": shutil.which("sh")},
                                          "no-such-harness-xyz": {"ok": False, "path": None}})
        ck("doctor-harness-missing",
           any(l.strip().startswith("no-such-harness-xyz") and l.endswith("MISSING")
               for l in doctor_lines()))
    finally:
        HARNESS_BINS = _real
    # every provider acct knows about gets a row, credential or not — absence is a state here
    ck("doctor-covers-providers",
       all(p in doctor_report()["providers"] for p in USAGE_PROVIDERS))
...
def main(argv=None):
    ...
    ap.add_argument("--json", action="store_true",
                    help="machine-readable output (usage rows; harness map for `providers`)")
    ...
    if cmd == "providers":
        ...
        print(json.dumps(doctor_report(), indent=2) if args.json else "\n".join(doctor_lines()))
    elif cmd == "usage":
