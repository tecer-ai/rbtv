...
from .base import Artifact, Message, ProviderClient, ProviderConfig, ProviderName, RequestOptions, Response, TokenUsage
...
class GeminiClient(ProviderClient):
    ...
    def _build_payload(self, messages: List[Message], options: RequestOptions) -> Dict[str, Any]:
        ...
        if options.extra_params is not None:
            ...
            image = bool(options.extra_params.get("image"))
        ...
        # Image generation gets NO default cap: an image's cost is not a token budget the
        # caller can reason about, and the models reject or mis-apply a text cap.
        generation_config: Dict[str, Any] = {}
        if not image:
            default_max_output = _GROUNDED_MAX_OUTPUT_TOKENS if grounding else _DEFAULT_MAX_OUTPUT_TOKENS
            generation_config["maxOutputTokens"] = options.max_tokens or default_max_output
        elif options.max_tokens is not None:
        ...
        if grounding:
            ...
            # The image models answer on the same generateContent endpoint; what makes it an
            # image call is the requested MODALITY. TEXT rides along because these models
            # narrate, and refusing it can fail the whole request.
    ...
    def _parse_response(self, data: Dict[str, Any], model: str) -> Response:
        ...
        artifacts = _inline_images(content_parts)
...
# Image parts come back INLINE (base64) on generateContent — there is no URL to fetch, so the
# bytes are the artifact. Both spellings of the key are accepted: the REST JSON uses inlineData,
# the proto/py SDK shape uses inline_data, and which one a response carries is not worth a bug.
_MIME_EXT = {"image/jpeg": "jpg", "image/svg+xml": "svg"}


def _inline_images(parts: List[Dict[str, Any]]) -> List[Artifact]:
    out: List[Artifact] = []
    for part in parts:
        ...
        ext = _MIME_EXT.get(mime) or mime.rsplit("/", 1)[-1].split("+")[0] or "bin"
        try:
        ...
        if not raw:
            continue
        out.append(Artifact(filename=f"image-{len(out) + 1}.{ext}", content=raw, content_type=mime))
    return out
