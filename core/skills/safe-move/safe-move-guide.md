...
2. Finds every reference to `old` — markdown links, wikilinks, frontmatter values, config paths, code imports, paths written as inline code in markdown prose, and any literal occurrence of the exact `old` path elsewhere (bare prose, inside a command span, a config header).
