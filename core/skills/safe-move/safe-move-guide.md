...
| term | The human-readable NAME a registry/ontology record carries in addition to its filename (e.g. `goal contract`). References written in the term form — a relation edge `{ verb: composed-of, to: goal contract }`, or prose — contain no path and no filename, so NO path matcher can see them. `--term` scans for them; see **Term references** below. |
...
- **Prints** the summary: move method; per-class counts; every ACTIONABLE `surface` record inline as a ready-to-paste `id:hash` pair with its `file:line`, syntax, and `match -> proposed`; warnings; the report path. A surfaced record whose `proposed` EQUALS its `match` needs no edit — the reference stays correct after the move (a bare basename that does not change; a relative path between two files that move together) — so it collapses to a `surface: N no-op` count instead of padding the decision set. The records themselves are still in the report.
...
| `--term <term>` | Also scan for references to this concept TERM (see **Term references**). Repeatable. Defaults to the moved record's own `term:` frontmatter value when it has one. |
| `--new-term <term>` | The term's replacement, for a rename. Omit for a retirement — every term reference is then surfaced with no proposed rewrite. |
| `--no-term-scan` | Skip the term scan entirely, including the `term:` frontmatter auto-derivation. |
...
**What the walk skips by name is VCS and dependency trees only** — `.git`, `node_modules`, `__pycache__`, `.venv`/`venv`, `.tox`, and the tool caches. **`build/`, `dist/`, and `target/` are NOT skipped by name.** They were, and it blinded the referrer scan wherever a workspace keeps hand-authored records under `build/` — on one campaign move the tool reported 0 references when ~40 were real, ~90% of them under `build/`. Genuine build OUTPUT is gitignored and the walk already skips every gitignored path; a TRACKED `build/` holds content. Pass `--exclude 'build/*'` if a particular one must be skipped anyway.
...
## Term references — for registry / ontology records

A registry record is named by a human-readable **term** as well as by its filename, and its load-bearing references are written in the term form: a relation edge (`- { verb: composed-of, to: goal contract }`) or prose. Those contain no path and no filename, so the path matchers cannot see them — retiring or renaming such a record silently leaves every one of them behind.

- **Turning it on.** `--term "<term>"` (repeatable). With no `--term`, the tool reads the moved file's own `term:` frontmatter field and scans for that; a target without one produces no term references and costs nothing. `--no-term-scan` disables both.
- **Rename vs retirement.** `--new-term "<term>"` supplies the replacement. Omit it (a retirement) and every term reference is surfaced with an empty `proposed` — the review set, with nothing auto-applied.
- **Two syntaxes, two risk levels.** `term-edge` is the structured mapping form — a value slot holding the term and nothing else, on a line that is purely mapping-shaped (`term: x`, `- to: x`, `- { verb: v, to: x }`). It is unambiguous, so it can be `auto`. `term-prose` is any other word-bounded occurrence and is ALWAYS `surface`: only you can tell a reference from an incidental use of the same words. A `key: value` fragment inside a running sentence is prose, not an edge.
- **Matching is case-sensitive** — a term is a chosen name, and case-folding it would sweep up ordinary sentences.
- **Scope.** The edge form is scanned in markdown and config files; the prose form in markdown only.
- **It does not know your graph.** The scan finds term text in files. An edge that exists only as a derived inverse (stated on no record) has no text to rewrite and will not appear — that is correct, not a miss.

```
python -m safe_move consult <abs-old> <abs-new> --term "goal contract" --new-term "cognitive unit"
python -m safe_move consult <abs-old> <abs-new> --term "goal contract"     # retirement: surface everything
```
...
- **A file in a language the AST backend does not cover** (`.sh`, `.ps1`, `.swift`, `.kt`, …) raises a `code-unsupported-language` warning. That warning means only that STRUCTURAL matching was not attempted for it — the file is still read and swept by the literal path sweep, so a path written out in full inside it IS caught. What may be missed there is an import-style specifier (a module/require name).
- **A reference between two files that move TOGETHER needs no rewrite** and is reported as "no change" (`proposed == match`), never as an empty rewrite. The resolution FORM is read at the reference's original location and only the resulting VALUE is recomputed for its post-move location; a file-relative sibling path therefore computes back to itself. (Reading the form at the post-move location made every such reference unresolvable, which surfaced as an EMPTY `proposed` — applying one would have DELETED the path token from the file.)
...
- **Literal occurrences of the exact `old` path are also detected and surfaced** (`literal-path`, never `auto`), wherever they appear — bare prose, inside a command span (`` `/cmd … old/path/x.md` ``), a config header, or HTML. This catches the self-referential case: a moved folder whose own files name its old path. The match is bounded by path delimiters, so a longer sibling (`old/path-other`) or a path under a different parent (`vendor/old/path`) is never matched, giving near-zero false positives. **Which written forms are searched:** the same path is written several ways in the wild, and the sweep searches FOUR of them, rewriting each hit in the form it was written in — scope-root-relative (`proj/refs`), **workspace-root-relative** (`1-projects/proj/refs` — how a reference is written when the scan is scoped to a subtree, so the path begins with the scope root's OWN directory name), **absolute POSIX**, and **absolute Windows-backslash** (`C:\…\proj\refs`). The workspace-relative form is why a `--scope-root`-narrowed run now sees such references at all: scope-relative matching reads that leading `1-projects/` as a DIFFERENT parent and skipped the site, so every live referrer of a real folder move came back as `references: 0`. A same-site hit found by two forms is reported once. A **relative** form (`../old/x.md`) is still not matched literally (those that resolve via a structured syntax are caught by the other matchers; the rest are left untouched, never corrupted). For a **folder** move, the moved folder's OWN files are swept even under skip-named dirs (`build/` / `dist/` / `target/`) — a project's records there are content being relocated, so EVERY matcher — markdown link, wikilink, frontmatter, config, code import — AND the literal sweep run over them: a structured reference (e.g. a markdown link or config path) to the old location inside a moved record is detected and classified like any other reference, not only caught as a bare literal, even though such dirs are skipped everywhere else in the scope. **This self-reference sweep requires the moved folder to be WITHIN the scan scope:** when the moved folder sits OUTSIDE `--scope-root`, its own files are NOT swept and a `moved-folder-out-of-scope` warning says so — in-scope references to it are still surfaced (with a workspace-root-relative proposed); re-run without `--scope-root` (or with a `--scope-root` that contains the moved folder) to also catch self-references inside the moved tree.
...
- **A bare-name frontmatter value additionally needs a UNIQUE basename, and is never `auto`.** `allowlist: roadmap.md` matches only when exactly one in-scope file is named `roadmap.md` — the same uniqueness rule the wikilink, inline-code, and prose matchers apply. Without it the least unique basenames in a workspace (`CLAUDE.md`, `README.md`) matched every frontmatter field naming them, and an unrelated project's task spec was reported as a reference to a moved folder's own `CLAUDE.md`. Even when unique it stays `surface`: the rewrite replaces a bare name with a FULL PATH, inside a file that may well own its own same-named file — auto-applying that would corrupt it. An explicit `[[wikilink]]` value is exempt from both rules (the syntax itself declares a link).
...
- Scan time scales with the **scope size**, not the move size. The default scope is the git top level of `old`; on a large repo/vault (~10k files) a whole-scope scan takes tens of seconds (each file is read once; ast-grep runs O(languages), not O(files)). Since `build/`/`dist/`/`target/` are no longer skipped by name, that scope is wider than it used to be — measured at +21% of walked files on one real vault, and those are exactly the files whose referrers were previously invisible. For a fast targeted move, pass `--scope-root <subtree>` to limit the scan to where references can plausibly live.
