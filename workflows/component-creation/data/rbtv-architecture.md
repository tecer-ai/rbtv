...
6. **Is it an ENTRY POINT that loads an agent or workflow?**
   - YES → **THIN LOADER** (skill or command — zero logic, delegates to workflow/task/agent)
...
| Hardcoded paths | Breaks when moved or reconfigured | Use path variables ({rbtv_path}, output resolution) |
...
|--------|-----------|-----------|
| IDE Command | Agent file | `{rbtv_path}` path in LOAD instruction |
| Agent menu | Workflow/Task | `exec`, `workflow`, or `template` attribute |
| Agent activation | config.yaml | Hardcoded {rbtv_path} path (step 2) |
...
| Create an entry point for IDE | Thin Loader (skill or command) |
